...
if (
  $_GET["id_reg"] || $_GET["jenis"]  || $_GET["ket"] || $_GET["dis"] || $_GET["disper"]
  ...
  //AMBIL DATA DISKON  
  $sql = "select * from ar_ap.diskon where 
             id_reg = " . QuoteValue(DPE_CHAR, $_POST["id_reg"]) . " 
            and pembayaran_id=" . QuoteValue(DPE_CHAR, $_POST['id_pembayaran']);
  $dataRincianDiskon = $dtaccess->FetchAll($sql);
  ...
  if ($dataDiskon["pembayaran_det_flag"] == "T") {
    $flag = "01";
  } elseif ($dataDiskon["pembayaran_det_flag"] == "P") {
    $flag = "02";
  } elseif ($dataDiskon["pembayaran_det_flag"] == "S") {
    $flag = "03";
  }
  ...
  for ($i = 0, $n = count($dataPembDet); $i < $n; $i++) {
    if ($dataPembDet[$i]["pembayaran_det_flag"] == "T") {
      $flag = "01";
    } elseif ($dataPembDet[$i]["pembayaran_det_flag"] == "P") {
      $flag = "02";
    } elseif ($dataPembDet[$i]["pembayaran_det_flag"] == "S") {
      $flag = "03";
    }
  }
...
<head>
  ...
  <title>Cetak Kwitansi Pembayaran R. Inap</title>
  <style>
    @media print {
      #tableprint {
        display: none;
...
<body onload="window.print();">
  ...
  <td align="center" width="100%" style="padding-left:200px">&nbsp;<font size="2"><?php echo $dataDiskon["pembayaran_det_kwitansi"]; ?><font size="2"></td> 
</table>-->
  <br>
  <table width="100%" border="0" cellpadding="1" cellspacing="0" style="border-collapse:collapse">
    <tr>
      <td align="center"><img src="<?php echo $fotoName; ?>" height="75"> </td>
      <td align="left" id="judul">
      ...
      <td width="60%"><b><u> KWITANSI</u></b></td>
    </tr>
  </table>
  <br>
  <hr>
  ...
  <table width="100%" border="0" cellpadding="1" cellspacing="0" style="border-collapse:collapse">
    ...
    <tr>
      ...
      <td align="left" width="20%"><?php echo $dataDiskon["pembayaran_det_kwitansi"]; ?></td>
    </tr>
    <tr>
      <td width="5%"></td>
      <td align="left" width="7%">Sudah terima dari </td>
      <td width="1%">:</td>
      <td align="center" width="1%"></td>
      <?php
  ...
  <?php } ?> -->
    ...
    <?php
    $totalTagihan = $_GET["totalbiayaservice"];
    $bayar = ($_GET["dibayar"] + $_GET["dibayar2"] + $_GET["dibayar3"] + $_GET["dis"] + $_GET["deposit_awal"]);
    $diskonNominal = $_GET["dis"];
    $kurangBayar = $_GET["total"] > $bayar;
    $kurangBayarNominal = $_GET["total"] - $_GET["dibayar"] - $_GET["dis"];



    $depositNominal = $_GET["deposit"];
    $depositNominalAwal = $_GET["deposit_awal"];
    $sisaDeposit = $_GET["deposit_awal"] - ($_GET["totalbiayaservice"] - $_GET["dis"]);

    $pembayaranNominal = $_GET["dibayar"] + $_GET["dibayar2"] + $_GET["dibayar3"] + $depositNominal;
    ?>
    <tr>
      <td width="5%"></td>
      <td align="left" width="7%">Banyaknya Uang</td>
      <td width="1%">:</td>
      <td align="center" width="1%"></td>
      <?php if ($kurangBayar > 0) {  ?>
        <td align="left" width="20%" bgcolor="#CCCCCC">&nbsp;<?php echo terbilang($_GET["dibayar"]); ?> Rupiah</td>
      <?php } else if ($depositNominalAwal > $totalTagihan) { ?>
        <td align="left" width="20%" bgcolor="#CCCCCC">&nbsp;<?php echo terbilang($pembayaranNominal); ?></td>
      <?php } else { ?>
        <td align="left" width="20%" bgcolor="#CCCCCC">&nbsp;<?php echo terbilang($pembayaranNominal); ?> Rupiah</td>
      <?php } ?>
    </tr>
    <tr>
      <td width="5%"></td>
      <td align="left" width="7%">Untuk Pembayaran</td>
      <td width="1%">:</td>
      <td align="center" width="1%"></td>
      <td align="left" width="20%"><?php echo $dataDiskon["pembayaran_det_ket"]; ?></td>
    </tr>
  </table>
  <br>
  ...
  <table width="100%" border="0">
    <tr>
      <td width="5%"></td>
      <td align="center" width="95%" style="padding-left:400px">Jombang <?php //echo $konfigurasi["dep_kota"];
    ...
    ?>
      <tr>
        ...
        <td align="left" width="85%">
          <hr width="25%" align="left"><b><i>Jumlah Rp. </i></b><?php echo currency_format($_GET["dibayar"]); ?>
          <hr width="25%" align="left">
        </td>
      </tr>
      <tr>
        <td width="15%"></td>
        <td align="left" width="85%">Kekurangan bayar : Rp. <?php echo currency_format($kurangBayarNominal); ?></td>
      </tr>
    <?php } else if ($depositNominalAwal > $totalTagihan) { ?>
      <tr>
        <td width="15%"></td>
        <td align="left" width="85%">
          <hr width="25%" align="left"><b><i>Jumlah Rp. <i><b><?php echo currency_format($pembayaranNominal) ?></b></i>
              <hr width="25%" align="left">
        </td>
      </tr>
    <?php } else { ?>
      <tr>
        <td width="15%"></td>
        <td align="left" width="85%">
          <hr width="25%" align="left"><b><i>Jumlah Rp. </i></b><?php echo currency_format($pembayaranNominal); ?>
          <hr width="25%" align="left">
        </td>
      </tr>
    <?php } ?>
    ...
    <? if ($diskonNominal > 0) { ?>
      <?php foreach ($dataRincianDiskon as $drd) : ?>
        <tr>
          <td width="15%"></td>
          <td align="left" width="85%"><small style="font-size: 8px;"><?= $drd['diskon_nama'] ?></small> : Rp. <?php echo currency_format($drd['diskon_nominal']); ?></td>
        </tr>
      <?php endforeach; ?>
      <tr>
        <td width="15%"></td>
        <td align="left" width="85%">Total Diskon : Rp. <?php echo currency_format($diskonNominal); ?></td>
      </tr>
    <? } ?>
    <!-- <? if ($depositNominal > 0) { ?>
      <tr>
        <td width="15%"></td>
        <td align="left" width="85%">Pembayaran Deposit : Rp. <?php echo currency_format($depositNominal); ?></td>
      </tr>
    <? } ?> -->
    <? if ($depositNominalAwal > $totalTagihan) { ?>
      <tr>
        <td width="15%"></td>
        <td align="left" width="85%">Sisa Deposit : Rp. <?php echo currency_format($sisaDeposit); ?></td>
      </tr>
    <? } ?>
    <tr>
      <td width="5%"></td>
    ...
    <tr>
      <td width="5%"></td>
      <td width="95%" align="center" style="padding-left:400px">(<?php echo $userName; ?> )</td>
    </tr>
  </table>
  </div>
</body>
