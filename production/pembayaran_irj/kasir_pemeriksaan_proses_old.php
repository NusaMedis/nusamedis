...
for ($i = 0, $n = count($dataTable); $i < $n; $i++) {
  ...
  if ($dataTable[$i]["fol_jenis"] <> "OA") $totalBiayaNonFarmasi = $totalBiayaNonFarmasi + $dataTable[$i]["fol_nominal"];
...
{
  $serviceCharge = ceil((10 / 100) * $totalBiayaNonFarmasi);
}


$grandTotalHargax = $totalHarga - $uangmuka["total"] + $serviceCharge;
...
if ($satuan >= 50) {
  $selisih = 100 - $satuan;
  $pembulatan_awal = $grandTotalHargax + $selisih;
} else {
  $selisih = 100 - $satuan;
...
$_POST['regid'] = $_GET['id_reg'];
$_POST['idcustusr'] = $_GET["id_cust_usr"];
$_POST['idpoli'] = $_GET["id_poli"];
...
if ($_POST["btnBayar"]) {
  ...
  // $kurangBayarNominal = $_POST["total"] - $_POST["dibayar"] - $_POST["txtDiskon"];
  // echo $_POST['total_harga']."x".$bayar;die();
  if ($kurangBayar > 0) {
    ...
    $dbValue[6] = QuoteValue(DPE_NUMERIC, $kurangnya);
    $dbValue[7] = QuoteValue(DPE_NUMERIC, $kurangnya);
...
<script type="text/javascript">
  ...
  function GantiDiskon(diskon) {
    ...
    kembalian = (tagihan_int - totaldibayar) - diskon_int + bulat;
  ...
  <div class="container body">
    <div class="main_container">
      ...
      <div class="right_col" role="main">
              ...
              <fieldset>
                ...
                <div id="kasir">
                  <table style="width: 100%;" role="grid" class="table table-striped table-bordered" cellspacing="0" width="100%">
                    ...
                    if ($satuan >= 50) {
                      $selisih = 100 - $satuan;
                      $pembulatan_awal = $total_awal + $selisih;
                    } else {
                      $selisih = 100 - $satuan;
                      $pembulatan_awal = $awal;
                    }
                    ...
                    <tr>
                      ...
                      <td class="tablesmallheader" width="15%" colspan='2' align='right'><?php echo "<b>Rp. " . number_format($totalBiaya, 2, '.', ',') . "</b>"; ?></td>
        ...
        <input type="hidden" name="cust_usr_kode" value="<?php echo $_POST["cust_usr_kode"]; ?>" />
    ...
    <?php if ($cetak == "y") { ?>
      ...
      <?php if ($_POST["reg_tipe_rawat"] == 'J' || $_POST["reg_tipe_rawat"] == 'R') { ?> //JIKA RAWAT JALAN
        <?php if ($_POST["dep_cetak_rincian"] == 'y') { ?>
          BukaWindow('cetak_kwitansi_rincian.php?dep_bayar_reg=<?php echo $_POST["dep_bayar_reg"]; ?>&id_reg=<?php echo $_POST["id_reg"]; ?>&ket=<?php echo $_POST["fol_keterangan"]; ?>&dis=<?php echo StripCurrency($_POST["txtDiskon"]); ?>&disper=<?php echo $_POST["diskonpersen"]; ?>&pembul=<?php echo $_POST["pembulatan"]; ?>&total=<?php echo $_POST["total_harga"]; ?>&dibayar=<?php echo StripCurrency($_POST["txtdibayar1"]); ?>&pembayaran_det_id=<?php echo $pembDetId; ?>&uangmuka_id=<?php echo $uangmukaId; ?>', 'Kwitansi');
