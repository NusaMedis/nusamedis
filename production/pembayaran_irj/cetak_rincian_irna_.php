<?php
  ...
  if($_GET["id_reg"] ) {
    ...
    $sql = "select sum(fol_nominal) as total from klinik.klinik_folio where fol_lunas = 'n' and id_pembayaran = ".QuoteValue(DPE_CHAR,$_GET['pembayaran_id'])." and id_biaya <> '9999999'";
    $TindakanJasa = $dtaccess->Fetch($sql);

    $JasaRS = $TindakanJasa['total'] * 0.1;

    $sql = "select sum(fol_nominal) as total, min(fol_waktu) as tindakan_pertama from klinik.klinik_folio where fol_lunas = 'n' and id_pembayaran = ".QuoteValue(DPE_CHAR,$_GET['pembayaran_id']);
    $TindakanJasa = $dtaccess->Fetch($sql);
    ...
    $sql = "select b.biaya_jenis_sem, c.jenis_tindakan_nama
      ...
      where a.fol_lunas='n'  
      and a.id_pembayaran =".QuoteValue(DPE_CHAR,$_POST["id_pembayaran"]);
  ...
  $tgl_pertama = date_format(date_create($TanggalRegistrasi['reg_tanggal']), 'Y-m-d');
   $tgl_kedua = date_format(date_create($TanggalRegistrasi['reg_tanggal_pulang']), 'Y-m-d');
  
  $tgl1 = new DateTime($tgl_pertama);
  ...
  <body>
        ...
        <?php
          ...
          for($a = 0, $s = count($dataFolio); $a < $s; $a++){
            ...
            if($dataFolio[$a]['biaya_jenis_sem'] == 'OA'){
              $sql = "select b.item_nama, b.penjualan_detail_harga_jual, b.penjualan_detail_jumlah, b.penjualan_detail_total, b.penjualan_detail_tuslag, a.penjualan_biaya_racikan 
          ...
          <tr>
            ...
            $sql = "SELECT sum(a.fol_nominal) as tot from klinik.klinik_folio a left join klinik.klinik_biaya b on a.id_biaya = b.biaya_id
              ...
              and id_pembayaran =".QuoteValue(DPE_CHAR,$_POST["id_pembayaran"])." and fol_lunas = 'n'";
              // echo $sql;
    ...
    <table style="width: 17cm;">
      ...
      <tr>
        ...
        <td class="rig"> Rp. </td><td class="nom lef"><?php echo currency_format($subtot - $deposit - $dataPasien['pembayaran_dijamin'] - $dataPasien['pembayaran_diskon']);?></td>
      </tr>
    </table>
  </body>
</html>