...
for($indexBayar=0,$jumBayar=3;$indexBayar<$jumBayar;$indexBayar++)   // Awal Looping Total Pembayaran dipaten 3 karen
{
    ...
    if ($_POST["txtdibayar".($indexBayar+1)]<>'') 
    {
      ...
      if($dataPas["reg_jenis_pasien"]==TIPE_PASIEN_UMUM) {
        ...
        $bayar = StripCurrency($_POST["txtdibayar1"]) + StripCurrency($_POST["txtdibayar2"]) + StripCurrency($_POST["txtdibayar3"]) + StripCurrency($_POST["txtDiskon"]) + StripCurrency($_POST["deposit_nominal"]) + $pembayaran_det_pembulatan;      
