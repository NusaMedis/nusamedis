<?php
    if(StripCurrency($_POST['pembayaran_det_pembulatan'])!=0){
          // echo StripCurrency($_POST['pembayaran_det_pembulatan']);die();
      ...
      $dbTable = "gl.gl_buffer_transaksidetil";
          ...
          $dbValue[2] = QuoteValue(DPE_CHAR,$datacoaUM["dep_coa_pembulatan"]);
          $dbValue[3] = QuoteValue(DPE_CHAR,$keterangan);
          $dbValue[4] = QuoteValue(DPE_CHAR,$_POST["job_id"]);
          $dbValue[5] = QuoteValue(DPE_CHAR,$depId);  
        if(StripCurrency($_POST['pembayaran_det_pembulatan'])>0){
          $dbValue[6] = QuoteValue(DPE_NUMERIC,'-'.StripCurrency($_POST['pembayaran_det_pembulatan']));
        }else{
          $dbValue[6] = QuoteValue(DPE_NUMERIC,str_replace("-", "", StripCurrency($_POST['pembayaran_det_pembulatan'])));
        }
         // print_r($dbValue);   die();
