...
//	   $poli = $auth->GetPoli();
   ...
   if(!$auth->IsAllowed("man_ganti_password",PRIV_CREATE)){
     ...
     if(!$_POST["tanggal_awal"]) $_POST["tanggal_awal"] = $skrR;
     if(!$_POST["tanggal_akhir"]) $_POST["tanggal_akhir"] = $skrR;
    
     
     if ($sql_where[0]) 
	   $sql_where = implode(" and ",$sql_where);
 
     $sql = "select a.*, b.cust_usr_kode, b.cust_usr_tanggal_lahir, c.jenis_nama, d.id_pembayaran,e.poli_nama, e.poli_tipe, d.reg_utama, a.id_resep
             ...
             where (penjualan_id in(select id_penjualan from apotik.apotik_penjualan_detail) or id_resep is not null) and a.is_terima!='y' and cust_usr_id <> '100' and penjualan_flag<>'R' and d.reg_jenis_pasien <> '2' and a.id_gudang like ".QuoteValue(DPE_CHAR,'%'.$theDep.'%');
     $sql .= " and ((date(a.penjualan_create) >= '".date("Y-m-d", strtotime($_POST["tanggal_awal"]))."' and date(a.penjualan_create) <= '".date("Y-m-d", strtotime($_POST["tanggal_akhir"]))."') 
               or 
    ...
    // }
     ...
     for($i=0,$counter=0,$n=count($dataTable);$i<$n;$i++,$counter=0){
      ...
      if($dataTable[$i]["penjualan_terbayar"]=='n' && ($TipeRawat['reg_tipe_rawat'] == 'G' || $TipeRawat['reg_tipe_rawat'] == 'J')){
        ...
        if($catatanDokter["rawat_terapi"]){
        $tbContent[$i][$counter][TABLE_ISI] = '<img hspace="2" width="30" height="30" src="'.$ROOT.'gambar/cetak.png" style="cursor:pointer" alt="Cetak Kwitansi" title="Cetak Kwitansi" border="0" onClick="ProsesCetakCatatanDokter(\''.$dataTable[$i]["id_resep"].'\');"/>';
        }else{
         $tbContent[$i][$counter][TABLE_ISI] = ''; 
        }
        $tbContent[$i][$counter][TABLE_ALIGN] = "center";
        $counter++;
          ...
          $styleRow[$i] = ($dataTable[$i]["penjualan_grandtotal"] == 0 && $catatanDokter["rawat_terapi"] != '') ? "color: $color" : "" ;
...
function ProsesCetakCatatanDokter(id) {
 
  BukaWindow('catatan_cetak.php?id='+id+'','Nota');
  //document.location.href='<?php echo $thisPage;?>';
}
                ...
                <div class="x_panel">
				  ...
				  <form  name="frmFind" action="<?php echo $_SERVER["PHP_SELF"]?>" method="POST" >
                        ...
                        <div class='input-group date' id='datepicker'>
							<input  id="tanggal_awal" name="tanggal_awal" type='text' class="form-control" value="<?=date("d-m-Y", strtotime($_POST["tanggal_awal"])) ?>" />
						...
						<div class='input-group date' id='datepicker2'>
							<input  id="tanggal_akhir" name="tanggal_akhir"  type='text' class="form-control" value="<?=date("d-m-Y", strtotime($_POST["tanggal_akhir"])) ?>" />
							<span class="input-group-addon">
								<span class="fa fa-calendar">
								</span>
							</span>
