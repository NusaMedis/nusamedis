...
if ($_GET["id"]) {
  ...
  d.cust_usr_tanggal_lahir, d.cust_usr_jenis_kelamin, d.cust_usr_nama, f.poli_nama, c.usr_id, f.form_asmed, g.jenis_nama, a.rawat_anak, a.rawat_neo
  ...
  $sql = "SELECT * from apotik.apotik_penjualan where penjualan_id = '$_GET[penj]'";
...
$asmedAnak = ($dataTable['rawat_anak']) ? unserialize($dataTable['rawat_anak']) : [];

$asmedAnak = ($dataTable['rawat_neo']) ? unserialize($dataTable['rawat_neo']) : $asmedAnak;

$terapiCPPT = (!$CPPT) ? [] : $terapiCPPT ;

$terapi = ($terapi) ? $terapi : $terapiCPPT;

$dataTable['rawat_terapi'] = ($dataTable['rawat_terapi'] && !$CPPT) ? $dataTable['rawat_terapi'] : $dataCPPT['terapiApotik'] ;

if(!$CPPT){
    $dataObat = ($dataPenj['tipe_resep'] == 'I') ? $asmedAnak['terapiInf'] : explode("+", $dataTable['rawat_terapi']);
}
else{
    $dataObat = ($dataPenj['tipe_resep'] == 'I') ? $dataCPPT['terapiInfus'] : explode("+", $dataTable['rawat_terapi']);
}
...
<body style="width: 10cm;">
    ...
    <?php for ($i = 0; $i < count($terapi); $i++) { ?>
        <tr>
                <td width="80%">R/ <?=$terapi[$i]['item_nama']?> </td>
                <td width="20%"><?php echo "No ".$terapi[$i]['penjualan_detail_jumlah'] ?></td>
            </tr>
        <tr>
                <td width="80%">&nbsp;</td>
                <td width="20%"></td>
    ...
    <?php for ($i = 0; $i < count($dataRacikan); $i++) { ?>
         <tr>
                <td width="80%"><?php echo "R/ "?> </td>
                <td width="20%"><?php echo "No ".$dataRacikan[$i]['rawat_terapi_racikan_jumlah'] ?></td>
        </tr>
        <?php
