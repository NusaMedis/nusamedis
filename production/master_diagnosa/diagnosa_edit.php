<?php
   ...
   /*  if(!$auth->IsAllowed("man_medis_diagnosa",PRIV_READ)){
	...
	function CheckDataIcd($icdNomor,$icdId=null)
	{
          ...
          $sql = "SELECT diagnosa_id FROM klinik.klinik_diagnosa
                    WHERE upper(diagnosa_nomor) = ".QuoteValue(DPE_CHAR,strtoupper($icdNomor));
                    
          if($icdId) $sql .= " and diagnosa_id <> ".QuoteValue(DPE_CHAR,$icdId);
          
          $rs = $dtaccess->Execute($sql);
          $dataAdaIcd = $dtaccess->Fetch($rs);
          
		return $dataAdaIcd["diagnosa_id"];
     }
	
	if($_POST["x_mode"]) $_x_mode = & $_POST["x_mode"];
	else $_x_mode = "New";
   
	if($_POST["diagnosa_id"])  $icdId = & $_POST["diagnosa_id"];
  
     if($_GET["nama"]) $_POST["nama"] = $_GET["nama"];
     if($_GET["kode"]) $_POST["kode"] = $_GET["kode"];
     
     $backPage = "diagnosa_view.php?kode=".$_POST["kode"]."&nama=".$_POST["nama"];
     ...
     if ($_GET["id"]) {
          ...
          $sql = "select a.*, b.poli_id from klinik.klinik_diagnosa a 
          left join global.global_auth_poli b on a.id_poli = b.poli_id
          where diagnosa_id= ".QuoteValue(DPE_CHAR,$icdId);
	...
	elseif($_x_mode=="Edit") $privMode = PRIV_UPDATE;
     ...
     if ($_POST["btnSave"] || $_POST["btnUpdate"]) {          
          if($_POST["btnUpdate"]){
               $icdId = & $_POST["diagnosa_id"];
               $_x_mode = "Edit";
          }
         
          if ($err_code == 0) {
               $dbTable = "klinik.klinik_diagnosa";
               
               $dbField[0] = "diagnosa_id";   // PK
               $dbField[1] = "diagnosa_nomor";
               $dbField[2] = "diagnosa_nama";
               $dbField[3]= "diagnosa_short_desc";
               $dbField[4]= "diagnosa_nomor_tanpa_titik";
               $dbField[5]= "id_poli";

			
               if(!$icdId) $icdId = $dtaccess->GetTransID();   
               $dbValue[0] = QuoteValue(DPE_CHAR,$icdId);
               $dbValue[1] = QuoteValue(DPE_CHAR,$_POST["diagnosa_nomor"]);
               $dbValue[2] = QuoteValue(DPE_CHAR,$_POST["diagnosa_nama"]);
               $dbValue[3]=QuoteValue(DPE_CHAR,$_POST["diagnosa_short_desc"]);
               $dbValue[4]=QuoteValue(DPE_CHAR,$_POST["diagnosa_nomor_tanpa_titik"]);
        ...
        if ($_GET["del"]) {
          $icdId = $enc->Decode($_GET["id"]);
           ...
           $sql = "delete from klinik.klinik_diagnosa where diagnosa_id = ".QuoteValue(DPE_CHAR,$icdId);
					...
					<form id="demo-form2" method="POST" class="form-horizontal form-label-left" action="<?php echo $_SERVER["PHP_SELF"]?>">
                      <div class="form-group">
                        ...
                        <div class="col-md-6 col-sm-6 col-xs-12">
                          <input type="text" id="diagnosa_nomor" name="diagnosa_nomor" value="<?php echo $_POST["diagnosa_nomor"];?>" required="required" class="form-control col-md-7 col-xs-12">
						          </div>
                      </div>
                      ...
                      <div class="form-group">
                        ...
                        <div class="col-md-6 col-sm-6 col-xs-12">
                          <input type="text" id="diagnosa_nomor_tanpa_titik" name="diagnosa_nomor_tanpa_titik" value="<?php echo $_POST["diagnosa_nomor_tanpa_titik"];?>" required="required" class="form-control col-md-7 col-xs-12">
                      </div>
                      </div>
                      <div class="form-group">
                      ...
                      <div class="form-group">
                        ...
                        <div class="col-md-6 col-sm-6 col-xs-12">
                          <input type="text" id="diagnosa_nama" name="diagnosa_nama" value="<?php echo $_POST["diagnosa_nama"];?>" required="required" class="form-control col-md-7 col-xs-12">
						            </div>
                      </div>
                        <div class="form-group">
                          ...
                          <div class="col-md-6 col-sm-6 col-xs-12">
                            <input type="text" id="diagnosa_short_desc" name="diagnosa_short_desc" value="<?php echo $_POST["diagnosa_short_desc"];?>" required="required" class="form-control col-md-7 col-xs-12">
                          </div>
                      </div>
                      ...
                      <?php echo $view->RenderHidden("diagnosa_id","diagnosa_id",$icdId);?>
