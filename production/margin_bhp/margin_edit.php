...
if ($_POST["btnSave"] || $_POST["btnUpdate"]) {
  ...
  $dbTable = "apotik.apotik_margin_bhp";
  ...
  $sql = "update apotik.apotik_margin_bhp set is_aktif ='N'
...
if ($_GET["del"]) {
  $marginId = $enc->Decode($_GET["id"]);
  //data margin edit
  $sql = "select * from apotik.apotik_margin_bhp where margin_id = " . QuoteValue(DPE_CHAR, $marginId);
  $rs = $dtaccess->Execute($sql);
  $dataMargin = $dtaccess->Fetch($rs);

  //cari margin yang mau diupdate
  $sql = "select * from apotik.apotik_margin_bhp where id_grup_item = " . QuoteValue(DPE_CHAR, $dataMargin["id_grup_item"]) . " and margin_id <> " . QuoteValue(DPE_CHAR, $marginId) . " order by margin_when_create desc";
  $rs = $dtaccess->Execute($sql);
  $dataMarginUpdate = $dtaccess->Fetch($rs);
  // aktifkan margin terakhir dengan jenis pasien yang sama
  $sql = "update apotik.apotik_margin_bhp set is_aktif ='y' where margin_id = " . QuoteValue(DPE_CHAR, $dataMarginUpdate["margin_id"]);
  $rs = $dtaccess->Execute($sql);

  $sql = "delete from apotik.apotik_margin_bhp where margin_id = " . QuoteValue(DPE_CHAR, $marginId);
