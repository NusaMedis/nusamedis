<?php
     ...
     $editPage = "cara_keluar_edit.php?konf=".$_GET["konf"];
     $thisPage = "cara_keluar_view.php?konf=".$_GET["konf"];
	...
	$isAllowedDel=1;
     ...
     function StripArr($num){
	   ...
	   $sql = "select * from klinik.klinik_cara_keluar_inap order by cara_keluar_inap_nama ";
     $rs = $dtaccess->Execute($sql,DB_SCHEMA);
     $dataCaraKeluar = $dtaccess->FetchAll($rs);
    // echo $sql;

	$table = new InoTable("table1","100%","left",null,1,2,1,null);     
  $tableHeader = "Master Cara Keluar Rawat Inap";
    ...
    //TOTAL HEADER TABLE
	 ...
	 for($i=0,$counter=0,$n=count($dataCaraKeluar);$i<$n;$i++,$counter=0){
          ...
          $tbContent[$i][$counter][TABLE_ISI] = $dataCaraKeluar[$i]["cara_keluar_inap_nama"];
          ...
          $counter++;
           ...
           if($isAllowedUpdate) {
               ...
               $tbContent[$i][$counter][TABLE_ISI] = '<a href="'.$editPage.'&id='.$enc->Encode($dataCaraKeluar[$i]["cara_keluar_inap_id"]).'"><img hspace="2" width="32" height="32" src="'.$ROOT.'gambar/icon/edit.png" alt="Edit" title="Edit" border="0"></a>';               
               $tbContent[$i][$counter][TABLE_ALIGN] = "center";
               $counter++;
               
          
               $tbContent[$i][$counter][TABLE_ISI] = '<a href="'.$editPage.'&id='.$enc->Encode($dataCaraKeluar[$i]["cara_keluar_inap_id"]).'&del=1"><img hspace="2" width="32" height="32" src="'.$ROOT.'gambar/icon/hapus.png" alt="Hapus" title="Hapus" border="0" onclick="javascript: return Hapus();"></a>';               
               $tbContent[$i][$counter][TABLE_ALIGN] = "center";
               $counter++;
                }
                ...
                <div class="x_panel">
                  ...
                  <div class="x_content">
					<table id="datatable-responsive" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
                      ...
                      <tbody>
                          <? for($i=0,$n=count($dataCaraKeluar);$i<$n;$i++) {   ?>
