...
// ini_set('display_errors', 1);
  ...
  /*  if(!$auth->IsAllowed("man_user_user_login",PRIV_READ)){
          ...
          // INSERT ATO UPDATE DATA KE TABEL
              ...
              // die();
               ...
               $dbKey[0] = 0; // -- set key buat clause wherenya , valuenya = index array buat field / value
                ...
                // --- buat nyimpen applicationnya ---                  
               $sql = "delete from global.global_auth_menu_role 
                         where id_usr = ".QuoteValue(DPE_CHAR,$usrId);
               $dtaccess->Execute($sql,DB_SCHEMA_GLOBAL);                    
               ...
               foreach($_POST["id_app"] as $key=>$value){
                    $usrAppId = $dtaccess->GetNewID("global.global_auth_menu_role","auth_menu_role_id",DB_SCHEMA_GLOBAL);
                    $dbValue[0] = QuoteValue(DPE_NUMERIC,$usrAppId);
                    $dbValue[1] = QuoteValue(DPE_CHAR,$usrId);
                    $dbValue[2] = QuoteValue(DPE_NUMERIC,$value);
                    $dbValue[3] = QuoteValue(DPE_CHAR,$depId);
               ...
               $dbKey[0] = 0; // -- set key buat clause wherenya , valuenya = index array buat field / value
                ...
                // --- buat nyimpen applicationnya ---                  
               $sql = "delete from global.global_user_validator 
                         where id_usr = ".QuoteValue(DPE_CHAR,$usrId);
               $dtaccess->Execute($sql,DB_SCHEMA_GLOBAL);                    
               
               $dbTable = "global.global_user_validator";
               
               $dbField[0] = "validator_auth_id";   // PK
               $dbField[1] = "id_validator";
               $dbField[2] = "id_usr";
               $dbField[3] = "id_dep";
            
               foreach($_POST["id_validator"] as $key=>$value){
                    $usrValidatorId = $dtaccess->GetTransID();
                    $dbValue[0] = QuoteValue(DPE_CHAR,$usrValidatorId);
                    $dbValue[1] = QuoteValue(DPE_NUMERIC,$value);
    ...
    $dataJabatan = $dtaccess->FetchAll($rs);     
     ...
     // -- CARI DATA MENU -- //
     $sql = "SELECT * FROM GLOBAL.GLOBAL_AUTH_MENU_ROLE A LEFT JOIN GLOBAL.GLOBAL_AUTH_MENU B ON B.MENU_ID = A.ID_MENU LEFT JOIN GLOBAL.GLOBAL_AUTH_ROLE C ON C.ROL_ID = A.ID_ROL WHERE A.ID_ROL = '1' AND B.MENU_LEVEL = '1' ORDER BY MENU_URUT ASC";
     $rs = $dtaccess->Execute($sql,DB_SCHEMA);
     $datamenu = $dtaccess->FetchAll($rs); 
				...
				<div class="col-md-2 col-sm-2 col-xs-12">
                  ...
                  <div class="x_content">
                      <div class="form-group">
                        ...
                        <td>
                              <table class="tabel">
                              <?php for($i=0,$n=count($datamenu);$i<$n;$i++){?>
                             
                              <tr>
                                   <td width="50%">
                                        <input onKeyDown="return tabOnEnter(this, event);" type="checkbox" name="id_menu[<?php echo $datamenu[$i]["menu_id"];?>]" id="id_menu[<?php echo $datamenu[$i]["menu_id"];?>]" value="<?php echo $datamenu[$i]["menu_id"];?>" <?php if ($_POST["id_menu"][$i]==$datamenu[$i]["menu_id"]) echo "checked"; ?>/>
                                        <label for="id_app[<?php echo $datamenu[$i]["menu_id"];?>]"><?php echo $datamenu[$i]["menu_nama"];?></label>                                       
                                   </td>
                              </tr>
                              <?php }?>
                  ...
                  <div class="x_content">
                      <div class="form-group">
                        <label class="control-label col-md-3 col-sm-3 col-xs-12" for="first-name"><span class="required"></span>
                         ...
                         <td  valign="top">
                              <table border= "0" width=80%">
                              <?php for($i=0,$n=count($dataValidator);$i<$n;$i++){

                              ?>
                              <tr>
                                   <td width="50%">
                                        <input onKeyDown="return tabOnEnter(this, event);" type="checkbox" name="id_validator[<?php echo $dataValidator[$i]["validator_id"];?>]" id="id_poli[<?php echo $dataValidator[$i]["validator_id"];?>]" value="<?php echo $dataValidator[$i]["validator_id"];?>" <?php if ($_POST["id_validator"][$i]==$dataValidator[$i]["validator_id"]) echo "checked"; ?> onClick="TampilCombo(this,document.getElementById('usr_app_def_<?php echo $i?>'))"/>
                                        <label for="id_validator[<?php echo $dataValidator[$i]["validator_id"];?>]"><?php echo $dataValidator[$i]["validator_nama"];?></label>                                       
                                   </td>
