<?php
	...
	if(!$auth->IsAllowed("man_ganti_password",PRIV_CREATE)){
     ...
     if ($_POST["btnNew"]){
     $editPage = "item_psikotropika_view.php?new=1"; 
     header('location:'.$editPage);
     }
     else $editPage = "item_psikotropika_view.php";
     if ($_POST["btnNew"]){
     $thisPage = "item_psikotropika_view.php?new=1"; }
     else $thisPage = "item_psikotropika_view.php";
 
 if($_POST["btnBack"]) {
     $backPage="item_psikotropika_view.php";
    ...
    /* if($_GET["id_jenis"]){
     ...
     $edit="item_psikotropika_view.php?edit=1";
     ...
     if($sql_where) $sql .= " where a.item_flag ='M' and a.item_aktif='y' and item_psikotropika='y' and ".$sql_where;
...
//     echo $sql;
     ...
     $numRows = $dtaccess->Fetch($rsNum); */
      //echo $sql;
     //*-- config table ---*//
     $tableHeader = "&nbsp;Master Psikotropika";
...
/*  function reklinik(kliniks) {
  ...
  function rejenis(jenis) {
   document.location.href='item_psikotropika_view.php?klinik='+jenis+'&currentPage=<?php echo $_GET["currentPage"];?>&recPerPage=<?php echo $_GET["recPerPage"];?>';
  }
  
function CheckData() {
	if(confirm('Anda Yakin Ingin Memberikan Flag Generik Pada Item Ini?')){
		document.frmView.btnSave.value = 'Simpan';
		document.location.href='item_psikotropika_view.php?new=1';
	}
}
...
function editData() {
	if(confirm('Anda Yakin Ingin Mengubah Flag Generik Pada Item Ini?')){
		document.frmView.btnSave.value = 'Simpan';
		document.location.href='item_psikotropika_view.php';
	}
}
