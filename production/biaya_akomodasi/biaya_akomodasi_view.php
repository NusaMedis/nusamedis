<?php
     ...
     $addPage  = "biaya_akomodasi_edit.php";
     $editPage = "biaya_akomodasi_edit.php";
     $detailPage = "biaya_akomodasi_detail_view.php";
     $thisPage = "biaya_akomodasi_view.php";     
     
      $sql_where[] = "1=1";
      /*
      $sql= "select a.*, e.biaya_id,e.biaya_nama FROM global.global_biaya_akomodasi a 
            left join klinik.klinik_biaya e on e.biaya_id=a.id_biaya";      
      if ($sql_where) $sql = $sql." where ".implode(" and ",$sql_where);
      $sql = $sql." order by a.id_kelas ASC";  */
      
      $sql= "select * from klinik.klinik_biaya where biaya_jenis_sem = 'BA'"; 
     ...
     for($i=0,$counter=0,$n=count($dataTable);$i<$n;$i++,$counter=0){
         ...
         $counter++;
          ...
          $counter++; 
               ...
               $tbContent[$i][$counter][TABLE_ISI] = '<a href="'.$editPage.'?del=1&id='.$enc->Encode($dataTable[$i]["biaya_akomodasi_id"]).'&id_kelas='.$dataTable[$i]["id_kelas"].'&id_kamar='.$dataTable[$i]["id_kamar"].'"><img hspace="2" width="32" height="32" src="'.$ROOT.'gambar/icon/hapus.png" alt="Hapus" title="Hapus" border="0" onclick="javascript: return hapus();"></a>';               
               $tbContent[$i][$counter][TABLE_ALIGN] = "center";
               $counter++;     */
          /* 
          if($isAllowedUpdate) {
