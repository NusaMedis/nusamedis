   ...
   // die();
    ...
    if($_POST['reg_status_pasien'] == "B" || $_POST['reg_status_pasien'] == "L" ) { 
        ...
        $kel = ($_POST['kel']) ? $_POST['kel'] : '';
         ...
         $dbValue[23] = QuoteValue(DPE_CHAR,str_replace("'",'*',$_POST["cust_usr_penanggung_jawab"]));
  ...
  <link rel="stylesheet" type="text/css" href="<?php echo $ROOT; ?>assets/vendors/sweetalert/sweetalert.css">
    ...
    $(document).ready(function(){
        ...
        setTimeout(function() {
          ...
          if (data.id_prop != null) {pilih_kota(data.id_prop);}
          else {pilih_kota(<?=$konf['dep_kode_prop']?>);}

          if (data.id_kota) {pilih_kecamatan(data.id_kota+'&prop='+data.id_prop);}
          if (data.id_kecamatan) {pilih_kelurahan(data.id_kecamatan+'&kec='+data.id_kota+'&prop='+data.id_prop);}
                      },"json");            
        }, 300);
         //jika ada poli ke 2           
        <?php if (count($row) > 0 ) { ?>
                     ...
                     <div class="item form-group">
                      ...
                      <div class="field form-group">
                        ...
                        <div class="col-md-8 col-sm-8 col-xs-12">
                          <input id="cust_usr_nama" name="cust_usr_nama" value="<?php echo str_replace("*", "'", $row["cust_usr_nama"]);?>" class="form-control col-md-7 col-xs-12" data-validate-length-range="2" data-validate-words="2"  placeholder="dua kata contoh:Moch Mansyur" required="required" type="text"> 
                          ...
                          <select class="form-control" name="id_prop" id="id_prop" onchange="pilih_kota(this.value)">
                            ...
                            for($i=0,$n=count($dataProvinsi);$i<$n;$i++) { ?>  
                              <option value="<?php echo $dataProvinsi[$i]['lokasi_propinsi'];?>" 
                                <?php if($dataProvinsi[$i]["lokasi_propinsi"]==$row["id_prop"]) { echo "selected"; } elseif($dataProvinsi[$i]["lokasi_propinsi"]==$konf['dep_kode_prop']) echo "selected";?>><?php echo $dataProvinsi[$i]['lokasi_nama'];?>
                              </option>';
                            <? } ?>                                                                   
                  ...
                  <div class="x_content">
                   ...
                   <option value="<?php echo $dataStatus[$i]["status_perkawinan_id"];?>" <?php if($dataStatus[$i]["status_perkawinan_id"]==$row["id_status_perkawinan"]) echo "selected"; ?>><?php echo ($i+1).". ".$dataStatus[$i]["status_perkawinan_nama"];?></option>
                      ...
                      <div class="item form-group">
                        ...
                        <div class="col-md-8 col-sm-8 col-xs-12">
                          <input type="text" class="form-control" name="cust_usr_penanggung_jawab" id="cust_usr_penanggung_jawab" size="30" maxlength="65" value="<?php echo str_replace('*',"'",$row["cust_usr_penanggung_jawab"]);?>" onKeyDown="return tabOnEnter_select_with_button(this, event);"/></font>
                     &nbsp;Status :
  ...
  <script>
    function pilih_kota(isi) {
      $.getJSON('select_kota.php?q='+isi, function(nilai) {
        ...
        $.each(nilai, function(index, val) {
          if (val.lokasi_kabupatenkota == isikota && val.lokasi_propinsi == isiprop) {var pilihankota = 'selected'}
          $("#kota").append(
    ...
    function pilih_kecamatan(isi) {
      $.getJSON('select_kota.php?kec='+isi, function(nilai) {
        ...
        $.each(nilai, function(index, val) {
          if (val.lokasi_kecamatan == isikec && val.lokasi_kabupatenkota == isikota && val.lokasi_propinsi == isiprop) {var pilihankec = 'selected'}
          $("#kec").append(
    ...
    function pilih_kelurahan(isi) {
      $.getJSON('select_kota.php?kel='+isi, function(nilai) {
        ...
        $.each(nilai, function(index, val) {
          if (val.lokasi_kelurahan == isikel) {var pilihankel = 'selected'}
          $("#kel").append(
