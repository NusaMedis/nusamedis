<?php    
    ...
    {   
     ...
     $sql = "select count(reg_id) as jkn_irj
            from klinik.klinik_registrasi a ";
     $sql .= " WHERE id_cust_usr = ".QuoteValue(DPE_CHAR,$_POST['cust_usr_id'])." and reg_jenis_pasien = '5' and reg_tanggal='".date('Y-m-d')."' and reg_tipe_rawat = 'J'";  
     $jkn_irj = $dtaccess->Fetch($sql);   
    # cek pasien masih irna
     $sql = "select count(reg_id) as jkn_irna
            from klinik.klinik_registrasi a ";
     $sql .= " WHERE id_cust_usr = ".QuoteValue(DPE_CHAR,$_POST['cust_usr_id'])." and reg_jenis_pasien = '5' and reg_status != 'I5' and reg_tipe_rawat = 'I'";  
    ...
    # pasien irj di poli yang sama
     ...
     $sql .= " WHERE cust_usr_nama = ".QuoteValue(DPE_CHAR,strtoupper($_POST["cust_usr_nama"]))." and cust_usr_tanggal_lahir = ".QuoteValue(DPE_DATE,date_db($_POST["cust_usr_tanggal_lahir"]));  
     $pasien_exist = $dtaccess->Fetch($sql);      


    if ( $sameday_irj['sameday'] >= 1  ){  echo "<script>alert('Pasien sudah terdaftar di klinik'); window.history.back();</script>"; } 
    elseif ( $jkn_irj['jkn_irj'] >= 1   ){  echo "<script>alert('Pasien masih di rawat jalan'); window.history.back();</script>"; } 
    elseif ( $jkn_irna['jkn_irna'] >= 1  ){  echo "<script>alert('Pasien rawat inap tidak bisa diregistrasi ke rawat jalan'); window.history.back();</script>"; } 
    elseif ( $pasien_exist['exist'] >= 1 && $_POST['reg_status_pasien'] == "B" ){  echo "<script>alert('Pasien memiliki nama dan tanggal lahir yang sama dengan pasien lain'); window.history.back();</script>"; }  
   ...
   // die();
    ...
    if($_POST['reg_status_pasien'] == "B" || $_POST['reg_status_pasien'] == "L" ) { 
        ...
        $lokasidaerah = $dtaccess->Fetch($sql);
         ...
         if ($norm_depan == 'n') {
          $dbField[28] = "cust_usr_kode";
          $dbField[29] = "cust_usr_kode_tampilan";
          if($_POST["id_card"] == "KTP") $dbField[30] = "cust_usr_nik";
         }else{ if($_POST["id_card"] == "KTP") { $dbField[28] = "cust_usr_nik";} }
        ...
        //$dbField[24] = "cust_usr_kode_tampilan";
         ...
         if ($norm_depan == 'n') {
          $dbValue[28] = QuoteValue(DPE_CHAR,$_POST["cust_usr_kode"]);
          $dbValue[29] = QuoteValue(DPE_CHAR,$_POST["cust_usr_kode_tampilan"]);
          $dbValue[30] = QuoteValue(DPE_CHAR,$_POST["cust_usr_no_identitas"]);
        }else{ if($_POST["id_card"] == "KTP") { $dbValue[28] = QuoteValue(DPE_CHAR,$_POST["cust_usr_no_identitas"]); } }
...
<html lang="en">
    ...
    function create_sep(){
        ...
        $.post( "create_sep.php",$("form#form_utama").serialize(),
        function( data ) {
...
<script>
<?php if($cetaksep=='yes'){?>
 //url = 'cetak_sep.php?id_reg=<? echo $reg_id;?>';
                ...
                <div class="x_panel">
                  <div class="x_content">
                    ...
                    <div class="col-md-6 col-sm-6 col-xs-12">
                        ...
                        <input type="text" name="reg_tgl_rujukan" id="reg_tgl_rujukan" class="form-control">
                    </div>
