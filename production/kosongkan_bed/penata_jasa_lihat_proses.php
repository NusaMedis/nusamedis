<?php        
     ...
     $_POST["dep_kasir_tindakan"] = $konfigurasi["dep_kasir_tindakan"];
 	  	 /* if(!$auth->IsAllowed("sirs_rawat_inap_kosongkan_bed",PRIV_CREATE)){
          die("access_denied");
          exit(1);
     } else if($auth->IsAllowed("sirs_rawat_inap_kosongkan_bed",PRIV_CREATE)===1){
          echo"<script>window.parent.document.location.href='".$ROOT."login.php?msg=Login First'</script>";
          exit(1);
     } */

     $_x_mode = "New";
     $thisPage = "penata_jasa_edit_view.php";
   ...
   /*   if ($_GET["id"]) {
          ...
          $sql = "select a.* from klinik.klinik_kamar a 
				  where a.kamar_id = ".QuoteValue(DPE_CHAR,$kamarId);
    ...
    $sql = "update klinik.klinik_kamar_bed set bed_reserved='y'
                         where bed_id = ".QuoteValue(DPE_CHAR,$_POST["id_bed"]);//[$i]);
...
//    echo $sql;
    ...
    $_POST["reg_umur_bulan"] = $dataPasien["reg_umur_bulan"];
     ...
     $sql = "select a.*,b.usr_name as dokter_nama from  klinik.klinik_folio a
            left join global.global_auth_user b on a.id_dokter  = b.usr_id  
			       where a.fol_lunas='n' and a.id_pembayaran = ".QuoteValue(DPE_CHAR,$_GET["pembayaran_id"])." 
 ...
 if(!$dataPerawat){
    ...
    $kembali = "penata_jasa_edit_proses.php?id_dokter=".$_POST["id_dokter"].
    "&reg_jenis_pasien=".$_POST["reg_jenis_pasien"].
//    "&id_poli=".$_POST["id_poli"].
    "&id_reg=".$_POST["id_reg"]."&pembayaran_id=".$dataBayar["pembayaran_id"].
...
//    id_poli = ".QuoteValue(DPE_CHAR,$_POST["id_poli"]).",
   ...
   if ($_GET["del"] || $_GET["id_register"] || $_GET["id_biaya"] || $_GET["id_pembayaran"]) { 
          ...
          // kembali ke atas      
          $kembali = "penata_jasa_edit_proses.php?id_reg=".$_GET["id_register"]."&reg_jenis_pasien=".$dataRegJenis["reg_jenis_pasien"]."&pembayaran_id=".$_GET["pembayaran_id"];
          header("location:".$kembali);
          exit();    
     } 
...
?>              
<?php //echo $view->RenderBody("module.css",true,true,"PENATA JASA"); ?>
<br />                                                                                                             
...
//    if(confirm('Cetak Invoice?')) 
       BukaWindow('penata_jasa_dot_cetak_sementara.php?dep_bayar_reg=<?php echo $_POST["dep_bayar_reg"];?>&id_reg=<?php echo $_POST["id_reg"];?>&ket=<?php echo $_POST["fol_keterangan"];?>&dis=<?php echo $_POST["diskon"];?>&disper=<?php echo $_POST["diskonpersen"];?>&pembul=<?php echo $_POST["pembulatan"];?>&total=<?php echo $_POST["total"];?>','Kwitansi');
...
<tr>
     ...
     <legend><strong>Data Pasien</strong></legend>
      ...
      <table width="100%" border="1" cellpadding="4" cellspacing="1">
          <tr>
               ...
               <td width= "40%" align="center" class="tablecontent" rowspan="8"><font color='red' size='10'><span id=txtIsi ><?php echo currency_format($grandTotalHarga);?></span></font></td>
          </tr>	
          <tr>
          ...
          <tr>
                ...
                <td width= "15%" align="left" class="tablecontent">Kelas</td>
                <td width= "40%" align="left" colspan="2" class="tablecontent-odd">
                 <select name="reg_kelas" disabled id="reg_kelas" onKeyDown="return tabOnEnter(this, event);">
                   <option value="--">[ Pilih Kelas ]</option>			
				              <?php for($i=0,$n=count($dataKelas);$i<$n;$i++){ ?>
         	         <option value="<?php echo $dataKelas[$i]["kelas_id"];?>" <?php if($_POST["reg_kelas"]==$dataKelas[$i]["kelas_id"]) echo "selected"; ?>><?php echo $dataKelas[$i]["kelas_nama"];?></option>
				            <?php } ?>
			            </select>
                  <!-- Karena di Disabled -->
                  <input type="hidden" id="reg_kelas"  name="reg_kelas" value="<?php echo $_POST["reg_kelas"];?>" onKeyDown="return tabOnEnter_select_with_button(this, event);"/>
     ...
     <table width="100%" border="1" cellpadding="4" cellspacing="1"> 
              ...
              <td width="10%" align='center'>Total Tagihan</td>
                ...
                <?php if(($_POST["reg_jenis_pasien"]=='1') or ($_POST["reg_jenis_pasien"]=='5') or ($_POST["reg_jenis_pasien"]=='7') or ($_POST["reg_jenis_pasien"]=='20') or ($_POST["reg_jenis_pasien"]=='16')) { ?>
                <td width="10%" align='center'>Dijamin</td>
                <td width="10%" align='center'>Subsidi</td>
                <td width="10%" align='center'>Iur Biaya</td>
				<? } else if($_POST["reg_jenis_pasien"]=='18') { ?>
              ...
              <td width="25%">
                    <?php if($dataTable[$i]["fol_jenis"]=="O" || $dataTable[$i]["fol_jenis"]=="OA" || $dataTable[$i]["fol_jenis"]=="OG" || 
                             ...
                             $dataTable[$i]["fol_jenis"]=="RA" || $dataTable[$i]["fol_jenis"]=="RG" || $dataTable[$i]["fol_jenis"]=="RI" || $dataTable[$i]["fol_jenis"]=="I"){
              ...
              <td width="10%" align='right'><?php echo currency_format($dataTable[$i]["fol_nominal"])?></td>
                ...
                <td width="10%" align='right'><?php echo $dataTable[$i]["dokter_nama"]?></td>
						  ...
						  <?php } ?>
              <?php if(($_POST["reg_jenis_pasien"]=='1') or ($_POST["reg_jenis_pasien"]=='21') or ($_POST["reg_jenis_pasien"]=='16') or ($_POST["reg_jenis_pasien"]=='7') or ($_POST["reg_jenis_pasien"]=='5')) { ?>                
						  <tr>                                     
                <td align="right" width="20%" class="tablesmallheader" colspan='14'>
                   <!--<input type="submit" name="btnPrint" id="btnPrint" value="Print Kwitansi Sementara" class="submit"/>-->     
                   <input type="button" name="kembali" id="kembali" value="Kembali" class="submit" onClick="document.location.href='penata_jasa_edit_view.php'";/>     
                </td>
              </tr>
              <?} else { ?>              
						  <tr>                                     
                <td align="right" width="20%" class="tablesmallheader" colspan='10'>
        <!--           <input type="submit" name="btnPrint" id="btnPrint" value="Print Kwitansi Sementara" class="submit"/>  -->   
                   <input type="button" name="kembali" id="kembali" value="Kembali" class="submit" onClick="document.location.href='penata_jasa_edit_view.php'";/>     
                </td>
              </tr>
