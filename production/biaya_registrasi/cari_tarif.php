...
function GetData($in_nama){
  ...
  $tbHeader[0][$counter][TABLE_ISI] = "Biaya";
	...
	for($i=0,$counter=0,$n=count($dataTable);$i<$n;$i++,$counter=0) {
		...
		$tbContent[$i][$counter][TABLE_ISI] = '<img src="'.$ROOT.'gambar/icon/cari.png" style="cursor:pointer;" border="0" alt="Pilih" title="Pilih" width="22" height="22" class="img-button" OnClick="javascript: sendValue(\''.$dataTable[$i]["biaya_id"].'\',\''.$dataTable[$i]["biaya_tarif_id"].'\',\''.addslashes(htmlentities($dataTable[$i]["biaya_nama"])).'\',\''.addslashes(htmlentities($dataTable[$i]["biaya_total"])).'\')"/>';
...
function sendValue(biaya_id,biaya_tarif_id,biaya_nama,biaya_total) {
//alert(id);
