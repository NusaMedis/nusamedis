<?php
/*
Barcode Render Class for logistik.php using the GD graphics library 
...
*/
  ...
  */
  ...
  class C128CObject extends BarcodeObject {
   var $mCharSet, $mChars,$mCharss;
   function C128CObject($Width, $Height, $Style, $Value)
   {
     $this->BarcodeObject($Width, $Height, $Style);
	 $this->mValue   = $Value;
	  $this->mCharss   = ' !"#$%&\'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\]^_`abcdefghijklmnopqrstuvwxyz{|}~';
  ...
  //        for ($i=0;$i<$len;$i++) {
	 //  if ((ord($this->mValue[$i])<10) || (ord($this->mValue[$i])>100)) {
