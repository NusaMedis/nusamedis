...
function InoUpload(& $in_files, $in_lokasi, $in_maxSize=null,& $in_name, $arr_mime=null) {
     ...
     if(!empty($in_files['error'])) {
		switch($in_files['error']) {
			...
			case '1':
				$error = 'The uploaded file exceeds the upload_max_filesize directive in logistik.php.ini';
				break;
			case '2':
		...
		$error = 'No file was uploaded..';
          ...
          if(!$error) {
               if($in_name) $destfile = $in_lokasi."/".$in_name;
                    ...
                    $in_name = rand().time().".".substr($in_files["name"],$pos-strlen($in_files["name"])+1);
