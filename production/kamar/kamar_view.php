...
//--mencari jumlah bed--//
$sql_bed = "select id_kamar,count(bed_id) as total_bed from klinik.klinik_kamar_bed where bed_keterangan = 'n' group by id_kamar";
...
$sql = "select a.kamar_id,a.kamar_nama,a.kamar_status,a.kamar_kode,b.kelas_nama,c.poli_nama,d.gedung_rawat_nama,
            a.id_poli, a.id_kelas, e.jenis_kelas_nama from klinik.klinik_kamar a
           left join klinik.klinik_kelas b on a.id_kelas = b.kelas_id
           left join global.global_auth_poli c on a.id_poli = c.poli_id
           left join global.global_gedung_rawat d on a.id_gedung_rawat = d.gedung_rawat_id
           left join klinik.klinik_jenis_kelas e on a.id_jenis_kelas = e.jenis_kelas_id where 1=1";
if ($_POST["id_kelas"]) $sql .= " and a.id_kelas='" . $_POST["id_kelas"] . "'";
if ($_POST["search"]) $sql .= " and d.gedung_rawat_nama LIKE'%" . $_POST["search"] . "%'";
...
//*-- config table ---*//
$tableHeader = "Manajemen | Master Kamar View";
...
$tbHeader[0][$counterHeader][TABLE_ISI] = "Gedung Rawat";
$tbHeader[0][$counterHeader][TABLE_WIDTH] = "15%";
$counterHeader++;
...
for ($i = 0, $counter = 0, $n = count($dataTable); $i < $n; $i++, $counter = 0) {
  ...
  $tbContent[$i][$counter][TABLE_NOWRAP] = true;
  $counter++;
  $jumKamar += $jml_bed[$dataTable[$i]["kamar_id"]];
  ...
  if ($isAllowedUpdate) {
    $tbContent[$i][$counter][TABLE_ISI] = '<a href="' . $editPage . '?id_kelas=' . $dataTable[$i]["id_kelas"] . '&id=' . $enc->Encode($dataTable[$i]["kamar_id"]) . '"><img hspace="2" width="25" height="25" src="' . $ROOT . 'gambar/icon/edit.png" alt="Edit" title="Edit" border="0"></a>';
    $tbContent[$i][$counter][TABLE_ALIGN] = "center";
    $counter++;
  }

  if ($isAllowedDel) {
    $tbContent[$i][$counter][TABLE_ISI] = '<a href="' . $editPage . '?id_kelas=' . $dataTable[$i]["id_kelas"] . '&id=' . $enc->Encode($dataTable[$i]["kamar_id"]) . '&del=1&id_poli=' . $dataTable[$i]["id_poli"] . '"><img hspace="2" width="25" height="25" src="' . $ROOT . 'gambar/icon/hapus.png" alt="Hapus" title="Hapus" border="0" onclick="javascript: return Hapus();"></a>';
    $tbContent[$i][$counter][TABLE_ALIGN] = "center";
    $counter++;
  }
}
...
$tbBottom[0][0][TABLE_ISI] = $jumKamar;
          ...
          <div class="row">
            ...
            <div class="col-md-12 col-sm-12 col-xs-12">
              <div class="x_panel">
                <div class="x_title">
                  <h2>Master Kamar</h2>
                  <a href="import_kamar.php" class="btn btn-danger pull-right">Import</a>
                  ...
                  <form action="" method="post">
                    <div class="col-md-4">
                      <div class="form-group">
                        <label for=""> Cari Berdasarkan Gedung Rawat</label>
                        <input type="text" class="form-control" name="search" value="<?= $_POST['search'] ?>">
                      </div>
                    </div>
                    <div class="col-md-1">
                      <div class="form-group">
                ...
                <div class="x_content">
                  <table class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
                    ...
                    <tbody>
                      <tr>
                        <td colspan="6" class="column-title">&nbsp;</td>
                        <td class="column-title"><?php echo $tbBottom[0][0][TABLE_ISI]; ?> </td>
                        <td colspan="3" class="column-title">&nbsp;</td>
