<?php
  ...
  /* SQL BIAYA KARTU */     
  $sql = "select a.*,b.*, c.*  from klinik.klinik_biaya_registrasi a left join klinik.klinik_biaya b on a.id_biaya = b.biaya_id left join klinik.klinik_biaya_tarif c on b.biaya_id = c.id_biaya where a.biaya_registrasi_tipe = 'B'"; 
  if ($Konfigurasi["dep_konf_header_klinik"]=='y') $sql .="and a.id_poli = ".QuoteValue(DPE_CHAR,$_POST["klinik"]);
  $sql .= "and c.id_jenis_pasien = ".QuoteValue(DPE_NUMERICKEY,$_POST["reg_jenis_pasien"]);
  ...
  if($daftar) {
    ...
    if($_POST["reg_jenis_pasien"]=="5" || $_POST["reg_jenis_pasien"]=="7" || $_POST["reg_jenis_pasien"]=="18" || $_POST["reg_jenis_pasien"]=='26') $dbValue[18] = QuoteValue(DPE_NUMERIC,StripCurrency($daftar["biaya_total"]));
