...
/*a.biaya_registrasi_tipe=".QuoteValue(DPE_CHAR,$regTipeAntrian)." and   
 ...
 $sql .= " and a.id_poli=" . QuoteValue(DPE_CHAR, $_GET["id_poli"]);
 // echo $sql;
...
if ($periksa) {
  ...
  $dbValue[5] = QuoteValue(DPE_CHAR, $custUsrId);
  ...
  $dbValue[9] = QuoteValue(DPE_CHAR, $_GET["id_poli"]);
  $dbValue[10] = QuoteValue(DPE_NUMERICKEY, $_GET["reg_jenis_pasien"]);
  ...
  if ($_GET["reg_jenis_pasien"] == "5" || $_GET["reg_jenis_pasien"] == "7" || $_GET["reg_jenis_pasien"] == "18" || $_GET["reg_jenis_pasien"] == '26') {
  ...
  if ($_GET["dokter"]) {
    $dbValue[2] = QuoteValue(DPE_CHAR, $_GET["dokter"]);
  } else {
  ...
  for ($i = 0; $i < count($dataBiayaSplit); $i++) {
    $sql = "select id_perawat from global.global_auth_user where usr_id = " . QuoteValue(DPE_CHAR, $_GET['dokter']);
    $UserPerawat = $dtaccess->Fetch($sql);

    if ($dataBiayaSplit[$i]['id_folio_posisi'] == '10') {
      $UsrId = $_GET['dokter'];
    } elseif ($dataBiayaSplit[$i]['id_folio_posisi'] == '2') {
      $UsrId = $UserPerawat['id_perawat'];
    }
