<?php
     ...
     $editPage = "edit_kuota.php?id_instalasi=".$_POST["id_instalasi"]."&id_sub_instalasi=".$_POST["id_sub_instalasi"]."&id_poli=".$_POST["id_poli"];

     $editLibur = "editLibur.php?id_instalasi=".$_POST["id_instalasi"]."&id_sub_instalasi=".$_POST["id_sub_instalasi"]."&id_poli=".$_POST["id_poli"];
...
//     $counterHeader++; 
     ...
     $tbHeader[0][$counterHeader][TABLE_ISI] = "Kuota";
     $tbHeader[0][$counterHeader][TABLE_WIDTH] = "5%";
     $counterHeader++; 

      $tbHeader[0][$counterHeader][TABLE_ISI] = "Pasien Terdaftar";
     $tbHeader[0][$counterHeader][TABLE_WIDTH] = "5%";
     $counterHeader++; 
     ...
     $counterHeader++; 
      ...
      $tbHeader[0][$counterHeader][TABLE_ISI] = "Pemberitahuan";
     $tbHeader[0][$counterHeader][TABLE_WIDTH] = "4%";
     $counterHeader++; 
     ...
     for($i=0,$counter=0,$n=count($dataTable);$i<$n;$i++,$counter=0){
            ...
            $tbContent[$i][$counter][TABLE_ISI] = "&nbsp;&nbsp;".$dataTable[$i]["jadwal_dokter_jam_mulai_android"];
	          $tbContent[$i][$counter][TABLE_ALIGN] = "right";
	          $counter++;
            
			      $tbContent[$i][$counter][TABLE_ISI] = "&nbsp;&nbsp;".$dataTable[$i]["jadwal_dokter_jam_selesai_android"];
	          $tbContent[$i][$counter][TABLE_ALIGN] = "left";
            $counter++;
            

            $tbContent[$i][$counter][TABLE_ISI] = "&nbsp;&nbsp;".$dataTable[$i]["jadwal_dokter_kuota"];
	          $tbContent[$i][$counter][TABLE_ALIGN] = "left";
            $counter++;

            $tanggal=date('Y-m-d');
            $query_jumlah="SELECT count(*) AS jumlah FROM klinik.klinik_registrasi_buffer WHERE id_jadwal= '".$dataTable[$i]["jadwal_dokter_id"]."' and reg_buffer_batal='n' and reg_buffer_flag='A' and reg_buffer_tanggal >='$tanggal'";
             $rs = $dtaccess->Execute($query_jumlah);
            //$rs = $dtaccess->Query($sql,$recordPerPage,$startPage);
            $dataTableKuota = $dtaccess->Fetch($rs);

            $tbContent[$i][$counter][TABLE_ISI] = "&nbsp;&nbsp;".$dataTableKuota["jumlah"];
            $tbContent[$i][$counter][TABLE_ALIGN] = "left";
            $counter++;


            $tbContent[$i][$counter][TABLE_ISI] = "&nbsp;&nbsp;".$dataTable[$i]["jadwal_dokter_status"];
	          $tbContent[$i][$counter][TABLE_ALIGN] = "left";
            $counter++;
...
//	          $counter++;   
    ...
    //        $counter++;
            ...
            $tbContent[$i][$counter][TABLE_ISI] = '<a href="'.$editPage.'&id='.$enc->Encode($dataTable[$i]["jadwal_dokter_id"]).'"><img hspace="2" width="25" height="25"  target="_blank" rel="nofollow" src="'.$ROOT.'gambar/icon/edit.png" alt="Edit" title="Edit" border="0"></a>';
            $tbContent[$i][$counter][TABLE_ALIGN] = "center";
            $counter++;


            $tbContent[$i][$counter][TABLE_ISI] = '<a href=""><img hspace="2" width="25" height="25"  target="_blank" rel="nofollow" src="'.$ROOT.'gambar/icon/aktif.png" alt="Notif" title="Notif" border="0"></a>';
            $tbContent[$i][$counter][TABLE_ALIGN] = "center";
            $counter++;
