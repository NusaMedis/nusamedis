...
$poliId = $auth->IdPoli();
     ...
     // KONFIGURASI
      ...
      $sql = "select a.*, e.usr_name, d.poli_nama,c.cust_usr_nama as nama,c.cust_usr_alamat,c.cust_usr_kode as rm, cust_usr_no_hp, f.jenis_nama, g.tipe_biaya_nama,c.cust_usr_id,x.*,y.*
      ...
      join global.global_skrining y  on y.buffer_id = a.reg_buffer_id
      where   reg_buffer_flag <>' ' and reg_buffer_flag <> ' ' and reg_buffer_flag <> 'W' and (reg_buffer_utama='' or reg_buffer_utama is null) and a.id_dep ='$depId' and
      ".$sql_where."
...
//sementara dihilangkan
      ...
      $tableHeader = "&nbsp;Daftar Registrasi Online";
      $counterHeader = 0;
      ...
      $tbHeader[0][$counterHeader][TABLE_ISI] = "No. Antrian";
      $tbHeader[0][$counterHeader][TABLE_WIDTH] = "1%";
      $counterHeader++;
      ...
      $tbHeader[0][$counterHeader][TABLE_ISI] = "Nama Akun";
      $tbHeader[0][$counterHeader][TABLE_WIDTH] = "5%";
      $counterHeader++;
      ...
      $tbHeader[0][$counterHeader][TABLE_ISI] = "Keterangan Periksa";
      $tbHeader[0][$counterHeader][TABLE_WIDTH] = "10%";
      $counterHeader++;


      $tbHeader[0][$counterHeader][TABLE_ISI] = "Persetujuan";
      $tbHeader[0][$counterHeader][TABLE_WIDTH] = "10%";
      $counterHeader++;
      ...
      for($i=0,$n=count($dataTable),$counter=0;$i<$n;$i++,$counter=0) {
        ...
        $sql="select count(fol_id) as jml_tind from klinik.klinik_folio where id_cust_usr=".QuoteValue(DPE_CHAR,$dataTable[$i]["id_cust_usr"])." and fol_lunas ='n'";
        $noLunas = $dtaccess->Fetch($sql);
        ...
        if($dataTable[$i]["is_daftar"]=='n' && $dataTable[$i]["reg_buffer_batal"]=='n' && $dataTable[$i]["reg_buffer_tanggal"]==date('Y-m-d') ){
          if ($noLunas["jml_tind"]>0) {
            // code...
            $tbContent[$i][$counter][TABLE_ISI] = "Pasien Belum Melunasi Tindakan Sebelumnya";  
          }
          else{
        ...
        $tbContent[$i][$counter][TABLE_ISI] = $dataTable[$i]["reg_buffer_no_antrian"];
        $tbContent[$i][$counter][TABLE_ALIGN] = "left";
        $tbContent[$i][$counter][TABLE_CLASS] = "tablecontent";
        $counter++;
        ...
        $tbContent[$i][$counter][TABLE_ISI] = $dataTable[$i]["nama_depan"]." ".$dataTable[$i]["nama_belakang"];
        $tbContent[$i][$counter][TABLE_ALIGN] = "left";
        $tbContent[$i][$counter][TABLE_CLASS] = "tablecontent";
        $counter++;
        ...
        $tbContent[$i][$counter][TABLE_ISI] = $dataTable[$i]["tujuan_periksa"].", ".$dataTable[$i]["tujuan_lain"];
        $tbContent[$i][$counter][TABLE_ALIGN] = "center";
        $tbContent[$i][$counter][TABLE_CLASS] = "tablecontent";
        $counter++;

        if ($dataTable[$i]["reg_buffer_jenis_pasien"]=="2") {
            # code...
          $tbContent[$i][$counter][TABLE_ISI] =  "Menyatakan benar-benar menggunakan pasien umum, tidak menggunakan asuransi/bpjs, dan apabila disarankan rawat inap maka berlaku sebagai pasien umum";
        }
