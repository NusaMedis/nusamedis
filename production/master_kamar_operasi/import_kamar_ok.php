<?php
    ...
    if(isset($_POST["btnImport"])){
      ...
      if ((!$myFile = @fopen(stripslashes($csvFile), "r")) || $err==true) {
        ...
        while ($data = fgetcsv($myFile, 500000, $delimiter))  {
          if($data[0]) {
            $dbTable = "temp.klinik_kamar_operasi";

            $dbField[] = "kamar_operasi_id";   // PK
            $dbField[] = "kamar_operasi_nama";
        ...
        $sql = "SELECT * FROM temp.klinik_kamar_operasi ORDER BY kamar_operasi_nama ASC";
  ...
  /* KLIK APPROVE */
    if(isset($_POST["btnApprove"])){
      $sql = "INSERT INTO klinik.klinik_kamar_operasi SELECT * FROM temp.klinik_kamar_operasi WHERE UPPER(kamar_operasi_nama) NOT IN (SELECT UPPER(kamar_operasi_nama) FROM klinik.klinik_kamar_operasi)";
      $dtaccess->Execute($sql);

      $sql = "DELETE FROM temp.klinik_kamar_operasi";
      $dtaccess->Execute($sql);

      header("Location: master_kamar_operasi_view.php");
      exit();
    }
  /* KLIK APPROVE */
?>
                      ...
                      <div class="x_content">
                            ...
                            <table class="table table-bordered">
                              ...
                              <tr>
                                <td colspan="2" align="right">
                                  ...
                                  <a href="master_kamar_operasi_view.php" class="btn btn-danger">Kembali</a>
                                </td>
                              </tr>
                            </table>
                          </form>
                        ...
                        <!-- Syarat Import -->
                          <font size="2" color="red">
                            ...
                            &nbsp;&nbsp;Kolom 1 &nbsp;&nbsp;: Nama Kamar Operasi <br />
                            &nbsp;&nbsp;Untuk Contoh data silahkan klik 
                            <a href="import_kamar_ok.csv">disini</a> <br>
                          </font>
                        <!-- Syarat Import -->
                      </div>
                    <!-- Sebelum Import -->
                    <?php if ($_POST['btnImport']) { ?>
                      <div class="x_content">
                        <table class="table table-bordered">
                          <tr>
                            <td>No</td>
                            <td>Nama Kamar Operasi</td>
                          </tr>
                          <?php if($dataTable) : ?>
                            <?php foreach($dataTable as $key => $value) : ?>
                              <tr>
                                <td><?= $key+1 ?></td>
                                <td><?= $value['kamar_operasi_nama'] ?></td>
                              </tr>
                            <?php endforeach; ?>
                          <?php endif; ?>
                        </table>
