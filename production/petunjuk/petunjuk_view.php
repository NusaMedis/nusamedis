<?php
 ...
 if($_GET["klinik"]) { $_POST["klinik"] = $_GET["klinik"]; 
     ...
     $addPage = "petunjuk_edit.php?tambah=".$_POST["klinik"];
     // -- paging config ---//
     $recordPerPage = 25;
    ...
    // if($_POST["klinik"] && $_POST["klinik"]!="--") $sql_where[] = "id_dep = ".QuoteValue(DPE_CHAR,$_POST["klinik"]); 
     if($_POST["_nama"]) $sql_where[] = "UPPER(petunjuk_nama) like ".QuoteValue(DPE_CHAR,strtoupper("%".$_POST["_nama"]."%"));
     if($sql_where) $sql_where = implode(" and ",$sql_where);
   
     $sql = "select a.* from apotik.apotik_obat_petunjuk a where id_dep ='$depId' ";
     if($sql_where) $sql .= " and ".$sql_where;
     $sql .= " order by petunjuk_nama asc ";
     $rs = $dtaccess->Execute($sql);
     $dataTable = $dtaccess->FetchAll($rs);
      //echo $sql;
      
     // --- ngitung jml data e ---              
     $sql = "select count(petunjuk_id) as total from  apotik.apotik_obat_petunjuk where id_dep ='$depId'";
    ...
    // }
     ...
     for($i=0,$counter=0,$n=count($dataTable);$i<$n;$i++,$counter=0){
       ...
       $counter++;
          ...
          //if($isAllowedUpdate) {
               $tbContent[$i][$counter][TABLE_ISI] = '<a href="'.$editPage.'?id='.$enc->Encode($dataTable[$i]["petunjuk_id"]).'&klinik='.$dataTable[$i]["id_dep"].'"><img hspace="2" width="32" height="32" src="'.$ROOT.'gambar/icon/edit.png" alt="Edit" title="Edit" border="0"></a>';               
               $tbContent[$i][$counter][TABLE_ALIGN] = "center";
               $counter++;
          //}
         // if($isAllowedDel) {
               $tbContent[$i][$counter][TABLE_ISI] = '<a href="'.$editPage.'?del=1&id='.$enc->Encode($dataTable[$i]["petunjuk_id"]).'"><img hspace="2" width="32" height="32" src="'.$ROOT.'gambar/icon/hapus.png" alt="Hapus" title="Hapus" border="0"></a>';               
               $tbContent[$i][$counter][TABLE_ALIGN] = "center";
               $counter++;
          //}
                ...
                <div class="x_panel">
                  ...
                  <div class="x_content">
					<form id="demo-form2" method="POST" class="form-horizontal form-label-left" action="<?php echo $_SERVER["PHP_SELF"]?>">
                        ...
                        <label class="control-label col-md-12 col-sm-12 col-xs-12">Nama Petunjuk</label>
