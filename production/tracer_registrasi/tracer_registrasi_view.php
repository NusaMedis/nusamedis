<?php 
    ...
    /*  if(!$auth->IsAllowed("man_ganti_password",PRIV_CREATE)){
	 ...
	 {
      global  $dtaccess; 
      $sql = "select reg_id, id_cust_usr, reg_tracer from klinik.klinik_registrasi";
	  $sql .=" where (reg_tracer_registrasi is null or reg_tracer_registrasi='n')  and reg_tracer_pending ='n' and ( reg_utama is null or reg_utama = '' ) ";
	  //$sql .=" and reg_status_pasien='L' ";
	//  $sql .=" and reg_jenis_pasien !='5' ";
       
         $rs = $dtaccess->Execute($sql);
         $dataTracer = $dtaccess->Fetch($rs);
		return $dataTracer['reg_id'];
	 }
       ...
       // -- end paging config ---//      
        ...
        $sql_where[] = "DATE(reg_tanggal) <= ".QuoteValue(DPE_DATE,date_db($in_tglakh));
          ...
          $sql = "select b.cust_usr_nama,b.cust_usr_kode,b.cust_usr_tempat_lahir,b.cust_usr_tanggal_lahir,
                  ...
                  where a.reg_status_pasien='L' and b.cust_usr_kode<>'500' and 
                  (a.reg_status='M0' or a.reg_status='E0' or 
                  a.reg_status='E1' or a.reg_status='E2' or a.reg_status='F0') 
                  and a.id_dep like '".$depId."' and (reg_tracer_registrasi is null or reg_tracer_registrasi='n') 
                  and (reg_tipe_rawat ='J' or reg_tipe_rawat ='G')";
		  $sql .= " 
     ...
     $table .= "</table>";
          ...
          for($i=0,$n=count($dataTable),$counter=0;$i<$n;$i++,$counter=0) {
               ...
               $tbContent[$i][$counter][TABLE_ISI] = '<img hspace="2" width="32" height="32" src="'.$ROOT.'../../../../gambar/icon/cetak.png" style="cursor: pointer; border:0px solid white" alt="Cetak Tracer" title="Cetak Tracer" onClick="ProsesCetakBarcode(\''.$enc->Encode($dataTable[$i]["cust_usr_id"]).'\',\''.$dataTable[$i]["reg_id"].'\');"/>';              
               $tbContent[$i][$counter][TABLE_ALIGN] = "center";
               $counter++; 

				        //echo "<script>returnProsesCetakBarcode(CekTracer('type=r'));</script>";
...
?>
<head>
<TITLE>TRACER REGISTRASI / KARCIS</TITLE>
...
function timer(){     
	 ...
	 if(CekTracer('type=r')){
		ProsesCetakBarcode(CekTracer('type=r'));
		//alert(CekTracer('type=r'));
	 }
} 
timer();
...
function ProsesCetakBarcode(id_reg) {
  BukaWindow('cetak_registrasi.php?reg_id='+id_reg+'','Cetak Tracer');
