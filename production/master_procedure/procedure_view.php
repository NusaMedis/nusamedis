  ...
  if(!$auth->IsAllowed("man_ganti_password",PRIV_CREATE)){
   ...
   /*  if(!$_GET["jenis"]) {
    ...
    if($_GET["id_poli"]) $_POST["id_poli"] = $_GET["id_poli"];
     ...
     $editPage = "procedure_edit.php?nama=".$_POST["_nama"]."&kode=".$_POST["_kode"];
     $hapusPage = "procedure_edit.php?del=1&nama=".$_POST["_nama"]."&kode=".$_POST["_kode"];
     $thisPage = "procedure_view.php?";
     
     // if($_POST["_kode"]) $sql_where[] = "upper(procedure_nomor) like ".QuoteValue(DPE_CHAR,"%".strtoupper($_POST["_kode"])."%");
     // if($_POST["_nama"]) $sql_where[] = "upper(procedure_nama) like ".QuoteValue(DPE_CHAR,"%".strtoupper($_POST["_nama"])."%");
     if($_POST["id_poli"]) $sql_where[] = " id_poli = ".QuoteValue(DPE_CHAR, $_POST["id_poli"]);

     //if($_POST["btnLanjut"] || $_GET["kode"] || $_GET["nama"]){}
     $sql = "select a.*, b.poli_nama from klinik.klinik_procedure a 
     ...
     $isAllowedDel = $auth->IsAllowed("man_medis_procedure",PRIV_DELETE);
     $isAllowedUpdate = $auth->IsAllowed("man_medis_procedure",PRIV_UPDATE);
     $isAllowedCreate = $auth->IsAllowed("man_medis_procedure",PRIV_CREATE);
     ...
     $tbHeader[0][$counterHeader][TABLE_ISI] = "Procedure";
    ...
    $counterHeader++;
     ...
     for($i=0,$counter=0,$n=count($dataTable);$i<$n;$i++,$counter=0){
          ...
          $tbContent[$i][$counter][TABLE_ISI] = $dataTable[$i]["procedure_nomor_tanpa_titik"];
          $tbContent[$i][$counter][TABLE_ALIGN] = "center";
          $counter++;

          $tbContent[$i][$counter][TABLE_ISI] = $dataTable[$i]["procedure_nomor"];
          $tbContent[$i][$counter][TABLE_ALIGN] = "center";
          $counter++;
          
          $tbContent[$i][$counter][TABLE_ISI] = $dataTable[$i]["procedure_nama"];
          $tbContent[$i][$counter][TABLE_ALIGN] = "left";          
          $counter++;

          $tbContent[$i][$counter][TABLE_ISI] = $dataTable[$i]["procedure_short_desc"];
          $tbContent[$i][$counter][TABLE_ALIGN] = "left";          
          $counter++;
          ...
          $counter++;
               ...
               $tbContent[$i][$counter][TABLE_ISI] = '<a href="'.$editPage.'&id='.$enc->Encode($dataTable[$i]["procedure_id"]).'"><img hspace="2" width="30" height="30" src="'.$ROOT.'gambar/icon/edit.png" alt="Edit" title="Edit" border="0"></a>';               
               $tbContent[$i][$counter][TABLE_ALIGN] = "center";
               $counter++;

               $tbContent[$i][$counter][TABLE_ISI] = '<img style="cursor: pointer;" hspace="2" width="30" height="30" src="'.$ROOT.'gambar/icon/hapus.png" alt="Hapus" id="delete" data-href="'.$enc->Encode($dataTable[$i]["procedure_id"]).'" title="Edit" border="0">';               
               $tbContent[$i][$counter][TABLE_ALIGN] = "center";
               $counter++;
                  ...
                  <div class="x_content">
                    ...
                    <script>
                      $("img#delete").click(function(){
                        var id_pro = $(this).data("href");

                        var r = confirm("Yakin akan menghapus data ini?");
                        if(r == true){
                          window.location.href = '<?=$hapusPage?>&id='+id_pro;
                        }
                        else{
