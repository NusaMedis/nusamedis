...
//echo "Diskon ".$_POST["txtDiskon"];  die();
   ...
   // echo $sql."<br>"; 
    ...
    {
      if($_POST["txtDiskonPersen"] || $_POST["txtDiskon"] || $_POST["txtServiceCash"] || $_POST["pembayaran_det_pembulatan"]) {
      ...
      $Pembulatan = $DataPembayaranLama["pembayaran_pembulatan"] + StripCurrency($_POST["pembayaran_det_pembulatan"]);
       ...
       $pembayaranTotal=StripCurrency($Total) + $ServiceCash;
       $pembayaranServiceCash=StripCurrency($ServiceCash);
       if(StripCurrency($_POST["txtDibayar"][0])>$_POST["total_harga"]){
       $pembayaranYgDibayar=StripCurrency($Total) + $ServiceCash;
       } else {
       $pembayaranYgDibayar=StripCurrency($Dibayar);
       }
       $pembayaranSubsidi=0;
       $pembayaranHrsBayar=StripCurrency($HrsBayar) + $ServiceCash;
    ...
    else if ($_POST["reg_jenis_pasien"]==TIPE_PASIEN_JKN)  //5
    {
      ...
      $Pembulatan = $DataPembayaranLama["pembayaran_pembulatan"] + StripCurrency($_POST["pembayaran_det_pembulatan"]);
       ...
       $pembayaranTotal=StripCurrency($_POST["total_harga"]) + $ServiceCash;
       }
       $pembayaranServiceCash=StripCurrency($ServiceCash);
       if($Dijamin=0){
       ...
       $pembayaranYgDibayar=StripCurrency($_POST["total_harga"]) + $ServiceCash;
       }
       $pembayaranSubsidi=StripCurrency($Subsidi) + $ServiceCash;
       $pembayaranHrsBayar=StripCurrency($HrsBayar) + $ServiceCash;
       if($Dijamin > $_POST["total_biaya"]){
       $pembayaranSelisihNegatif=0;
    ...
    else if ($_POST["reg_jenis_pasien"]==TIPE_PASIEN_IKS)  //7
    {
      ...
      $_POST["total_harga"] = StripCurrency($_POST["txtTotalDibayar"]); 
      //echo "0"; die();
      } elseif($_POST["txtDiskonPersen"]==0 || $_POST["txtDiskon"]==0 || $_POST["txtServiceCash"]==0 || $_POST["pembayaran_det_pembulatan"]==0) {
      $Total=StripCurrency($_POST["txtTotalDibayar"])+$DataPembayaranLama["pembayaran_yg_dibayar"]; 
      $_POST["total_harga"] = StripCurrency($_POST["txtTotalDibayar"]); 
      //echo "a"; die();
      } else {
      ...
      $Pembulatan = $DataPembayaranLama["pembayaran_pembulatan"] + StripCurrency($_POST["pembayaran_det_pembulatan"]);
       ...
       $pembayaranDiskonPersen=StripCurrency($DiskonPersen);
       $pembayaranJBayar=$_POST["id_jbayar"];
       $pembayaranDijamin = StripCurrency($Dijamin) + $ServiceCash;
       $pembayaranPembulatan = StripCurrency($Pembulatan);
    } 
    else if ($_POST["reg_jenis_pasien"]==TIPE_PASIEN_FASILITAS) //20
    {
      ...
      $Pembulatan = $DataPembayaranLama["pembayaran_pembulatan"] + StripCurrency($_POST["pembayaran_det_pembulatan"]);
       ...
       $pembayaranSubsidi=StripCurrency($HrsBayar);
       $pembayaranHrsBayar=StripCurrency($Subsidi);
       $pembayaranSelisihNegatif=0;
       $pembayaranSelisihPositif=0;
       $pembayaranDiskon=StripCurrency($Diskon);
       $pembayaranDiskonPersen=StripCurrency($DiskonPersen);
    ...
    //-- KESATUAN SEMUANYA UPDATE klinik_pembayaran
        ...
        $sql = "update klinik.klinik_pembayaran set 
                ...
                pembayaran_pembulatan = ".QuoteValue(DPE_NUMERIC,$pembayaranPembulatan).",pembayaran_catatan = ".QuoteValue(DPE_CHAR,$_POST["catatan"])."
                where pembayaran_id = ".QuoteValue(DPE_CHAR,$_POST["pembayaran_id"]);
        // echo $sql; die();
