...
else if($_POST["reg_tipe_rawat"] == 'G') $sql_where[] = "c.reg_status != 'G1' and c.reg_status != 'G0'";
...
if ($_POST["btnLanjut"] || $_POST["btnExcel"]) {
  ...
  c.reg_status, c.reg_jenis_pasien, c.reg_tipe_jkn, c.reg_tipe_rawat,c.reg_utama,
  d.poli_nama, f.jenis_nama,h.fol_nominal, reg_tanggal_pulang, reg_waktu_pulang, i.rawat_id ".$sqlIf."
  from 
  ...
  left join klinik.klinik_perawatan i on c.reg_id = i.id_reg
  where  c.reg_batal is null ";
  $sql .= " and reg_status !='I9' and c.id_poli<>" . QuoteValue(DPE_CHAR, $_POST["poli_tipe"]);
...
//echo $sql;
        ...
        if (!$_POST["btnExcel"]) {
          ...
          $tbHeader[0][$counterHeader][TABLE_ISI] = "Cetak INACBG";
          $tbHeader[0][$counterHeader][TABLE_WIDTH] = "5%";
          $counterHeader++;
    ...
    // code...
          $tbHeader[0][$counterHeader][TABLE_ISI] = "Transfer Tindakan";
          $tbHeader[0][$counterHeader][TABLE_WIDTH] = "5%";
          $counterHeader++;
  ...
  // $tbHeader[0][$counterHeader][TABLE_ISI] = "Input SEP";
  // $tbHeader[0][$counterHeader][TABLE_WIDTH] = "5%";
  // $counterHeader++;
        }
  ...
  // }
            ...
            $TransferPage = "transfer_tindakan.php?&id_reg=".$dataTable[$i]["reg_id"]."&id_pembayaran=".$dataTable[$i]['pembayaran_id']."&btnLanjut=Lanjut";
            ...
            $inputSEP = "bridging_inacbg.php?id_reg=".$dataTable[$i]["reg_id"];
 ...
 // echo $sql;
  ...
  //           $counter++;
            ...
            $tbContent[$i][$counter][TABLE_ISI] = '<img hspace="2" width="34" height="34" src="' . $ROOT . 'gambar/cetak.png" style="cursor:pointer" alt="Cetak INACBG" title="Cetak INACBG" border="0" onClick="CetakBridge(\'' . $dataTable[$i]["rawat_id"] . "-" . $dataTable[$i]["reg_tipe_rawat"]. '\');"/>';
            $tbContent[$i][$counter][TABLE_ALIGN] = "left";
            $tbContent[$i][$counter][TABLE_CLASS] = "tablecontent";
            $counter++;
            ...
            if ($_POST["reg_tipe_rawat"] == 'I' && $CheckLunas['id_pembayaran'] <> '' && $totalHarga == 0 || $dataDiskon['pembayaran_diskon'] > 0) {
    // code...
              $tbContent[$i][$counter][TABLE_ISI] = '<a href="'.$TransferPage.'" target="_blank"><img hspace="2" width="30" height="30" src="'.$ROOT.'gambar/transfer_pasien_inap.png" alt="Transfer Tindakan" title="Transfer Tindakan" border="0"/></a>';
  ...
  // $tbContent[$i][$counter][TABLE_ISI] = '<a href="'.$inputSEP.'"><img hspace="2" width="30" height="30" src="'.$ROOT.'gambar/icon/edit.png" alt="Edit" title="Input SEP" border="0"/></a>';;
  // $tbContent[$i][$counter][TABLE_ALIGN] = "left";
  // $tbContent[$i][$counter][TABLE_CLASS] = "tablecontent";
  // $counter++;
  //           $sql = "select id_pembayaran from klinik.klinik_pembayaran_det where id_pembayaran = ".QuoteValue(DPE_CHAR, $dataTable[$i]['pembayaran_id']);
  ...
  //           $sql = "select pembayaran_diskon from klinik.klinik_pembayaran where pembayaran_id = ".QuoteValue(DPE_CHAR, $dataTable[$i]['pembayaran_id']);
  //           $dataDiskon = $dtaccess->Fetch($sql);
  //           $sql = "select klinik_waktu_tunggu_when_create as prev from klinik.klinik_waktu_tunggu where id_waktu_tunggu_status='K0' or  id_waktu_tunggu_status='K1' AND id_reg = " .QuoteValue(DPE_CHAR,$dataTable[$i]['reg_id']);
  //           $rs = $dtaccess->Fetch($sql);
  //           if (!$rs["prev"] and $dataTable[$i]["reg_tipe_rawat"]=="I") {
  //   // $tbContent[$i][$counter][TABLE_ISI] = '<a href="'.$timePage.'"><img hspace="2" width="30" height="30" src="'.$ROOT.'gambar/icon/time.png" alt="Mulai Response Time" title="Pulang" border="0"/></a>';
  //             $tbContent[$i][$counter][TABLE_ISI] = '<img hspace="2" width="34" height="34" src="' . $ROOT . 'gambar/icon/time.png" style="cursor:pointer" alt="Mulai Response Time" title="Response Time" border="0" onClick="respontime(\'' . $dataTable[$i]["reg_id"] . "-" . $dataTable[$i]["id_cust_usr"] . "-" . $dataTable[$i]["id_poli"] . '\');"/>';
  ...
  /*
    ...
    // }
          ...
          $tbContent[$i][$counter][TABLE_ISI] = "Transfer Rawat Inap";
        }
        $tbContent[$i][$counter][TABLE_ALIGN] = "center";
        $tbContent[$i][$counter][TABLE_CLASS] = "tablecontent";
        $counter++;
...
// cari jenis pasien e
      $sql = "select * from global.global_jenis_pasien where jenis_id<>'20' and jenis_flag = 'y' order by jenis_nama desc";
...
// cari jenis bayar ee //
     ...
     function CetakBridge(id) {
      var all_id = id.split('-');
      if (all_id[1] == 'I'){
        window.open('cetak_bridge.php?id=' + all_id[0], 'Cetak INACBG');
      }
      else{
       window.open('cetak_bridge_rajal.php?id=' + all_id[0], 'Cetak INACBG');
     }
   ...
   function ProsesCetakIRNA(id) {
      ...
      // body...
       ...
       var id_reg=all_id[0];
        ...
        $.post("../pembayaran_irj/responsetime.php", {
            id_reg: id_reg,
            ...
            if (data == 'y') {
                ...
                alert("Berhasil Menyimpan Datay");
            }
             else if (data == 'n') {
                      ...
                      <div class="col-md-4 col-sm-6 col-xs-12">
                        ...
                        <div class="col-md-4 col-sm-6 col-xs-12">
                          ...
                          <select name="reg_jenis_pasien" class="select2_single form-control" id="reg_jenis_pasien">
                            <option value="5" <?php if ($_POST['reg_jenis_pasien'] == '5') echo "selected"; ?>>1. JKN / KIS</option>
                            <option value="7" <?php if ($_POST['reg_jenis_pasien'] == '7') echo "selected"; ?>>2. Asuransi</option>
                            <option value="20" <?php if ($_POST['reg_jenis_pasien'] == '20') echo "selected"; ?>>3. Karyawan</option>
                          </select>
                        </div>
                        <div class="col-md-4 col-sm-6 col-xs-12">
