<?php
  ...
  if($_GET["id_reg"] ) {
    ...
    $sql = "select b.biaya_jenis_sem, c.jenis_tindakan_nama
      ...
      where fol_lunas = 'n' and a.id_dep=".QuoteValue(DPE_CHAR,$depId)." 
      ...
      <?php
        ...
        <?php
          ...
          for($a = 0, $s = count($dataFolio); $a < $s; $a++){
            $sql = "select * from klinik.klinik_folio_pelaksana a left join global.global_auth_user b on b.usr_id = a.id_usr left join klinik.klinik_folio_posisi c on c.fol_posisi_id = a.id_fol_posisi where id_rol_jabatan = 'D' and id_fol = ".QuoteValue(DPE_CHAR,$dataFolio[$a]['fol_id']);
