     ...
     if(!$dataPas["reg_tipe_layanan"] && $dataPas["reg_kelas"]<>"1") $dataPas["reg_tipe_layanan"]="1";
      ...
      $noRef = $lastKode["kode"]+1; 
        ...
        if (StripCurrency($pembayaran_det_pembulatan) > 0) {
          ...
          $pembulatan = str_replace('-', '', StripCurrency($pembayaran_det_pembulatan));
        }
        ...
        StripCurrency($_POST["txtDiskon"]) + StripCurrency($_POST["deposit_nominal"])+$pembulatan;  
      
      if($_POST["total_harga"]>$bayar){
        if($dataPas["id_cust_usr"]=="100" || $dataPas["id_cust_usr"]=="500"){
          $keterangan ="Jurnal Penerimaan Kurang Bayar a.n ".$dataFolioPas[0]["fol_keterangan"]." (".$dataPas["cust_usr_kode"].") 
                        Tgl ".date_db($_POST['tanggal_posting'])." dengan nomor kwitansi ( ".$_POST['kwitansi_nomor']." )";
        }else{
