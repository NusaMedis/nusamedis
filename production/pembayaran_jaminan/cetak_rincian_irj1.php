<?php
    ...
    $sql = "select a.*, b.*, c.*, d.*
        ...
        where  a.fol_nama <> '' and a.id_biaya <> '9999999' and fol_nominal <> 0 and id_pembayaran =".QuoteValue(DPE_CHAR,$_POST["id_pembayaran"])." order by kategori_kassa_id, a.id_biaya, fol_nama asc" ;
        $dataFolio = $dtaccess->FetchAll($sql);
//echo $sql;
    $sql = "select a.*
        from klinik.klinik_folio a
        where  a.fol_nama <> '' and a.id_biaya = '9999999' and fol_nominal <> 0 and id_pembayaran =".QuoteValue(DPE_CHAR,$_POST["id_pembayaran"]);
        $dataRegFarmasi = $dtaccess->Fetch($sql);
//echo $sql;
    $sql = "select a.penjualan_nomor, a.penjualan_biaya_racikan, b.penjualan_detail_tuslag,a.penjualan_biaya_resep, b.* from apotik.apotik_penjualan a left join apotik.apotik_penjualan_detail b on b.id_penjualan = a.penjualan_id where a.id_pembayaran = ".QuoteValue(DPE_CHAR,$_POST["id_pembayaran"]);
...
<table width="100%" border="0" cellpadding="1" cellspacing="0" style="border-collapse:collapse">
  ...
  <tr>
      ...
      <td width="1%">:</td>
      <td><?php        echo str_replace("*", "'", $dataPasien["cust_usr_nama"]); ?></td>
  </tr>
  <tr>
...
<table width="100%" border="1" cellpadding="1" cellspacing="0" style="border-collapse:collapse">
    ...
    <tr>
        <td colspan="5"><b>&nbsp;&nbsp;&nbsp;&nbsp;Rawat Jalan</b></td>
    </tr>
    ...
    <?php 
        $subTotalIRJ += $dataFolio[$i]["fol_nominal"];
    ?>
        <?php } ?>
    <tr>
        <td colspan="6" align="right">SUB TOTAL</td>
        <td align="right"><?php echo currency_format($subTotalIRJ); ?></td>
    </tr>
    
    <?php if ($dataRegFarmasi["id_reg"]=='') { ?>
    <tr>
    ...
    <tr>
        ...
        <?php for ($i=0; $i < count($dataFarmasi); $i++) { ?>
    <tr>
        <td align="center"> 
            <?php echo $i+1; ?>
        </td>
        ...
        <td>
            <?php echo strtoupper($dataFarmasi[$i]["item_nama"]); ?>
        </td>
        <td>
        ...
        ?>
        <td align="right">
            <?php echo currency_format($dataFarmasi[$i]["penjualan_detail_total"]/$dataFarmasi[$i]["penjualan_detail_jumlah"]); ?>
        </td>
        <td align="right">
        ...
        <?php } } ?>
        <?php
           // if($dataFarmasi[0]["penjualan_biaya_racikan"] != 0 || $dataFarmasi[0]["penjualan_biaya_racikan"] != null){
         $sql = "select a.penjualan_nomor, a.penjualan_biaya_racikan, b.penjualan_detail_tuslag,a.penjualan_biaya_resep, b.* 
         from apotik.apotik_penjualan a 
         left join apotik.apotik_penjualan_detail b on b.id_penjualan = a.penjualan_id 
         where a.id_pembayaran = ".QuoteValue(DPE_CHAR,$_POST["id_pembayaran"])." and a.penjualan_biaya_racikan != '0' or a.penjualan_biaya_racikan != null";
        $dataRacikan = $dtaccess->FetchAll($sql);
         for ($i=0; $i < count($dataRacikan); $i++) {
        // if($dataFarmasi[$i]["penjualan_biaya_racikan"] != 0 || $dataFarmasi[$i]["penjualan_biaya_racikan"] != null){
        ?>
    <tr>
        <td colspan="6" align="right">Biaya Racikan</td>
        <td align="right"><?php echo currency_format($dataRacikan[$i]["penjualan_biaya_racikan"]); ?></td>

        <!-- <td colspan="6" align="right">Biaya Racikan</td>
        <td align="right"><?php echo currency_format($dataFarmasi[0]["penjualan_biaya_racikan"]); ?></td> -->
    </tr>
    <?php
        }
    ...
    <tr>
        ...
        <?php
            ...
            $subTotalFarmasi = $subTotalFarmasi + $dataFarmasi[0]["penjualan_biaya_resep"] + $dataFarmasi[0]["penjualan_biaya_racikan"];
        ?>
        <td align="right"><?php echo currency_format($subTotalFarmasi); ?></td>
    </tr>
    <tr>
        <?php
            $grandTotal = $subTotalIRJ + $subTotalFarmasi;
        ?>
