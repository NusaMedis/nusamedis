...
function GetData($in_nama,$in_kode){
  ...
  if(!$in_tglakh) { $in_tglakh = $skr; }
 	...
 	if($in_kode) $sql_where[] = "cust_usr_kode like '$in_kode%'";
	...
	$sql = "select a.cust_usr_id, b.reg_id,a.cust_usr_nama, a.cust_usr_kode , a.cust_usr_alamat, c.jenis_nama, b.id_pembayaran, d.poli_nama , b.reg_tanggal,
          ...
          where reg_tipe_rawat = 'I'  
          and cust_usr_kode <>'100' and b.reg_id in (select id_reg from klinik.klinik_rawatinap where id_reg = reg_id)";            
        //$sql .= " b.reg_status = 'E0' ";		 
...
function sendValue(id,kode,nama,reg,byr,tgl) {
	self.parent.document.getElementById('id_cust_usr').value = id;
	self.parent.document.getElementById('rm_anak').value = kode;
	self.parent.document.getElementById('nama_anak').value = nama;
  self.parent.document.getElementById('id_reg_anak').value = reg;
  self.parent.document.getElementById('id_pembayaran_anak').value = byr;
  self.parent.document.getElementById('tanggal_registrasi').value = tgl;
