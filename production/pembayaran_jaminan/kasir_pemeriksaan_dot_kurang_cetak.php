<?php
	...
	if($_GET["id_reg"] || $_GET["jenis"]  || $_GET["ket"] || $_GET["dis"] || $_GET["disper"] || $_GET["pembul"] || $_GET["total"]) {
		$sql = "select b.cust_usr_nama,b.cust_usr_kode,b.cust_usr_no_hp,b.cust_usr_jenis_kelamin,
            ...
            c.usr_name,e.jenis_nama,a.reg_kode_trans, a.reg_status
    ...
    $sql = "select * from klinik.klinik_folio where id_pembayaran = ".QuoteValue(DPE_CHAR,$_POST["id_pembayaran"])." 
...
/*		for($i=0,$n=count($dataFolio);$i<$n;$i++) { 
   ...
   $grandTotalHarga = $totalHarga;
    ...
    $sql = "select * from klinik.klinik_pembayaran_det a where a.id_pembayaran = ".QuoteValue(DPE_CHAR,$_POST["id_pembayaran"])." 
            and pembayaran_det_flag='T' order by pembayaran_det_ke";
    ...
    for($i=0,$n=count($dataPembDet);$i<$n;$i++){
      ...
      //update nomer kwitansi 
        ...
        $sql = "select * from klinik.klinik_pembayaran_uangmuka where id_pembayaran_det=".QuoteValue(DPE_CHAR,$_GET["pembayaran_det_id"]);
        $rs = $dtaccess->Execute($sql);
        $dataUangMuka = $dtaccess->Fetch($rs);
        
        if($dataUangMuka){
          $sql = "update klinik.klinik_pembayaran_uangmuka set uangmuka_no_kwitansi=".QuoteValue(DPE_CHAR,$_POST["kwitansi_nomor"])."
                  where uangmuka_id=".QuoteValue(DPE_CHAR,$dataUangMuka["uangmuka_id"]);
          $dtaccess->Execute($sql);
        }
...
<table width="100%" border="0" cellpadding="1" cellspacing="0" style="border-collapse:collapse"> 
  ...
  <tr>
    ...
    <td align="center" width="5%">&nbsp;</td>
    <td align="left" width="10%">Nama Dokter</td>  
    <td align="center" width="1%">:</td>  
    <td align="left" width="20%">&nbsp;<?php echo "&nbsp;".$dataPasien["usr_name"];?></td>  
  </tr>
  <tr>
...
<br>
<?php if($dataPasien["id_poli"]==$op["poli_id"]){?>
<table width="100%" border="0" cellpadding="1" cellspacing="0" style="border-collapse:collapse">
  <tr>
  <td align="center" width="100%">&nbsp;<font size="2">KWITANSI PEMBAYARAN<font size="2"></td>
  </tr>
</table>
<?php } ?>
<br>
...
<!--         <td align="left">&nbsp;</td>     -->
         ...
         <td align="right"><?php echo currency_format($dataFolio[$i]["fol_nominal"]);?></td>
          ...
          <? } else {?>
          <td width="10%" align='right'><?php if($_POST["reg_jenis_pasien"]=="5" && $dataPasien["id_poli"]<>$op["poli_id"]){echo "0";} else echo currency_format($dataFolio[$i]["fol_dijamin"])?></td>
 ...
 <!--   <td align="center">&nbsp;</td>    -->
    ...
    <? } else {?>
    <td align="right"><?php if($_POST["reg_jenis_pasien"]=="5" && $dataPasien["id_poli"]<>$op["poli_id"]){echo "0";} else echo currency_format($totalDijamin);?></td>
  ...
  <tr>
    ...
    <?php } ?>
    <td align="center">&nbsp;</td>
    <td align="left" colspan='2'>TOTAL HRS BAYAR : <?php if($dataPasien["reg_jenis_pasien"]=="5" && $dataPasien["id_poli"]<>$op["poli_id"]) {echo currency_format($totalHarga);} else echo currency_format($totalHrsBayar);?></td>
  ...
  <?php for($i=0,$n=count($dataPembayaranDet);$i<$n;$i++){ ?>
  <tr>
    <td align="left" colspan="2">* Pembayaran ke : <?php echo ($i+1);//$dataPembayaranDet[$i]["pembayaran_det_ke"];?> (<?php echo format_date($dataPembayaranDet[$i]["pembayaran_det_tgl"]); ?>)</td>
    <td align="right"><?php echo currency_format($dataPembayaranDet[$i]["pembayaran_det_total"]);?></td>
  <tr>
  <?php } ?>
  <tr>
    <td align="center" colspan="8">---------------------------------------------------------------------------------------------------------------------------------------------</td>
