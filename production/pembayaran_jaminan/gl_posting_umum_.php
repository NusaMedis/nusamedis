...
$sql = "select dep_coa_piutang_perorangan 
         from gl.gl_konf 
...
//   $pembulatan = '-'.StripCurrency($pembayaran_det_pembulatan);
// }else{
//   $pembulatan = StripCurrency($pembayaran_det_pembulatan);
// }
...
//       $dbValue[2] = QuoteValue(DPE_CHAR,$datacoaPiutangPerorangan["dep_coa_piutang_perorangan"]);
...
for($indexBayar=0,$jumBayar=3;$indexBayar<$jumBayar;$indexBayar++)   // Awal Looping Total Pembayaran dipaten 3 karen
{
    ...
    if ($_POST["txtdibayar".($indexBayar+1)]<>'') 
    {
      ...
      // if($dataPas["reg_jenis_pasien"]==TIPE_PASIEN_UMUM) {
        ...
        $bayar = StripCurrency($_POST["txtdibayar1"]) + StripCurrency($_POST["txtdibayar2"]) + StripCurrency($_POST["txtdibayar3"] + $_POST['deposit_nominal']) + 
        StripCurrency($_POST["txtDiskon"]) + StripCurrency($_POST["deposit_nominal"])+$bulat;    
        // echo $_POST['total_harga'].'-'.$bayar;die();
        if(($_POST["total_harga"]>$bayar && $_POST["total_harga"]>$_POST['deposit_nominal_awal']) && $indexBayar==0){    //KURANG BAYAR  Untuk Awak Transaksi //KURANG BAYAR  Untuk Awak Transaksi
          $beda = $_POST["total_harga"]-StripCurrency($_POST["txtdibayar".($indexBayar+1)]);
          $dbTable = "gl.gl_buffer_transaksidetil";
