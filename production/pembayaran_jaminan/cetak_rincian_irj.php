<?php
    ...
    $sql = "select a.*, b.*, c.*, d.*
		...
		where a.fol_nama <> '' and a.id_biaya <> '9999999' and id_pembayaran =".QuoteValue(DPE_CHAR,$_POST["id_pembayaran"])." order by kategori_kassa_id, a.id_biaya, fol_nama asc" ;
...
//echo $sql;
    $sql = "select a.penjualan_nomor, a.penjualan_biaya_racikan, a.penjualan_biaya_resep, b.* from apotik.apotik_penjualan a left join apotik.apotik_penjualan_detail b on b.id_penjualan = a.penjualan_id where (penjualan_grandtotal is not null or penjualan_grandtotal != 0) and id_pembayaran = ".QuoteValue(DPE_CHAR,$_POST["id_pembayaran"]);
  ...
  <tr>
      ...
      <td>
        <?php if ($dataPasien['cust_usr_kode'] != '100') {
            echo str_replace("*", "'", $dataPasien["cust_usr_nama"]);
        } else{
...
<table width="100%" border="1" cellpadding="1" cellspacing="0" style="border-collapse:collapse">
    ...
    <?php } ?>
        <?php 
            $sql = "select a.penjualan_nomor, a.penjualan_biaya_racikan, a.penjualan_biaya_resep, b.* 
            from apotik.apotik_penjualan a 
            left join apotik.apotik_penjualan_detail b on b.id_penjualan = a.penjualan_id 
            where a.id_reg = ".QuoteValue(DPE_CHAR,$dataRegFarmasi[$a]["id_reg"]);
            $dataFarmasi = $dtaccess->FetchAll($sql);
