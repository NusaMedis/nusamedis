<?php
     ...
     $editPage = "detail_masuk_edit.php?id_cara=".$_GET["id_cara"]."&id=".$_GET["id"];
     $thisPage = "detail_masuk_view.php?id_cara=".$_GET["id_cara"]."&id=".$_GET["id"];
     $backPage = "masuk_view.php?id_cara=".$_GET["id_cara"]."&id=".$_GET["id"];

     $prosedurId=$enc->Decode($_GET["id"]);
     $rujukanId=$enc->Decode($_GET["id_cara"]);
	...
	$rujukanFlag = $enc->Decode($_GET["id_cara"]);
	
	 $sql = "select * from global.global_rujukan_det 
			 where id_rujukan = ".QuoteValue(DPE_CHAR,$rujukanFlag);
    ...
    //TOTAL HEADER TABLE
	 ...
	 for($i=0,$counter=0,$n=count($dataInstalasi);$i<$n;$i++,$counter=0){
          ...
          $tbContent[$i][$counter][TABLE_ISI] = $dataInstalasi[$i]["rujukan_det_nama"];
          ...
          $counter++;
           ...
           if($isAllowedUpdate) {
               ...
               $tbContent[$i][$counter][TABLE_ISI] = '<a href="'.$editPage.'&id_det='.$enc->Encode($dataInstalasi[$i]["rujukan_det_id"]).'&id_cara='.$enc->Encode($dataInstalasi[$i]["rujukan_id"]).'"><img hspace="2" width="32" height="32" src="'.$ROOT.'gambar/icon/edit.png" alt="Edit" title="Edit" border="0"></a>';               
               $tbContent[$i][$counter][TABLE_ALIGN] = "center";
               $counter++;
               
          
               $tbContent[$i][$counter][TABLE_ISI] = '<a href="'.$editPage.'&id_det='.$enc->Encode($dataInstalasi[$i]["rujukan_det_id"]).'&del=1"><img hspace="2" width="32" height="32" src="'.$ROOT.'gambar/icon/hapus.png" alt="Hapus" title="Hapus" border="0" onclick="javascript: return Hapus();"></a>';               
               $tbContent[$i][$counter][TABLE_ALIGN] = "center";
               $counter++;
                }
...
//tampilin cara masuk-nya
	$sql = "select * from global.global_rujukan where rujukan_id = ".QuoteValue(DPE_CHAR,$rujukanId);
	$rs = $dtaccess->Execute($sql);
	$rujukanMasuk = $dtaccess->Fetch($rs);	
	  
	  $tableHeader = "&nbsp;Prosedur Masuk : ".$prosedurMasuk["prosedur_masuk_nama"]."<br>&nbsp;&nbsp;&nbsp;Cara Masuk : ".$rujukanMasuk["rujukan_nama"];
