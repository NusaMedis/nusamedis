<?php
     ...
     $editPage = "prosedur_edit.php?konf=".$_GET["konf"];
     $thisPage = "prosedur_view.php?konf=".$_GET["konf"];
     $detailPage = "masuk_view.php";
	...
	$isAllowedDel=1;
     ...
     function StripArr($num){
	   ...
	   $sql = "select * from global.global_prosedur_masuk order by prosedur_masuk_nama asc ";
     $rs = $dtaccess->Execute($sql,DB_SCHEMA);
     $dataInstalasi = $dtaccess->FetchAll($rs);
    // echo $sql;

	$table = new InoTable("table1","100%","left",null,1,2,1,null);     
  $tableHeader = "&nbsp;Prosedur Masuk";
	...
	$counter=0;
     ...
     $tbHeader[0][$counterHeader][TABLE_ISI] = "Prosedur Masuk";
    ...
    //TOTAL HEADER TABLE
	 ...
	 for($i=0,$counter=0,$n=count($dataInstalasi);$i<$n;$i++,$counter=0){
          ...
          $tbContent[$i][$counter][TABLE_ISI] = $dataInstalasi[$i]["prosedur_masuk_nama"];
          $tbContent[$i][$counter][TABLE_ALIGN] = "left";
          $counter++;
          
               $tbContent[$i][$counter][TABLE_ISI] = '<a href="'.$detailPage.'?id='.$enc->Encode($dataInstalasi[$i]["prosedur_masuk_id"]).'"><img hspace="2" width="32" height="32" src="'.$ROOT.'gambar/icon/cari.png" alt="Detail" title="Detail" border="0"></a>';               
               $tbContent[$i][$counter][TABLE_ALIGN] = "center";
               $counter++;
           ...
           if($isAllowedUpdate) {
               ...
               $tbContent[$i][$counter][TABLE_ISI] = '<a href="'.$editPage.'&id='.$enc->Encode($dataInstalasi[$i]["prosedur_masuk_id"]).'"><img hspace="2" width="32" height="32" src="'.$ROOT.'gambar/icon/edit.png" alt="Edit" title="Edit" border="0"></a>';               
               $tbContent[$i][$counter][TABLE_ALIGN] = "center";
               $counter++;
               
          
               $tbContent[$i][$counter][TABLE_ISI] = '<a href="'.$editPage.'&id='.$enc->Encode($dataInstalasi[$i]["prosedur_masuk_id"]).'&del=1"><img hspace="2" width="32" height="32" src="'.$ROOT.'gambar/icon/hapus.png" alt="Hapus" title="Hapus" border="0" onclick="javascript: return Hapus();"></a>';               
               $tbContent[$i][$counter][TABLE_ALIGN] = "center";
               $counter++;
                }
