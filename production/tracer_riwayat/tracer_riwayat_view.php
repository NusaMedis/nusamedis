<?php 
    ...
    /*  if(!$auth->IsAllowed("man_ganti_password",PRIV_CREATE)){
	 ...
	 {
      global  $dtaccess; 
      $sql = "select reg_id, id_cust_usr, reg_tracer_riwayat from klinik.klinik_registrasi";
	  $sql .=" where (reg_tracer_riwayat is null or reg_tracer_riwayat='n')  and reg_tracer_pending ='n' and ( reg_utama is null or reg_utama = '' ) ";
       ...
       // -- end paging config ---//      
        ...
        $sql_where[] = "DATE(reg_tanggal) <= ".QuoteValue(DPE_DATE,date_db($in_tglakh));
          ...
          $sql = "select b.cust_usr_nama,b.cust_usr_kode,b.cust_usr_tempat_lahir,b.cust_usr_tanggal_lahir,
                  ...
                  and a.id_dep like '".$depId."' and (reg_tracer_riwayat is null or reg_tracer_riwayat='n') 
                  and (reg_tipe_rawat ='J' or reg_tipe_rawat ='G' or reg_tipe_rawat ='I')
     ...
     $table .= "</table>";
          ...
          for($i=0,$n=count($dataTable),$counter=0;$i<$n;$i++,$counter=0) {
               ...
               $tbContent[$i][$counter][TABLE_ISI] = '<img hspace="2" width="32" height="32" src="'.$ROOT.'../../../../gambar/icon/cetak.png" style="cursor: pointer; border:0px solid white" alt="Cetak Tracer" title="Cetak Tracer" onClick="ProsesCetakBarcode(\''.$enc->Encode($dataTable[$i]["reg_id"]).'\',\''.$dataTable[$i]["reg_id"].'\');"/>';              
               $tbContent[$i][$counter][TABLE_ALIGN] = "center";
               $counter++; 
...
function ProsesCetakBarcode(id_reg) {
  BukaWindow('cetak_tracer_riwayat.php?id_reg='+id_reg+'','Cetak Tracer Riwayat');
