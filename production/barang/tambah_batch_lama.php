<?php
  ...
  $_POST["id_gudang"] = $gudang["id_gudang"];  //Ambil Gudang yang aktif  
...
/*    $dateNow = date_db($_POST["batch_tgl_jatuh_tempo"]);
    ...
    if($dateNow<$now) {
     ...
     exit();        
              ...
              if ($_POST["batch_tgl_jatuh_tempo"])
                   $dbValue[8] = QuoteValue(DPE_CHAR,date_db($_POST["batch_tgl_jatuh_tempo"]));   //sesuai konfigurasi apotik 
//  print_r($dbValue); die();           
              $dbKey[0] = 0; // -- set key buat clause wherenya , valuenya = index array buat field / value
              $dtmodel = new DataModel($dbTable,$dbField,$dbValue,$dbKey);
...
<?php //echo $view->RenderBody("module.css",true,false,"TAMBAH BATCH"); ?>
