<?php
     ...
     /*if(!$auth->IsAllowed("akunt_master_konfigurasi",PRIV_READ) && !$auth->IsAllowed("man_pengaturan_konf_akuntansi",PRIV_READ)){
          die("access_denied");
          exit(1);

     } elseif($auth->IsAllowed("akunt_master_konfigurasi",PRIV_READ)===1 || $auth->IsAllowed("man_pengaturan_konf_akuntansi",PRIV_READ)===1){
...
function GetData($in_nama=null){
  ...
  if($sql_where) $sql .= " and ".$sql_where;
    ...
    $tbContent[$i][$counter][TABLE_ISI] = "&nbsp;".$dataTable[$i]["no_prk"];
		...
		$tbContent[$i][$counter][TABLE_ISI] = '<img src="'.$ROOT.'gambar/icon/cari.png" style="cursor:pointer" border="0" alt="Pilih" title="Pilih" width="18" height="18" class="img-button" OnClick="javascript: sendValue(\''.addslashes(htmlentities($dataTable[$i]["nama_prk"])).'\',\''.$dataTable[$i]["no_prk"].'\',\''.$dataTable[$i]["id_prk"].'\')"/>';    
		$tbContent[$i][$counter][TABLE_ALIGN] = "center";
		$tbContent[$i][$counter][TABLE_CLASS] = $class;                    
		$counter++;
...
<form name="frmSearch">
	<br><br>
		<div class="col-md-12 col-sm-12 col-xs-12">
				<label><center>Pencarian&nbsp;Akun</center></label>
