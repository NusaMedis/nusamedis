...
if (!function_exists('_replace_double_quote')) {
  function _replace_double_quote($a)
  {
...
//     }
  ...
  $sql_where[] = "reg_tanggal >= " . QuoteValue(DPE_DATE, date_db($_POST["tanggal_awal"]));
  $sql_where[] = "reg_tanggal <= " . QuoteValue(DPE_DATE, date_db($_POST["tanggal_akhir"]));
...
// }
  ...
  //  {  //Jika Rawat Inap
    ...
    where  c.reg_batal is null and (reg_status = 'I2' or reg_status = 'I3')";
...
//echo $sql;
        ...
        $tableHeader = "&nbsp;Proses Mutu Pasien";
        $counterHeader = 0;
        ...
        if (!$_POST["btnExcel"]) {
          ...
          $tbHeader[0][$counterHeader][TABLE_ISI] = "Mulai Berkas Datang";
          $tbHeader[0][$counterHeader][TABLE_WIDTH] = "5%";
          $counterHeader++;

          $tbHeader[0][$counterHeader][TABLE_ISI] = "Informasi Tagihan Selesai";
          $tbHeader[0][$counterHeader][TABLE_WIDTH] = "5%";
          $counterHeader++;
        }
        ...
        if (!$_POST["btnExcel"]) {
          ...
          $tbHeader[0][$counterHeader][TABLE_ISI] = "Selesai Pembayaran";
          $tbHeader[0][$counterHeader][TABLE_WIDTH] = "5%";
          $counterHeader++;
 ...
 // echo $sql;
            ...
            $sql="select * from klinik.klinik_waktu_tunggu where id_reg='$dataTable[$i][reg_id]'";
            $rs_time = $dtaccess->Fetch($sql);

            if ($rs_time["id_waktu_tunggu_status"]=="K0") {
    // $tbContent[$i][$counter][TABLE_ISI] = '<a href="'.$timePage.'"><img hspace="2" width="30" height="30" src="'.$ROOT.'gambar/icon/time.png" alt="Mulai Response Time" title="Pulang" border="0"/></a>';
          ...
          if (is_null($rs_time["klinik_waktu_tunggu_when_update"])) {
    // $tbContent[$i][$counter][TABLE_ISI] = '<a href="'.$timePage.'"><img hspace="2" width="30" height="30" src="'.$ROOT.'gambar/icon/time.png" alt="Mulai Response Time" title="Pulang" border="0"/></a>';


            $tbContent[$i][$counter][TABLE_ISI] = '<img hspace="2" width="34" height="34" src="' . $ROOT . 'gambar/icon/time.png" style="cursor:pointer" alt="Mulai Response Time" title="Response Time" border="0" onClick="respontime1(\'' . $dataTable[$i]["reg_id"] . "-" . $dataTable[$i]["id_cust_usr"] . "-" . $dataTable[$i]["id_poli"] . '\');"/>';
          } 
          else { 
         ...
         $sql="select keterangan from klinik.klinik_waktu_tunggu where id_reg='$dataTable[$i][reg_id]' and id_waktu_tunggu_status='K0'";
         $rs_ket1 = $dtaccess->Fetch($sql);

         $tbContent[$i][$counter][TABLE_ISI] = '<div><textarea>' . _replace_double_quote($dataTable[$i]["reg_catatan"]) . '</textarea><br><button class="updateCatatan" data-id="' . $dataTable[$i]["reg_id"] . '">Simpan</button></div>';
         $tbContent[$i][$counter][TABLE_ALIGN] = "left";
         $tbContent[$i][$counter][TABLE_CLASS] = "tablecontent";
         $counter++;


         if ($rs_time["id_waktu_tunggu_status"]=="K1") {
    // $tbContent[$i][$counter][TABLE_ISI] = '<a href="'.$timePage.'"><img hspace="2" width="30" height="30" src="'.$ROOT.'gambar/icon/time.png" alt="Mulai Response Time" title="Pulang" border="0"/></a>';
           $tbContent[$i][$counter][TABLE_ISI] = " ";
         } 
         else { 

          $tbContent[$i][$counter][TABLE_ISI] = '<img hspace="2" width="34" height="34" src="' . $ROOT . 'gambar/icon/time.png" style="cursor:pointer" alt="Mulai Response Time" title="Response Time" border="0" onClick="respontime3(\'' . $dataTable[$i]["reg_id"] . "-" . $dataTable[$i]["id_cust_usr"] . "-" . $dataTable[$i]["id_poli"] . '\');"/>';
        }
        ...
        $sql="select keterangan from klinik.klinik_waktu_tunggu where id_reg='$dataTable[$i][reg_id]' and id_waktu_tunggu_status='K1'";
        $rs_ket2 = $dtaccess->Fetch($sql);

        $tbContent[$i][$counter][TABLE_ISI] = '<div><textarea>' . _replace_double_quote($dataTable[$i]["reg_catatan"]) . '</textarea><br><button class="updateCatatank1" data-id="' . $dataTable[$i]["reg_id"] . '">Simpan</button></div>';
        $tbContent[$i][$counter][TABLE_ALIGN] = "left";
        $tbContent[$i][$counter][TABLE_CLASS] = "tablecontent";
        $counter++;
...
// cari data poliklinik
      ...
      $sql_poli = "select poli_nama, poli_id from  global.global_auth_poli  where poli_tipe='I'";
       ...
       var id_reg=all_id[0];
         ...
         $.post("responsetime1.php", {
          ...
          if (data == 'y') {
            ...
            alert("Anda Melewati Batas Waktu >120 menit Harap Mengisi Kolom Keterangan");
          } else {
            alert("Gagal Menyimpan Data");
                    ...
                    <div class="x_content">
                      <form name="frmEdit" action="<?php echo $_SERVER["PHP_SELF"] ?>" method="POST" class="">
                        ...
                        <div class="col-md-4 col-sm-6 col-xs-12">
                          ...
                          <select name="reg_tipe_rawat" class="select2_single form-control" id="reg_tipe_rawat" required="";>
                            <option value="I2" <?php if ($_POST["reg_tipe_rawat"] == "J") echo "selected" ?>>Menginap</option>;
                            <option value="I3" <?php if ($_POST["reg_tipe_rawat"] == "I") echo "selected" ?>>Rencana Pulang</option>
                        ...
                        <div class="col-md-4 col-sm-6 col-xs-12">
                          ...
                          <select name="reg_jenis_pasien" class="select2_single form-control" id="reg_jenis_pasien">
                            <option value="2" <?php if ($_POST['reg_jenis_pasien'] == '2') echo "selected"; ?>>Umum</option>
                            <option value="3" <?php if ($_POST['reg_jenis_pasien'] == '3') echo "selected"; ?>>BPJS</option>
                          </select>
                        </div>
                        <div class="col-md-4 col-sm-6 col-xs-12">
...
<script type="text/javascript" language="JavaScript">
  ...
  $(".updateCatatan").click(function() {
    ...
    var catatan = $(this).parent('div').find('textarea').val();
    var jenis='K0';
    ...
    $.ajax({
      type: "GET",
      url: "update_catatan.php",
      data: _data,
      success: function(response) {
        if (response == 'ok') {
          alert('catatan berhasil diperbarui');
        } else {
          alert('catatan gagal diperbarui');
          window.location.reload()
        }
      },
      error: function() {
        alert('Sistem gagal merespon');
        window.location.reload()
      }
    });
  });

  $(".updateCatatanK1").click(function() {
    var id_reg = $(this).data('id');
    var catatan = $(this).parent('div').find('textarea').val();
    var jenis='K1';
