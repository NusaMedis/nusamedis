...
function HitungDetik($tanggalnya){
  if(($tanggalnya==NULL) && ($tanggalnya==0)){
    ...
    if(count($temp)!=6){
     ...
     if(!empty($_POST["id_poli"])) $sql_where2[] = "b.id_poli = ".QuoteValue(DPE_CHAR,$_POST["id_poli"]);
//     $sql_where[] = "1=1";
  ...
  if($_POST["btnLanjut"] || $_POST["btnExcel"]){  
     ...
     $sql = "select a.*,b.cust_usr_nama,b.cust_usr_kode, poli_nama from  klinik.klinik_registrasi a left join global.global_customer_user b on a.id_cust_usr = b.cust_usr_id
       left join global.global_auth_poli c on a.id_poli = c.poli_id";

     $sql .= " where c.poli_tipe = 'J' and ".$sql_where; 
    ...
    $jumlah_berdurasi = array(0,0);
     for($i=0,$counter=0,$n=count($dataRegistrasi);$i<$n;$i++,$counter=0){
      ...
      $durasi_detik = HitungDetik($datawaktuTunggu[$dataRegistrasi[$i]["reg_id"]][$dataStatus[$j]["waktu_tunggu_status_kode"]]["klinik_waktu_tunggu_durasi"]);


          if ($dataStatus[$j]["waktu_tunggu_status_urut"]>1)
          { 
          $tbContent[$i][$counter][TABLE_ISI] = $durasi_detik;//$datawaktuTunggu[$dataRegistrasi[$i]["reg_id"]][$dataStatus[$j]["waktu_tunggu_status_kode"]]["klinik_waktu_tunggu_durasi_detik"];
          $tbContent[$i][$counter][TABLE_ALIGN] = "center";          
          $counter++;

      $jumlah_berdurasi[$j-1]++;
      $total_durasi[$j-1]+=$durasi_detik;//$datawaktuTunggu[$dataRegistrasi[$i]["reg_id"]][$dataStatus[$j]["waktu_tunggu_status_kode"]]["klinik_waktu_tunggu_durasi_detik"];
          }
       }
       $tgl = DateAdd($tgl,1);
       //print_r($tgl);   
    ...
    //echo $sql;
     ...
     $dataDokter = $dtaccess->FetchAll($rs);
      ...
      if($_POST["btnExcel"]){
          ...
          header('Content-Disposition: attachment; filename=rekap_waktu_tunggu.xls');
      }  
     ...
     $tableHeader = "IRJ | Rekap Waktu Tunggu";
...
<?php if($_x_mode=="cetak"){ ?> 
  window.open('rekap_waktu_tunggu_cetak.php?klinik=<?php echo $_POST["klinik"];?>&tgl_awal=<?php echo $_POST["tgl_awal"];?>&tgl_akhir=<?php echo $_POST["tgl_akhir"];?>&shift=<?php echo $_POST["shift"];?>&dokter=<?php echo $_POST["id_dokter"];?>&id_jenis=<?php echo $_POST["id_jenis"];?>&id_poli=<?php echo $_POST["id_poli"];?>', '_blank');
<?php } ?>
...
?>
<h4 align="right">Rata-rata durasi Pasien Sampai di Poli: <?=FormatTime(round($total_durasi[0]/$jumlah_berdurasi[0]));?></h4>
<?
}
