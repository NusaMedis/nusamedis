$(document).ready(function() {
	...
	var form_list_kontrol = $('#form_list_kontrol');
	form_list_kontrol.submit(function(e) {
		e.preventDefault();
		$.ajax({
			url: uri+'kontrol-list.php',
			type: 'GET',
			data: form_list_kontrol.serialize(),
			dataType: 'json',
			beforeSend: function () {
			...
			success: function (rspns) {
				...
				if (rspns.metaData.code != 200) {
					...
					rspn = rspns.response.list;
					new PNotify({
						title: 'Sukses',
						text: 'Daftar Surat Kontrol ditemukan',
						type: 'success',
						styling: 'bootstrap3',
						addclass: 'dark'
					});
					var html = '';
					html  +=  '<table id="datatable" class="table table-bordered table-hover">';
					html  +=    '<thead>';
					html  +=      '<th>No. Surat</th>';
					html  +=      '<th>Kontrol/Inap</th>';
					html  +=      '<th>Tgl. Rencana Kontrol</th>';
					html  +=      '<th>Tgl. Entri</th>';
					html  +=      '<th>No. SEP Asal</th>';
					html  +=      '<th>Poli Asal</th>';
					html  +=      '<th>Poli Tujuan</th>';
					html  +=      '<th>Nama DPJP</th>';
					html  +=      '<th>No.Kartu</th>';
					...
					$.each(rspn, function (x,y) {
						html  +=      '<tr>';
						html  +=        '<td><a class="btn btn-xs btn-default" href=edit_kontrol.php?no_kontrol='+y.noSuratKontrol+'>'+y.noSuratKontrol+'</a></td>';
						html  +=        '<td>'+y.jnsPelayanan+'</td>';
						html  +=        '<td>'+y.tglRencanaKontrol+'</td>';
						html  +=        '<td>'+y.tglTerbitKontrol+'</td>';
						html  +=        '<td>'+y.noSepAsalKontrol+'</td>';
						html  +=        '<td>'+y.namaPoliAsal+'</td>';
						html  +=        '<td>'+y.namaPoliTujuan+'</td>';
						html  +=        '<td>'+y.namaDokter+'</td>';
						html  +=        '<td>'+y.noKartu+'</td>';
						html  +=        '<td>'+y.nama+'</td>';
						html  +=        '<td><a class="btn btn-xs btn-warning" target="_blank" href=cetak_kontrol.php?no_kontrol='+y.noSuratKontrol+'>Cetak</a><br/><button class="btn btn-xs btn-danger" onclick="setDestroy(\''+y.noSuratKontrol+'\')"> Hapus </button></td>';
						html  +=      '</tr>';
					})
					html  +=    '</tbody>';
					html  +=  '</table>';

					$('#listKontrol').html(html);					
				}
			},
			error: function () {
				$('.bpjs-loader').css('display', 'none');
	...
	function setDestroy(noKontrol){
		$.getJSON('hapus-kontrol.php', {noKontrol: noKontrol}, function(rspns) {
			if (rspns.metaData.code != 200) {
				new PNotify({
					title: 'Error',
					text: rspns.metaData.message,
...
// 	console.log('init_poli_kontrol');

// 	jnsLayanan = $('#tipe_pelayanan').val();
// 	tglLayanan = $('#rujukan_tglRujukan').val();
// 	nomor = $('#nomor_sep').val();


// 	$.ajax({
// 		url: uri+'poli-list.php',
// 		type: 'get',
// 		data: { jnsLayanan: jnsLayanan , nomor: nomor , tglLayanan: tglLayanan },
// 		dataType: 'json',
// 		beforeSend: function () {
// 			$('.bpjs-loader').css('display', 'block');
// 		},
...
// 					// html  +=    '<thead>';
// 					// html  +=      '<th>Nama Spesialis/Sub</th>';
// 					// html  +=      '<th>Kapsitas</th>';
// 					// html  +=      '<th>Jml. Rencana Kontrol & Rujukan</th>';
// 					// html  +=      '<th>Prosentase</th>';
...
// 					// 	html  +=        '<td>'+y.namaPoli+'</td>';
// 					// 	html  +=        '<td>'+y.kapasitas+'</td>';
// 					// 	html  +=        '<td>'+y.jmlRencanaKontroldanRujukan+'</td>';
// 					// 	html  +=        '<td>'+y.persentase+'</td>';
// 					// 	html  +=        '<td><a class="btn btn-xs btn-warning" onclick="setNoRujukan(\''+y.noKunjungan+'\')">'+'Pilih'+'</a></td>';
...
function createNoKontrol() {
	var form_sep = $('#form_kontrol');
	var datas = form_sep.serializeArray();
	datas.push(
		{ name: "noSEP", value: $('#no_sep').val() },
		{ name: "kodeDokter", value: $('#dpjp_kontrol').val() },
		{ name: "poliKontrol", value: $('#poli_tujuan_kontrol').val() },
		{ name: "tglRencanaKontrol", value: $('#tglRencanaKontrol').val() },
		);
