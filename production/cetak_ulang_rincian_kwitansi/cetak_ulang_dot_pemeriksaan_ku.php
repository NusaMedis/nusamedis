<?php
	...
	if($_GET["id_reg"] || $_GET["jenis"]  || $_GET["ket"] || $_GET["dis"] || $_GET["disper"] || $_GET["pembul"] || $_GET["total"]) {
		$sql = "select l.jkn_nama,b.cust_usr_nama,b.cust_usr_kode,b.cust_usr_no_hp,b.cust_usr_jenis_kelamin,
            ...
            c.usr_name,e.jenis_nama,a.reg_kode_trans,a.reg_tanggal,g.*,h.*, a.reg_tipe_paket, m.perusahaan_plafon, m.perusahaan_diskon
...
/*		for($i=0,$n=count($dataFolio);$i<$n;$i++) { 
   ...
   $grandTotalHarga = $totalHarga;
    ...
    $sql = "select a.*,b.jbayar_nama,c.* from klinik.klinik_pembayaran a 
            ...
            where c.pembayaran_det_id = ".QuoteValue(DPE_CHAR,$_GET["id_pembayaran_det"]).
            " and a.id_dep=".QuoteValue(DPE_CHAR,$depId);
     ...
     */
      ...
      // NYARI NOMER KWITANSI
   		$sql = "select pembayaran_det_kwitansi as kode from 
              ...
              where pembayaran_det_id=".QuoteValue(DPE_CHAR,$_GET["id_pembayaran_det"]);
...
<table width="100%" border="0" cellpadding="1" cellspacing="0" style="border-collapse:collapse">
  <tr> 
    <td align="center" width="100%">&nbsp;<font size="2">RINCIAN BIAYA TINDAKAN RAWAT JALAN<font size="2"></td>  
  </tr>
</table>
<br>
<table width="100%" border="0" cellpadding="1" cellspacing="0" style="border-collapse:collapse"> 
...
<!--         <td align="left">&nbsp;</td>     -->
 ...
 <!--   <td align="center">&nbsp;</td>    -->
  ...
  <tr>
    ...
    <?php } ?>
    <td align="center">&nbsp;</td>
    <td align="left" colspan='2'>TOTAL HRS BAYAR : <?php if($dataPasien["reg_jenis_pasien"]=="5" || $dataPasien["reg_jenis_pasien"]=="26") {echo currency_format($totalHarga);} else echo currency_format($totalHrsBayar);?></td>
  ...
  <tr>
    <td align="left" colspan="2">* Pembayaran ke : <?php echo $dataPembayaranDet[$i]["pembayaran_det_ke"];?></td>
