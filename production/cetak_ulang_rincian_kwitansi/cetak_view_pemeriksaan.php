...
if ($_POST["reg_tipe_rawat"] && $_POST["reg_tipe_rawat"] != "--" && $_POST["reg_tipe_rawat"] != "A"){
  $sql_where[] = "d.reg_tipe_rawat = " . QuoteValue(DPE_CHAR, $_POST["reg_tipe_rawat"]);
}
else if($_POST["reg_tipe_rawat"] == "A"){
  $sql_where[] = " j.poli_tipe = 'A' ";
}
...
if ($_POST["btnLanjut"] || $_POST["btnExcel"]) {
  $sql = "select a.pembayaran_det_id,a.pembayaran_det_kwitansi,a.id_reg,a.pembayaran_det_total,
             a.pembayaran_det_tgl,a.pembayaran_det_create,b.pembayaran_id,a.pembayaran_det_dibayar,
             c.cust_usr_kode,c.cust_usr_nama,
             d.reg_tipe_layanan,d.reg_tipe_rawat,reg_id, a.id_jbayar, pembayaran_id, a.pembayaran_det_deposit,
             h.jenis_nama,j.poli_nama,k.jbayar_nama from klinik.klinik_pembayaran_det a 
             ...
             left join global.global_jenis_bayar k on k.jbayar_id = a.id_jbayar";
  $sql .= " where (1=1) 
              and " . $sql_where;
...
for ($i = 0, $counter = 0, $n = count($dataTable); $i < $n; $i++, $counter = 0) {
  ...
  $sql = "select j.fol_keterangan, d.reg_keterangan from klinik.klinik_folio j 
  ...
  $sql = "select sum(deposit_history_nominal) as dp from klinik.klinik_deposit_history where deposit_history_flag = 'M' and id_cust_usr = " . QuoteValue(DPE_CHAR, $dataTable[$i]["id_cust_usr"])." and id_pembayaran = ".QuoteValue(DPE_CHAR,$dataTable[$i]['pembayaran_id']);
  ...
  $editPage = "edit_informasi.php?id_pembayaran=".$dataTable[$i]['pembayaran_id']."&id_reg=".$dataTable[$i]['reg_id'];
  ...
  if ($dataTable[$i]["cust_usr_kode"] == '500' || $dataTable[$i]["cust_usr_kode"] == '100') {
    $tbContent[$i][$counter][TABLE_ISI] = $namapasien;
  } else {
  ...
  $counter++;
  //}
  if ($dataTable[$i]['id_jbayar'] == 'Diskon') {
    $tbContent[$i][$counter][TABLE_ISI] = 'Diskon';
  }elseif ($dataTable[$i]['id_jbayar'] == 'DP') {
    $tbContent[$i][$counter][TABLE_ISI] = 'Deposit';
  }else{
  ...
  if ($dataTable[$i]['id_jbayar'] == 'DP') {
    $tbContent[$i][$counter][TABLE_ISI] = currency_format($dataTable[$i]["pembayaran_det_deposit"]);
  }else{
    $tbContent[$i][$counter][TABLE_ISI] = currency_format($dataTable[$i]["pembayaran_det_dibayar"]);
  }
  $tbContent[$i][$counter][TABLE_ALIGN] = "right";
  $counter++;
  $totalSeluruh += $dataTable[$i]["pembayaran_det_dibayar"];
...
$tableHeader = "Cetak Ulang Kwitansi";
if ($_POST["btnExcel"]) {
...
<script language="JavaScript">
  ...
  function ProsesCetakGlobal(id) {
    ...
    <?php if ($_POST["dep_konf_cetak_kasir"] == 'n') { ?>
      window.open('cetak_ulang_dot_pemeriksaan_ku.php?id_reg=' + all_id[0] + '&id_pembayaran=' + all_id[1] + '&pembayaran_jenis=' + all_id[2] + '&diskon=' + all_id[3] + '&diskon_persen=' + all_id[4] + '&pembayaran_det_id=' + all_id[5], 'Cetak Ulang Kwitansi Klinik');
    <? } else { ?>
      window.open('cetak_tagihan.php?id_reg=' + all_id[0] + '&pembayaran_id=' + all_id[1] ,'Cetak Ulang Global' );
    <?php } ?>
    //document.location.href='<?php echo $thisPage; ?>';
  }
                ...
                <form name="frmView" method="POST" action="<?php echo $_SERVER["PHP_SELF"]; ?>" onSubmit="return CheckSimpan(this);">
                  <table align="center" border=0 cellpadding=2 cellspacing=1 width="100%" id="tblSearching">
                    ...
                    <div class="col-md-4 col-sm-6 col-xs-12">
                      ...
                      <select name="reg_tipe_rawat" class="select2_single form-control" id="reg_tipe_rawat" onKeyDown="return tabOnEnter(this, event);">
                        ...
                        <option value="A" <?php if ('A' == $_POST["reg_tipe_rawat"]) echo "selected"; ?>><?php echo "Farmasi"; ?></option>
                      </select>
                    ...
                    <div class="col-md-4 col-sm-6 col-xs-12">
                      ...
                      <select name="reg_jenis_pasien" class="select2_single form-control" id="reg_jenis_pasien" onKeyDown="return tabOnEnter(this, event);">
                        <!--onChange="this.form.submit();" -->
