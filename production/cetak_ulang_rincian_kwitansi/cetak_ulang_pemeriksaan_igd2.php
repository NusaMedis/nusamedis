<?php
    ...
    if($_GET["id_reg"] || $_GET["pembayaran_id"]) {
        $sql = "select b.cust_usr_nama,b.cust_usr_kode,b.cust_usr_no_hp,b.cust_usr_jenis_kelamin,
            ...
            c.usr_name,e.jenis_nama,a.reg_kode_trans,a.reg_tanggal,a.reg_umur_hari,a.reg_umur_bulan,a.reg_umur,
             ...
             k.jkn_nama, 
            a.reg_no_sep, a.reg_kelas, l.perusahaan_nama, a.reg_diagnosa_inap, hak_kelas_inap,a.id_poli,b.cust_usr_jkn,a.reg_waktu, reg_keterangan
    ...
    $sql = "select a.*, b.*, c.*, d.*
        ...
        and a.fol_nama <> '' and a.id_poli <> '33' and id_pembayaran =".QuoteValue(DPE_CHAR,$_POST["id_pembayaran"])." order by kategori_kassa_id, a.id_biaya, fol_nama asc" ;
        $dataFolio = $dtaccess->FetchAll($sql);
//echo $sql;
    $sql = "select a.*
        from klinik.klinik_folio a
        where a.id_dep=".QuoteValue(DPE_CHAR,$depId)." 
        and a.fol_nama <> '' and a.id_poli = '33' and id_pembayaran =".QuoteValue(DPE_CHAR,$_POST["id_pembayaran"]);
        $dataRegFarmasi = $dtaccess->Fetch($sql);
//echo $sql;
    $sql = "select a.penjualan_nomor, a.penjualan_biaya_racikan, a.penjualan_biaya_resep, b.* from apotik.apotik_penjualan a left join apotik.apotik_penjualan_detail b on b.id_penjualan = a.penjualan_id where a.id_reg = ".QuoteValue(DPE_CHAR,$dataRegFarmasi["id_reg"]);
...
<table width="100%" border="0" cellpadding="1" cellspacing="0" style="border-collapse:collapse">
  ...
  <tr>
      ...
      <td width="1%">:</td>
      <td>
        <?php if ($dataPasien['cust_usr_kode'] != '100') {
            echo $dataPasien["cust_usr_nama"];
        } else{
            echo $dataPasien['reg_keterangan'];
        }
        ?>
       </td>
  </tr>
...
<table width="100%" border="1" cellpadding="1" cellspacing="0" style="border-collapse:collapse">
    ...
    <tr>
        ...
        <td>&nbsp;</td>
        <td align="right">
            <?php echo currency_format($dataFolio[$i]["fol_hrs_bayar"]); ?>
        </td>
    </tr>
    ...
    <tr>
        ...
        ?>
        <td align="right">
            <?php echo currency_format(intval(intval($hargaSatuan*1.1)*1.05)); ?>
        </td>
        <td align="right">
