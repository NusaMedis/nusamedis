...
/*if($_POST["btnPilih"]){
$addDetailPage = "input_racikan.php?q=".$batchId."&item=".$_POST['id_racikan']."&kode=".$enc->Encode($kode)."&transaksi=".$enc->Encode($penjualanId)."&id_reg=".$enc->Encode($idReg)."&id_pembayaran=".$pembayaranId."&jenis_id=".$_POST['jenis_id'];
  ...
  //simpan stok_batch_dep
        ...
        $rs = $dtaccess->Execute($sql);
          ...
          $dbField[16] = "id_jam_aturan_pakai";
               ...
               $penjualandetId = $dtaccess->GetTransID();
          if ($Konfigurasi["conf_biaya_tuslag_persen"] == "y") {
               $tuslag = $Konfigurasi["conf_biaya_tuslag"];
               $row['item_tuslag'] = currency_format($hargasatuan*($tuslag/100));
        }
          $totalsatuan = StripCurrency($hargasatuan*$_POST["penjualan_detail_jumlah"]);
          $totaltuslag = StripCurrency($_POST["penjualan_detail_jumlah"]*$hargasatuan*($tuslag/100));
          $totalharga = StripCurrency($totalsatuan+$totaltuslag);
          ...
          $dbValue[12] = QuoteValue(DPE_NUMERIC,StripCurrency($row["item_tuslag"]));
                ...
                <div class="x_panel">
                  ...
                  <div class="x_content">
                      <form name="frmView" action="<?php echo $_SERVER["PHP_SELF"]?>" method="POST" >
                        ...
                        <select class="form-control asc" name="id_racikan" id="id_racikan" onKeyDown="return tabOnEnter_select_with_button(this, event);" >
                          <?php 
          ...
          <table><th>Silahkan klik tombol Tambah Master Racikan</th></table>
                    <? } ?>
                       ...
                       <?php } ?>               
                            ...
                            <tr><th>Jam Aturan Minum</th><th><select class="form-control" name="id_jam_aturan_pakai" id="id_jam_aturan_pakai" class="inputField" onKeyDown="return tabOnEnter_select_with_button(this, event);" >
                              <option value="">[- Pilih Jam Aturan Pakai -]</option>
                            <?php for($i=0,$n=count($dataJam);$i<$n;$i++) { ?>
                              <option value="<?php echo $dataJam[$i]["jam_aturan_pakai_id"];?>" <?php if($_POST["id_jam_aturan_pakai"]==$dataJam[$i]["jam_aturan_pakai_id"]) echo "selected";?>><?php echo $dataJam[$i]["jam_aturan_pakai_nama"];?></option>
                       <?php } ?>               
                         </select></th></tr>
