...
// SECURITY CHECKER
$sql = "select penjualan_terbayar, id_fol from apotik.apotik_penjualan where penjualan_id = ".QuoteValue(DPE_CHAR,$penjualanId);
$dataPenjualan = $dtaccess->Fetch($sql);

$sql = "select reg_utama from klinik.klinik_registrasi where reg_id = ".QuoteValue(DPE_CHAR,$_POST['id_reg']);
$RegUtama = $dtaccess->Fetch($sql);

$sql = "select reg_tipe_rawat from klinik.klinik_registrasi where reg_id = ".QuoteValue(DPE_CHAR,$RegUtama['reg_utama']);
$TipeRawat = $dtaccess->Fetch($sql);

$sql = "select * from gl.gl_buffer_transaksi where id_pembayaran_det = ".QuoteValue(DPE_CHAR,$penjualanId);
$Jurnal = $dtaccess->Fetch($sql);

$sql = "select fol_lunas from klinik.klinik_folio where fol_id = ".QuoteValue(DPE_CHAR,$dataPenjualan['id_fol']);
...
if ($_x_mode == "Edit"  || $_POST["tombol_f2"]) {  //Jika ditekan Tombol Lanjut atau keyboard F2s
  ...
  $sql = "select b.id_cust_usr as cust_usr_kode, b.*,f.*, c.reg_jenis_pasien , c.reg_status , c.reg_tanggal, c.reg_id, c.id_pembayaran, c.id_poli, d.rawat_terapi, c.id_dokter, g.jenis_nama, h.jkn_nama, i.perusahaan_nama,  
  ...
  $_POST["catatan"] = (!$_POST["btnBayar"]) ?  $dataPasien["penjualan_catatan"] : $_POST["catatan"];
          ...
          /* jika ITEM RACIKAN  */
            for ($i=0; $i <count($cek2) ; $i++) { 
              ...
              $sql="DELETE  from logistik.logistik_stok_item 
                  where stok_item_saldo=".QuoteValue(DPE_CHAR,$stok['stok_dep_saldo'])."
            ...
            $sql = "select * from logistik.logistik_stok_item where id_gudang = ".QuoteValue(DPE_CHAR,$theDep)." and id_item = ".QuoteValue(DPE_CHAR,$cek2[$i]['id_item'])." order by id_gudang asc, stok_item_create asc";
           ...
           /* jika bukan racikan */
            ...
            $sql = "select * from logistik.logistik_stok_item where id_gudang = ".QuoteValue(DPE_CHAR,$theDep)." and id_item = ".QuoteValue(DPE_CHAR,$item['id_item'])." order by id_gudang asc, stok_item_create asc";
            $dataAdjustment = $dtaccess->FetchAll($sql);
            /* SQL PENGURUTAN */
...
$sql = "select penjualan_keterangan,penjualan_catatan,cust_usr_nama from apotik.apotik_penjualan
             where penjualan_id = " . QuoteValue(DPE_CHAR, $penjualanId);
$rs = $dtaccess->Execute($sql);
$tanggungan = $dtaccess->Fetch($rs);

$_POST["fol_keterangan"] = $tanggungan["cust_usr_nama"];
...
$sql = "select a.*,b.item_nama,b.item_kode,b.item_racikan,c.jenis_nama,d.petunjuk_nama, f.batch_no, 
			f.batch_tgl_jatuh_tempo, g.aturan_minum_nama, h.aturan_pakai_nama, i.jam_aturan_pakai_nama, j.id_pembayaran
...
if($dataCheckPembayaran){
  echo "<script> alert('Sudah Dibayar'); location.replace('penjualan_bebas_view.php'); </script>";
}
...
<script language="Javascript">
  ...
  function GantiHarga(dari) {
    ...
    document.getElementById('txtIsiTotale').innerHTML = formatCurrency(parseInt((hrgmargin) + (pajakJumlah) + parseInt(tuslagJumlah)));
    document.getElementById('txtHargaTotal').value = formatCurrency(parseInt((hrgmargin) + (pajakJumlah) + parseInt(tuslagJumlah)));
    document.getElementById('hargaitem').value = formatCurrency(parseInt((hrgmargin)));
    document.getElementById('hargabeliitem').value = formatCurrency(parseInt((hargaBeliJumlah)));
  ...
  $(document).ready(function() {
    ...
    $('#obat_nama').autocomplete({
      ...
      onSelect: function(suggestion) {
        ...
        // alert stok
        // if ((suggestion.data.stok_batch_dep_saldo <= suggestion.data.item_stok_alert) && suggestion.data.stok_batch_dep_saldo > 0) {
        ...
        // ketika stoknya kosong
        // if (suggestion.data.stok_batch_dep_saldo <= 0) {
        //   $('#alert').html("Stok Obat ini Habis ! ");
        //   $('#btnSave').hide();
        //   $('#btnS').show();
        // } else {
        //   $('#btnSave').show();
                ...
                <div class="x_content">
                  <?php if ($_x_mode == 'New') { ?>
                    <form name="frmFind" id="demo-form2" method="POST" class="form-horizontal form-label-left" action="<?php echo $_SERVER["PHP_SELF"] ?>">
                      ...
                      <div class="form-group">
                        <label class="control-label col-md-3 col-sm-3 col-xs-12" for="last-name">Jenis Pasien<span class="required">*</span>
                        </label>
                        <div class="col-md-6 col-sm-6 col-xs-12">
                          <select class="form-control" name="reg_jenis_pasien" id="reg_jenis_pasien" required="">
                            <option>- Pilih Jenis Pasien -</option>
                            <option value="2">Umum</option>
                            <option value="20">Karyawan</option>
                          </select>
                          <input type="hidden" name="custTambah" id="custTambah" /></div>
                      </div>
                      <div id="pgw" class="form-group" style="display: none;">
                        <label class="control-label col-md-3 col-sm-3 col-xs-12">Nama Karyawan <h6>*Jika Jenis Pasien Karyawan</h6></label>
                        <div class="col-md-4 col-sm-4 col-xs-12">
                          <a href="<?php echo $findPage; ?>&TB_iframe=true&height=550&width=800&modal=true" class="thickbox" title="Cari Pasien">
                          <?php echo $view->RenderTextBox("pgw_nama", "pgw_nama", "30", "100", $_POST["pgw_nama"], "inputField", "readonly", false); ?><img src="<?php echo ($ROOT); ?>gambar/finder.png" border="0" align="top" style="cursor:pointer" title="Cari Pasien" alt="Cari Pasien" /></a>
                          <?php echo $view->RenderHidden("pgw_id", "pgw_id", $_POST["cust_usr_id"]); ?>
                          <?php echo $view->RenderHidden("pgw_nip", "pgw_nip", $_POST["cust_usr_id"]); ?>
                    ...
                    <form name="frmFind" id="demo-form2" method="POST" class="form-horizontal form-label-left" action="<?php echo $_SERVER["PHP_SELF"] ?>">
                      ...
                      ?>
                        <div class="control-label col-md-6 col-sm-6 col-xs-12">
                          ...
                          <div class="form-group">
                            <label class="control-label col-md-3 col-sm-3 col-xs-12">Cara Bayar <span class="required">*</span></label>
                          ...
                          <div class="form-group">
                            ...
                            <div class="col-md-4 col-sm-4 col-xs-12">
                                <textarea id="catatan" name="catatan" class="form-control col-md-7 col-xs-12"><?php echo $_POST["catatan"]; ?></textarea>
                            </div>
                          </div>
                          <div class="form-group">
                        ...
                        <table id="datatable-responsive" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
                          <tr>
                            ...
                            <td align="left" width="15%">Jmlh</td>
                          ...
                          ?>
                            ...
                            <tr class="tablecontent-odd">
                              ...
                              if($dataTable[$i]["item_racikan"] == 'y  '){
                                echo '<a href="' . $editRacikan . '">' . $dataTable[$i]["item_nama"] . '&nbsp;' . $tambahan . '</a>'; 
                              }
                              else{
      ...
      <td align="left"  class="tablecontent-odd">
        <input type="text" id="batch_tgl_jatuh_tempo" name="batch_tgl_jatuh_tempo" class="form-control">
                          ...
                          <tr class="tablesmallheader">
                            <td align="center" width="2%" colspan="2"><input type="submit" name="btnSave" id="btnSave" value="Tambah Obat" class="submit" onClick="javascript:return CekTindakan(document.frmEdit);"><input type="button" style="display:none;" name="btnS" id="btnS" value="Tambah Obat" class="submit"></td>
...
<script type="text/javascript">
  $('#reg_jenis_pasien').on('change', function() {
    var q = $('#reg_jenis_pasien').val();
    if (q == 20) {
      $('#pgw').css('display', 'block');
    }else{
      $('#pgw').css('display', 'none');
    }
  });
</script>
</html>