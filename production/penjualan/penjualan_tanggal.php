...
//     $poli = $auth->GetPoli();
   ...
   if(!$auth->IsAllowed("man_ganti_password",PRIV_CREATE)){
     ...
     $sql = "select a.*, b.cust_usr_kode, b.cust_usr_tanggal_lahir, c.jenis_nama, d.id_pembayaran,e.poli_nama, d.reg_utama from apotik.apotik_penjualan a
    ...
    // }
     ...
     for($i=0,$counter=0,$n=count($dataTable);$i<$n;$i++,$counter=0){
      ...
      $sql = "SELECT rawat_terapi from klinik.klinik_perawatan where id_reg = ".QuoteValue(DPE_CHAR,$dataTable[$i]['reg_utama']);
      ...
      if($dataTable[$i]["penjualan_terbayar"]=='n' && ($TipeRawat['reg_tipe_rawat'] == 'G' || $TipeRawat['reg_tipe_rawat'] == 'J')){
        ...
        $tbContent[$i][$counter][TABLE_ISI] = '<img hspace="2" width="30" height="30" src="'.$ROOT.'gambar/cetak.png" style="cursor:pointer" alt="Cetak Kwitansi" title="Cetak Kwitansi" border="0" onClick="ProsesCetakCatatanDokter(\''.$dataTable[$i]["reg_utama"].'\');"/>';
        }else{
         $tbContent[$i][$counter][TABLE_ISI] = ''; 
        }
        $tbContent[$i][$counter][TABLE_ALIGN] = "center";
