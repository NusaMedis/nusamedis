...
$sql .= "and b.id_dep = " . QuoteValue(DPE_CHAR, $depId) . " and f.id_gudang =" . QuoteValue(DPE_CHAR, $theDep);
...
for ($i = 0; $i < count($dataTable); $i++) {
     ...
     $row['item_harga_beli'] = intval($hargabeli);
     $row['item_harga_margin'] = intval($hmargin);
     $row['item_harga_jual'] = intval($hmargin);
     $row['margin_nilai'] = intval($margin["margin_nilai"]);
     $row['hpp'] = $dataTable[$i]['item_hpp'];
     if ($Konfigurasi["conf_biaya_tuslag_persen"] == "y") {
          $tuslag = $Konfigurasi["conf_biaya_tuslag"];
          $row['item_tuslag'] = intval(($hmargin + $ppn) * ($tuslag / 100));
     }
     $row['ppn'] = intval($hmargin * 0.1);
