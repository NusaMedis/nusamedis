...
if (isset($_GET['reg_id'])) {
  ...
  $sql1 = "select id_cust_usr from klinik.klinik_registrasi where reg_id ='$_GET[reg_id]'and id_dep='$depId'";
  $rs1 = $dtaccess->Execute($sql1);
  $dataTable1 = $dtaccess->Fetch($rs1);

  $sql = "select * from klinik.klinik_perawatan where id_dep ='$depId' and id_reg = " . QuoteValue(DPE_CHAR, $_GET["reg_id"]);
  $dataRawat = $dtaccess->Fetch($sql);

  if($dataRawat["waktu_mulai_asmed"] == null){
  ...
  $sql = "select a.reg_id,b.rawat_id from klinik.klinik_registrasi a left join klinik.klinik_perawatan b on b.id_reg = a.reg_id where a.id_cust_usr = " . QuoteValue(DPE_CHAR, $dataTable1["id_cust_usr"]) . " and a.reg_tipe_rawat = 'J' and a.id_poli <> '33' and a.reg_status <>' ' and a.id_poli <> '20' and reg_when_update <> '$waktu_tglSekarang' and a.id_dep ='$depId' order by reg_tanggal desc limit 1 ";
  ...
  $sql = "select rawat_tanggal,rawat_pemeriksaan_fisik,rawat_diagnosa_utama,rawat_anamnesa,rawat_ket,rawat_obgyn, rawat_anak, rawat_poli from klinik.klinik_perawatan where rawat_id = " . QuoteValue(DPE_CHAR, $dtRegTerakhir["rawat_id"]);
  //echo $sql;
  ...
  $sql = "SELECT anamnesa_isi_detail_nilai from klinik.klinik_anamnesa_isi_detail where id_anamnesa_pilihan_detail = '53863fbdc19bc43b1d9c5401dd7eaa6e' and id_rawat = ".QuoteValue(DPE_CHAR, $dataRawat['rawat_id']);
  $dataAlergi = $dtaccess->Fetch($sql);
  ...
  $USGTambahanLalu = $dataRawatObgyn['USGTambahan'];
  ...
  $jenis_kel_lalu = $dataRawatObgyn['jenis_kelamin'];
  
  $ket_diagnosa_empat_lalu = $diag_lalu;

  if(count($procedure_lalu) > 0){
    for($i = 0; $i < count($procedure_lalu); $i++){
      $strp[] = implode("-", $procedure_lalu[$i]);
    }
    array_push($strp, $dataRawatObgyn['planning_penatalaksanaan']);
    $proc_lalu = implode(" ; ", $strp);
  }
  else{
    $proc_lalu = $dataRawatObgyn['planning_penatalaksanaan'];
  }
...
// echo $dataRawatObgynSekarang['usia_kehamilan_hari1'];
 ...
 //Materi Edukasi 
  ...
  $USGTambahan = $dataRawatObgynSekarang['USGTambahan'];
...
if ($LetakJaninMelintangSkg) {
  ...
  if ($dataLayananTerakhir['anamnesa_isi_nilai'] == '0bstetri') {
    ...
    if($GS0 || $GS1 || $GS2 || $FetalPool || $Fetus || $CRL || $DJJ){
      $janin = ($GS1 == '1') ? 'Tunggal' : '';
      $janin = ($GS1 == '2') ? 'Kembar' : '';
     $UsgTerakhir = 'GS : ' . $GS0 . ' - ' . $janin . ' - ' . $GS2 . ' ; Fetal Pool : ' . $FetalPool . ';' . ' Fetus : ' . $Fetus . ' ; CRL : ' . $CRL . " mm " . ' ; BPD : ' . $BPD . ' cm ' . ' ; DJJ : ' . $DJJ . ' GA : ' . $GAMinggu . ' Minggu' . ' ' . $GAHari . ' Hari' . '; HPL/TP : ' . $HPLTP.'; Tambahan : '.$USGTambahanLalu.' ; Jenis kelamin : '.$jenis_kel_lalu;
    }
    else{
       $UsgTerakhir = 'Janin : ' . $tunggal . ' ' . $kembar . ' ' . $hidup . ' ' . $iufd . ';' . ' Letak Janin : ' . $LetakKepala . ' ' . $LetakSungsang . ' ' . $LetakMelintang . ' ' . $LetakOblique . ';' . ' ; BPD : ' . $BPD . ' cm ' . "; Plasenta : " . $plasental . " Grade : " . $Grade . '; GA : ' . $GAMinggu . ' Minggu' . ' ' . $GAHari . ' Hari' . '; HPL/TP : ' . $HPLTP.'; Tambahan : '.$USGTambahanLalu.' ; Jenis kelamin : '.$jenis_kel_lalu;
    }
...
else if($dataTable[0]['form_asmed'] == 'dalam'){
  $dataRawatLalu = unserialize($dataPerawatanTerakhir['rawat_poli']);
  $dataRawatSkr = unserialize($dataRawat['rawat_poli']);
}
...
$Obat = '';
  ...
  for ($i = 0; $i < count($dataTable); $i++) {
    ...
    array_push($data, [
      ...
      'cust_usr_alergi'   => $dataAlergi['anamnesa_isi_detail_nilai'],
      ...
      'janin_kembar_det' => $dataRawatObgynSekarang['janin_kembar_det'],
    ...
    for($x=1; $x < $dataRawatObgynSekarang['janin_kembar_det']; $x++){
       $data[0]['jenis_kelamin_'.$x] = $dataRawatObgynSekarang['jenis_kelamin_'.$x];
    }
      ...
      array_push($data, [
        ...
        'rawat_terapi'   => $terapiArea,
        'rawat_diagnosa_utama'   => $dataTable[$i]['rawat_diagnosa_utama'],
        'rawat_pemeriksaan_dalam'   => $dataTable[$i]['rawat_pemeriksaan_dalam'],
        'rawat_id'   => $dataTable[$i]['rawat_id'],
        'layanan'   => $dataLayanan['anamnesa_isi_nilai'],
        'rawat_tanggal_terakhir'   => date_db($dataPerawatanTerakhir['rawat_tanggal']),
        'rawat_anamnesa_terakhir'   => $dataRawatLalu['keluhanUtama'],
        'rawat_pemeriksaan_fisik_terakhir'   => $dataPerawatanTerakhir['rawat_pemeriksaan_fisik'],
        'rawat_diagnosa_utama_terakhir'   => $dataPerawatanTerakhir['rawat_diagnosa_utama'],
        'rawat_ket_terakhir'   => $dataRawatLalu['planning_penatalaksanaan'],
        'obat'   => $terapiAreaTerakhir,
        'obat_sekarang'   => $ObatSekarang,
        'id_rawat_lalu' => $dtRegTerakhir['rawat_id'],
        'diagnosa_lalu' => $dataRawatLalu['diagnose_skr'],
        'diagnosa_sekarang' => $dataRawatSkr['diagnose_skr'],
        'lap_tindakan' => $lap_tindakan,
        'reg_tanggal_terakhir'   => format_date($dataHistory['reg_tanggal_terakhir']),
        'reg_terakhir'   => $dataPerawatanTerakhir['rawat_id'],

        'lainlainSkg'   => $dataRawatSkr['lain-lain'],
        'status_lokalis'   => $dataRawatSkr['status_lokalis'],
        'ket_diagnosa_empat'   => $dataRawatSkr['ket_diagnosa_empat'],
        'lap_tindakan'   => $dataRawatSkr['lap_tindakan'],
        'terapi'   => $terapiArea,

        'memahamiMateri' => $dataRawatSkr['memahamiMateri'] ,
        'butuhLeaflet' => $dataRawatSkr['butuhLeaflet'] ,
        'membatasiMateri' => $dataRawatSkr['membatasiMateri'] ,
        'pengulanganMateri' => $dataRawatSkr['pengulanganMateri'],
        'bisaMengulang' => $dataRawatSkr['bisaMengulang'] ,
        'lain_lainEdukasi' => $dataRawatSkr['lain_lainEdukasi'] ,
        'lainEd_det' => $dataRawatSkr['lainEd_det'] ,

        'diagnosa' => $dataRawatSkr['diagnosa'] ,
        'penjelasan_penyakit' => $dataRawatSkr['penjelasan_penyakit'] ,
        'pemeriksaan_penunjang' => $dataRawatSkr['pemeriksaan_penunjang'] ,
        'terapi_edukasi' => $dataRawatSkr['terapi_edukasi'],
        'tindakan_medis' => $dataRawatSkr['tindakan_medis'] ,
        'perkiraan_hari_rawat' => $dataRawatSkr['perkiraan_hari_rawat'] ,
        'penjelasan_komplikasi' => $dataRawatSkr['penjelasan_komplikasi'] ,
        'informed_concent' => $dataRawatSkr['informed_concent'] ,
        'kondisi' => $dataRawatSkr['kondisi'] ,
        'konsul' => $dataRawatSkr['konsul'] ,
        'konsul_det' => $dataRawatSkr['konsul_det'] ,
        'edukasi_pulang' => $dataRawatSkr['edukasi_pulang'] ,
        'edukasi_lain' => $dataRawatSkr['edukasi_lain'] ,
        'lain_det' => $dataRawatSkr['lain_det'] ,
      ]);
    }
    else if($dataTable[0]['form_asmed'] == 'dalam'){
      array_push($data, [
        ...
        'rawat_kasus_keterangan'   => $dataTable[$i]['rawat_kasus_keterangan'],
        'cust_usr_foto'   => $dataTable[$i]['cust_usr_foto'],
        'rawat_ket'   => $dataRawatSkr['planning_penatalaksanaan'],
        'rawat_terapi'   => $terapiArea,
        'rawat_diagnosa_utama'   => $dataTable[$i]['rawat_diagnosa_utama'],
        'rawat_pemeriksaan_dalam'   => $dataTable[$i]['rawat_pemeriksaan_dalam'],
        'rawat_id'   => $dataTable[$i]['rawat_id'],
        ...
        'ket_diagnosa_empat'   => $dataRawatSkr['ket_diagnosa_empat'],
        'lap_tindakan'   => $dataRawatSkr['lap_tindakan'],
        'pemeriksaanPenunjang'   => $dataRawatSkr['pemeriksaanPenunjang'],
        'terapi'   => $terapiArea,
  ...
  $sql = "select a.reg_no_antrian, a.reg_tanggal,a.reg_id,a.reg_kode_trans,a.reg_waktu,a.reg_status,a.reg_tipe_jkn,a.id_pembayaran,b.cust_usr_id,b.cust_usr_kode,b.cust_usr_kode_tampilan,b.cust_usr_nama,b.cust_usr_tanggal_lahir,b.cust_usr_alamat,c.poli_nama, f.jenis_nama, e.jkn_nama,perusahaan_nama, a.reg_status_pasien, q.usr_name
    ...
    ";
  $sql .= " where id_pembayaran notnull  and c.poli_tipe = 'J' and a.reg_status <>' '  and d.id_usr = '$userId' and a.id_dep = '$depId' ";
  $sql .= $kondisi;
  ...
  for ($i = 0; $i < count($dataTable); $i++) {
    ...
    array_push($data, [
      ...
      'dokter'   => $dataTable[$i]['usr_name'],
       'reg_no_antrian'   => $dataTable[$i]['reg_no_antrian']
    ]);
  }
