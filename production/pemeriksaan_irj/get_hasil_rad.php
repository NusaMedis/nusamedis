...
//     where a.id_reg = '$rad[reg_id]' and a.id_reg is not null and a.id_reg != '' order by tindakan_tanggal, tindakan_waktu desc OFFSET $start LIMIT $limit "; 
...
if ($id_cust_usr) {
  ...
  #ambil data
  $sql = "select *, b.id_reg from radiologi.radiologi_resume a
  left join  klinik.klinik_folio b on b.fol_id=a.id_fol
  left join klinik.klinik_perawatan_tindakan d on b.fol_id  = d.id_fol
  left join klinik.klinik_biaya c on b.id_biaya  = c.biaya_id
      where a.id_cust_usr = '$id_cust_usr'  and a.id_reg is not null and a.id_reg != '' order by when_create desc OFFSET $start LIMIT $limit ";
  $rs = $dtaccess->Execute($sql);
  $dataTable = $dtaccess->FetchAll($rs);
  ...
  for ($i = 0; $i < count($dataTable); $i++) {
    ...
    array_push($data, [
      'id_reg'   => $dataTable[$i]['id_reg'],
      'tindakan_tanggal'   => date_format(date_create($dataTable[$i]['when_create']), 'd-m-Y'),
      'tindakan_waktu'   =>   date_format(date_create($dataTable[$i]['when_create']), 'H:i:s'),
      'resume_id'   => $dataTable[$i]['resume_id'],
      'fol_jumlah'   => $dataTable[$i]['fol_jumlah'],
      'rawat_tindakan_id'   => $dataTable[$i]['rawat_tindakan_id'],
      'id_biaya'   => $dataTable[$i]['id_biaya'],
