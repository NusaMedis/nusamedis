<?php
  ...
  $sql = "select a.biaya_tarif_id, a.biaya_total, b.biaya_nama, a.is_cito
    ...
    if ($dataKelasTarif["dep_konf_tarif_jenis_pasien"]=='y') {
      $sql .=" and a.id_jenis_pasien = ".QuoteValue(DPE_CHAR,$_POST['jenis_pasien']);
    }
    
    //$sql .=" and ( a.id_kelas = ".QuoteValue(DPE_CHAR,'4023d2b9644b5c0ec1090d1dc0c60aa3'); //kelas 2 / pasien umum
    $sql .=" and a.id_kelas = ".QuoteValue(DPE_CHAR,$dataKelasTarif["dep_konf_reg_kelas_irj"]); //NON CLASS
...
//echo $sql;
  $sql = "select c.biaya_tarif_id,c.biaya_total,b.biaya_nama from  klinik.klinik_biaya_registrasi a 
               left join klinik.klinik_biaya b on a.id_biaya = b.biaya_id 
               left join klinik.klinik_biaya_tarif c on b.biaya_id = c.id_biaya
              where a.id_poli=".QuoteValue(DPE_CHAR,$_POST["id_poli"]);
  $karcis = $dtaccess->Fetch($sql);

  #push biaya karcis
  array_push($data, ['biaya_tarif_id' => $karcis['biaya_tarif_id'], 'biaya_total' => $karcis['biaya_total'], 'biaya_nama' => $karcis['biaya_nama'] ]);
