...
if ($_GET["id"] || $_GET["id_reg"]) {
	...
	$sql = "SELECT b.* from klinik.klinik_perawatan a 
			left join klinik.klinik_perawatan_diagnosa b on a.rawat_id = b.id_rawat
			where rawat_diagnosa_urut = '1' and id_reg = ". QuoteValue(DPE_CHAR, $_GET["id_reg_utama"]);
	$dataDiagnosa = $dtaccess->Fetch($sql);
...
if ($_GET['id_reg']) {
	$sql = "select a.tindakan_tanggal, tindakan_waktu,a.fol_id, a.id_biaya, a.id_biaya_tarif, a.id_reg, a.fol_lunas, a.id_dokter as dokter, a.fol_jumlah, 
			...
			left join global.global_auth_poli i on i.poli_id = h.id_poli
...
<title>TINDAKAN <?php echo strtoupper($dataTable[0]['poli_nama']) ?> CETAK</title>
...
<body>
	<div class="wrapper">
		<div>
			...
			<br>
			<img class="img" src="<?php echo $ROOT; ?>lib/barcode2/barcode.php?text=<?php echo $barcode ?>&print=true&size=30>">


			<!-- <p style="font-size:13px; margin-bottom: -10px;"> <?= $_REQUEST['nmps'] ?> (<?= $_REQUEST['norm'] ?>)</p> -->
			<h5 style="margin-bottom:-20px; ">TINDAKAN <?php echo strtoupper($dataTable[0]['poli_nama']) ?> <?php echo date('d-m-Y'); ?><h5>
		...
		<div class="block" style="margin-top: 8px">
			<b>Diagnosa : <?=$dataDiagnosa['rawat_diagnosa_kode']." - ".$dataDiagnosa['rawat_diagnosa_tindakan_nama']?></b>
		</div>
		<br>
		<br>
		<div>
