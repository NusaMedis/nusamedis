...
if ($LetakJaninMelintangSkg) {
  ...
  if ($dataLayananTerakhir['anamnesa_isi_nilai'] == '0bstetri') {
     ...
     if ($pmrksaanObgyn['insersi_fudus']) { 
        $plasental =  ($pmrksaanObgyn['insersi_fudus'] == 'true') ? 'Fundus;' : '' ;
       } 
       if ($pmrksaanObgyn['insersi_corpus']) { 
        $plasental =  ($pmrksaanObgyn['insersi_corpus'] == 'true') ? 'Corpus;' : '';
       } 
       if ($pmrksaanObgyn['insersi_sbr']) { 
        $plasental =  ($pmrksaanObgyn['insersi_sbr'] == 'true') ? 'SBR;' : '' ;
       } 
       if ($pmrksaanObgyn['insersi_anterior']) { 
        $plasental =  ($pmrksaanObgyn['insersi_anterior'] == 'true') ? 'Ant;' : '';
       } 
      if ($pmrksaanObgyn['insersi_posterior']) { 
       $plasental =  ($pmrksaanObgyn['insersi_posterior'] == 'true') ? 'Post;' : '' ;
       } 
...
$Obat = '';
  ...
  $sql = "select a.reg_no_antrian, a.reg_tanggal,a.reg_id,a.reg_kode_trans,a.reg_waktu,a.reg_status,a.reg_tipe_jkn,a.id_pembayaran,b.cust_usr_id,b.cust_usr_kode,b.cust_usr_kode_tampilan,b.cust_usr_nama,b.cust_usr_tanggal_lahir,b.cust_usr_alamat,c.poli_nama, f.jenis_nama, e.jkn_nama,perusahaan_nama, a.reg_status_pasien, q.usr_name
    ...
    ";
  $sql .= " where c.poli_tipe = 'J' and a.reg_status <>' '  and d.id_usr = '$userId' ";
  $sql .= $kondisi;
