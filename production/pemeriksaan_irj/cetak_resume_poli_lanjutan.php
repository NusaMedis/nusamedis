...
$sql = "SELECT *, e.waktu_asmed, a.id_dokter from klinik.klinik_registrasi a
...
$sql_hpht = "SELECT anamnesa_isi_detail_nilai FROM klinik.klinik_anamnesa_isi_detail WHERE id_rawat = '$rawat_id' AND id_anamnesa = '5d2c31995c9ef8fbad77eaee2abc6ecd' AND id_anamnesa_detail = '50e70246c321cc4b0309bb9f5203fd42' and anamnesa_isi_detail_nilai not like '%Hari%' and anamnesa_isi_detail_nilai not like '% - %'";
...
$sql_FU = "SELECT anamnesa_isi_detail_nilai FROM klinik.klinik_anamnesa_isi_detail WHERE id_rawat = '$rawat_id' AND id_anamnesa = 'cb155cdaee721f191fbdbfbd18b7d060' AND id_anamnesa_detail = 'a9e2977ec5f1ed34dac3c39d9283e410'";
...
<body>
	<div class="wrapper" style="width: 18cm; margin : auto;">
		...
		<div>
			...
			<?php if($fisik['Letak Anak'] || $pmrksaanObgyn['kepala'] || $pmrksaanObgyn['sungsang'] || $pmrksaanObgyn['oblique'] || $pmrksaanObgyn['lintang'] ) { 
				if($pmrksaanObgyn['kepala']) $fisik['Letak Anak'] = 'Kepala';
				if($pmrksaanObgyn['sungsang']) $fisik['Letak Anak'] = 'Sungsang';
				if($pmrksaanObgyn['oblique']) $fisik['Letak Anak'] = 'Oblique';
				if($pmrksaanObgyn['lintang']) $fisik['Letak Anak'] = 'Lintang';

				?> 
				Letak Anak : <?=$fisik['Letak Anak']?> <br>
			<?php } ?>
			<?php if($pmrksaanObgyn['tinggi_fu'] || $dataFU['anamnesa_isi_detail_nilai']) { ?>Tinggi FU : <?=($pmrksaanObgyn['tinggi_fu']) ? $pmrksaanObgyn['tinggi_fu'] : $dataFU['anamnesa_isi_detail_nilai'] ?><br><?php } ?>
...
<?php if (
	...
	<div style="margin-top: -15px;">
		<div class="block"> Janin &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; : <?= ($pmrksaanObgyn['janin_tunggal'] == 'true') ? 'Tunggal;' : '' ?>
			<?= ($pmrksaanObgyn['janin_kembar'] == 'true') ? 'Kembar;('.$pmrksaanObgyn['janin_kembar_det'].')' : '' ?>
...
<?php }?>
	<?php for($x=1; $x < $pmrksaanObgyn['janin_kembar_det']; $x++) { 
		if($pmrksaanObgyn['jenis_kelamin_'.$x]){
		?>
		<div>
			<div class="block">
				Kelamin &nbsp; &nbsp; &nbsp;:
				<?php
				if ($pmrksaanObgyn['jenis_kelamin_'.$x] == 'Laki') {
					echo "Laki - laki";
				}elseif ($pmrksaanObgyn['jenis_kelamin_'.$x] == 'Perempuan') {
					echo "Perempuan";
				}else{
					echo "&nbsp;";
				}
...
<?php endif;
		?>
		<?php if($pmrksaanObgyn['USGTambahan']) { ?>
		<div>
			<div class="block">
				USG Tambahan&nbsp; &nbsp; &nbsp;:<br>
				<?php
					echo $pmrksaanObgyn['USGTambahan'];
				?>
			</div>
		</div>
		<?php } ?>
		...
		<div style="margin-top: 7px">
			...
			<?php 
			$pmrksaanObgyn['ket_diagnosa_satu'] = ($pmrksaanObgyn['ket_diagnosa_satu'] == '' && $pmrksaanObgyn['usia_kehamilan_minggu1'] != '') ? $pmrksaanObgyn['usia_kehamilan_minggu1']." Mgg" : $pmrksaanObgyn['ket_diagnosa_satu'];

			if($pmrksaanObgyn['janin_hidup'] == null || $pmrksaanObgyn['janin_iufd'] == null){
				if($pmrksaanObgyn['djj'] == '+'){
					$pmrksaanObgyn['ket_diagnosa_tiga'] = "Hidup";
				}
				else if($pmrksaanObgyn['djj'] == '-'){
					$pmrksaanObgyn['ket_diagnosa_tiga'] = "IUFD";
				}
				else{
					$pmrksaanObgyn['ket_diagnosa_tiga'] = "";
				}
			}
			else if($pmrksaanObgyn['janin_hidup']){
				$pmrksaanObgyn['ket_diagnosa_tiga'] = "Hidup";
			}
			else if($pmrksaanObgyn['janin_iufd']){
				$pmrksaanObgyn['ket_diagnosa_tiga'] = "IUFD";
			}
...
?>
		<div>
			...
			<?php if($pmrksaanObgyn['haid_lama_hari']){ ?> Haid Selama : <?=$pmrksaanObgyn['haid_lama_hari']?> Hari <br> <?php } ?>
			
			<?php if($pmrksaanObgyn['haid_lama_banyak_hari']){ ?> Haid lama dan banyak : <?=$pmrksaanObgyn['haid_lama_banyak_hari']?> Hari <br> <?php } ?>

			<?php if($pmrksaanObgyn['haid_lama_bulan']){ ?> Haid selama 1 bulan : <?=$pmrksaanObgyn['haid_lama_bulan']?> Kali <br> <?php } ?>

			<?php if($pmrksaanObgyn['terus_menerus_lama']){ ?> Lama haid terus menerus : <?=$pmrksaanObgyn['terus_menerus_lama']?> Hari <br> <?php } ?>
		...
		<div style="white-space: pre-line;">
			<?=$pmrksaanObgyn['pemeriksaan_dalam']?>
		</div>
		<div style="margin-top: 5px;">
