...
//    $sql="select * from klinik.klinik_folio where id_pembayaran = ".QuoteValue(DPE_CHAR,$dataReg["id_pembayaran"]);
//    $dataLunas = $dtaccess->Fetch($sql);

$sql = "select count(fol_id) as lunas from klinik.klinik_folio WHERE id_pembayaran = " . QuoteValue(DPE_CHAR, $dataReg['id_pembayaran']) . " and fol_lunas = 'y'";
$lunas = $dtaccess->Fetch($sql);
   if($lunas['lunas'] > 0 ){

   	if ($dataReg['reg_utama'] == '') {
   		$status = 'E3';
   	}elseif ($dataReg['reg_status'] != '') {
   		$status = 'E9';
   	}elseif ($dataReg['id_poli'] != $dataReg['id_poli_asal']) {
   		$status = 'E9';
   	}
		...
		$dbValue[1] = QuoteValue(DPE_CHAR,$status);
    ...
    #data Registrasi
     ...
     $dataRegistrasi = $dtaccess->Fetch($sql);
		  ...
		  $dbValue[4] = QuoteValue(DPE_CHAR,"E2");
		  ...
		  $dbValue[8] = QuoteValue(DPE_CHAR,$_POST["jenis_kb_id"]);	
				//print_r($dbValue); die();
         $dbKey[0] = 0; // -- set key buat clause wherenya , valuenya = index array buat field / value
         $dtmodel = new DataModel($dbTable,$dbField,$dbValue,$dbKey);
		 $dtmodel->Update() or die("update  error");	
...
$sql = "update klinik.klinik_folio set tipe_rawat = 'J' where id_pembayaran = ".QuoteValue(DPE_CHAR,$dataRegistrasi['id_pembayaran']);
$result = $dtaccess->Execute($sql);
		 ...
		 // ---- insert ke klinik waktu tunggu ----
		  ...
		  $dbValue[5] = QuoteValue(DPE_CHAR,"E2");
		  ...
		  $dbValue[10] = QuoteValue(DPE_CHAR,"E2");
