...
$sql = "SELECT b.reg_jenis_pasien,a.cust_usr_nama,b.id_dokter,a.cust_usr_id,c.pembayaran_id,b.id_poli, pembayaran_flag 
     from global.global_customer_user a 
     left join klinik.klinik_registrasi b on b.id_cust_usr = a.cust_usr_id  
     left join klinik.klinik_pembayaran c on c.pembayaran_id = b.id_pembayaran 
...
switch ($_GET['func']) {
     case 'store':
          ...
          $dbValue[14] = QuoteValue(DPE_CHAR, $_POST["keterangan"]);
          //print_r($dbValue);die();
          ...
          if ($a) {
               ...
               $rs['keterangan'] = $_POST["keterangan"];
          }
     ...
     case 'update':
          ...
          $dbValue[13] = QuoteValue(DPE_CHAR, $_POST["keterangan"]);
          ...
          if ($a) {
               ...
               $rs['aturan_pakai_nama'] = $AturanPakai['aturan_pakai_nama'];
               $rs['jam_aturan_pakai_nama'] = $JamAturanPakai['jam_aturan_pakai_nama'];
               $rs['keterangan'] = $JamAturanPakai["keterangan"];
          }
     ...
     default:
          $sql = "SELECT  rawat_item_id, id_item,keterangan, terapi_jumlah_item, terapi_dosis, id_aturan_minum, id_aturan_pakai, a.id_jam_aturan_pakai, a.item_nama, a.petunjuk_nama, a.aturan_minum_nama, a.aturan_pakai_nama, a.jam_aturan_pakai_nama from klinik.klinik_perawatan_terapi a";
...
if ($_GET['func'] == 'store') {
     ...
     $dbField[17] = "penjualan_detail_harga_beli";
       $dbField[18] = "penjualan_detail_ket";
     ...
     $dbValue[11] = QuoteValue(DPE_NUMERIC, StripCurrency($hargamargin));
     $dbValue[12] = QuoteValue(DPE_NUMERIC,StripCurrency($hargapokok + $PajakJumlah + $TuslagJumlah));
     ...
     $dbValue[17] = QuoteValue(DPE_NUMERIC, StripCurrency($HargaBeliJumlah));
      $dbValue[18] = QuoteValue(DPE_CHAR, $_POST["keterangan"]);
// print_r($dbValue);die();
     $dbKey[0] = 0; // -- set key buat clause wherenya , valuenya = index array buat field / value
     $dtmodel = new DataModel($dbTable, $dbField, $dbValue, $dbKey, DB_SCHEMA_GLOBAL);
     if($dtPasien['pembayaran_flag'] == 'y' || $dtPasien['pembayaran_flag'] == 'p'){
    ...
    //    $dtaccess->Execute($sql,DB_SCHEMA_LOGISTIK);
     ...
     $dbField[14] = "penjualan_detail_ket";
     ...
     $dbValue[10] = QuoteValue(DPE_NUMERIC, StripCurrency($hargamargin));
