...
<body>
	<div class="wrapper" style="width: 18cm; margin : auto;">
		...
		<?php if($dataTipeLayanan['anamnesa_isi_nilai'] == '0bstetri') { ?>
			<div>
				Keluhan Utama : <?=str_replace("\n", "<br>", $_GET['keluhanUtama'])?><br>
		...
		<div>
			...
			<?php if($fisik['Letak Anak'] || $_GET['kepala'] || $_GET['sungsang'] || $_GET['oblique'] || $_GET['lintang'] ) { 
				if($_GET['kepala']) $fisik['Letak Anak'] = 'Kepala';
				if($_GET['sungsang']) $fisik['Letak Anak'] = 'Sungsang';
				if($_GET['oblique']) $fisik['Letak Anak'] = 'Oblique';
				if($_GET['lintang']) $fisik['Letak Anak'] = 'Lintang';

				?> 
				Letak Anak : <?=$fisik['Letak Anak']?> <br>
			<?php } ?>
			<?php if($_GET['tinggi_fu']) { ?>Tinggi FU : <?=$_GET['tinggi_fu']?><br><?php } ?>
		...
		<?php if($_REQUEST['janin_tunggal'] || $_REQUEST['janin_kembar'] || $_REQUEST['janin_hidup'] || $_REQUEST['janin_iufd']) { ?>
			<div style="margin-top: -15px;">
				...
				<?= ($_REQUEST['janin_kembar'] == 'true') ? 'Kembar('.$_REQUEST['janin_kembar_det'].');' : '' ?>
	...
	<?php }?>
	<?php for($x=1; $x < $_REQUEST['janin_kembar_det']; $x++) { 
		if($_REQUEST['jenis_kelamin_'.$x]){
		?>
		<div>
			<div class="block">
				Kelamin &nbsp; &nbsp; &nbsp;:
				<?php
				if ($_REQUEST['jenis_kelamin_'.$x] == 'Laki') {
					echo "Laki - laki";
				}elseif ($_REQUEST['jenis_kelamin_'.$x] == 'Perempuan') {
					echo "Perempuan";
				}else{
					echo "&nbsp;";
				}
...
<?php endif;
?>
<?php if($_GET['USGTambahan']) { ?>
	<div>
		<div class="block">
			USG Tambahan&nbsp; &nbsp; &nbsp;:<br>
			<?php
			echo $_GET['USGTambahan'];
			?>
		</div>
	</div>
<?php } ?>
...
<div style="margin-top: 7px">
	...
	if($_REQUEST['janin_hidup'] == null || $_REQUEST['janin_iufd'] == null){
		if($_REQUEST['djj'] == '+'){
			$_GET['ket_diagnosa_tiga'] = "Hidup";
		}
		else if($_REQUEST['djj'] == '-'){
			$_GET['ket_diagnosa_tiga'] = "IUFD";
		}
		else{
			$_GET['ket_diagnosa_tiga'] = "";
		}
	}
	else if($_REQUEST['janin_hidup']){
		$_GET['ket_diagnosa_tiga'] = "Hidup";
	}
	else if($_REQUEST['janin_iufd']){
		$_GET['ket_diagnosa_tiga'] = "IUFD";
	}
	...
	<?=" / ".$_GET['ket_diagnosa_satu']?>  <?=($_GET['ket_diagnosa_dua'] == 'T') ? " / Tunggal" : ""?> <?=($_GET['ket_diagnosa_dua'] == 'G') ? " / Gemelli" : ""?> <?=" / ".$_GET['ket_diagnosa_tiga']?>  <?=" / ".$_GET['ket_diagnosa_lima']?>
...
<div>
	<ol style="margin: 0px">
		...
		?>
		<?php if($_GET['planning_penatalaksanaan']){ ?><li><?=$_GET['planning_penatalaksanaan']?></li><?php }?>
	</ol>
</div>
<?php
}
	...
	<div>
		...
		Tekanan Darah Sistole : <?=$_GET['tekanan_darah_sistole']?> mm/Hg <br>
		Tekanan Darah Diastole : <?=$_GET['tekanan_darah_diastole']?> mm/Hg <br>
