...
$sql .= " group by who_when_update order by who_when_update asc";
  $Kasir = $dtaccess->FetchAll($sql); // Pendapatan MEDIS

  if(!$_GET["kode"]){
    $sql = "select pembayaran_who_create
    ...
    $sql .= " where p.pembayaran_yg_dibayar!='0' and pembayaran_create >= ".QuoteValue(DPE_DATE, date_db($_GET['tgl_awal']).' '.$_GET['waktu_awal'])." and pembayaran_create <= ".QuoteValue(DPE_DATE, date_db($_GET['tgl_akhir']).' '.$_GET['waktu_akhir']);
    if ($_GET['usr_id'] != '--') $sql .= " and pembayaran_who_create = ".QuoteValue(DPE_CHAR, $_GET['usr_id']);
    $sql .= " group by pembayaran_who_create";
       // echo $sql;
    $rs = $dtaccess->Execute($sql);
    $Kasir2 = $dtaccess->FetchAll($rs);
  }
...
//if ($_GET["klinik"] && $_GET["klinik"] != "--") $where[] = "j.id_dep = " . QuoteValue(DPE_CHAR, $_GET["klinik"]);
        ...
        if ($_GET["kode"]) {
          $where[] = "c.cust_usr_kode = " . QuoteValue(DPE_CHAR, $_GET["kode"]);
        }
  ...
  //$sql .= " order by pembayaran_create desc, pembayaran_multipayment_create desc, id_reg desc";
      ...
      <tbody>
        ...
        // echo $sql;
          ...
          $sql = "select sum(pembayaran_det_dibayar) as bank, jbayar_nama from klinik.klinik_pembayaran_det a left join global.global_jenis_bayar b on b.jbayar_id = a.id_jbayar where id_pembayaran = ".QuoteValue(DPE_CHAR,$dataTable[$x]['pembayaran_id'])." and jbayar_id <> '01' and jbayar_id <> 'x'"." and pembayaran_det_create >= " . QuoteValue(DPE_DATE, date_db($_GET["tgl_awal"]).' '.$_GET['waktu_awal'])." and pembayaran_det_create <= " . QuoteValue(DPE_DATE, date_db($_GET["tgl_akhir"]).' '.$_GET['waktu_akhir'])." and pembayaran_det_kwitansi = ".QuoteValue(DPE_CHAR,$dataTable[$x]['pembayaran_det_kwitansi'])." group by jbayar_nama";
          ...
          $sql = "select sum(pembayaran_det_dibayar) as piutang_umum from klinik.klinik_pembayaran_det a left join global.global_jenis_bayar b on b.jbayar_id = a.id_jbayar where id_pembayaran = ".QuoteValue(DPE_CHAR,$dataTable[$x]['pembayaran_id'])." and id_jbayar = 'x'"." and pembayaran_det_create >= " . QuoteValue(DPE_DATE, date_db($_GET["tgl_awal"]).' '.$_GET['waktu_awal'])." and pembayaran_det_create <= " . QuoteValue(DPE_DATE, date_db($_GET["tgl_akhir"]).' '.$_GET['waktu_akhir'])." and pembayaran_det_kwitansi = ".QuoteValue(DPE_CHAR,$dataTable[$x]['pembayaran_det_kwitansi']);
          ...
          if ($pembayaran_det_kwitansi[$x-1] != $pembayaran_det_kwitansi[$x]): 
            ...
            <tr>
              ...
              <td align="right" width="5%"><?= str_replace(',', '.', currency_format($TotalPiutangUmum['piutang_umum'])) ?></td>
...
for ($i = 0; $i < count($Kasir2); $i++) {
  ...
  $sql .= " and pembayaran_who_create = '". $Kasir2[$i]['pembayaran_who_create'] ."'";
  ...
  <?php if (count($dataPenjualan) > 0) : ?>
    <?php if (($_GET['layanan'] == 'Lain' || $_GET['layanan'] == '--') && $_GET['cust_usr_jenis'] != '7' && $_GET['cust_usr_kode'] == '' && $_GET['id_poli'] == '--') { ?>
      <table class="table table-bordered" cellspacing="0" width="100%" border="1" style="margin-top:15px; font-size: 8px;">
        ...
        <tbody>
          <?php 
          for ($x = 0; $x < count($dataPenjualan); $x++) { 

            $sql = "select sum(pembayaran_det_total) as total from kasir.kasir_pembayaran_det where id_pembayaran = ".QuoteValue(DPE_CHAR,$dataPenjualan[$x]['pembayaran_id'])." and id_jbayar = '01'";
            $PembayaranTunai = $dtaccess->Fetch($sql);

            $sql = "select sum(pembayaran_det_total) as total from kasir.kasir_pembayaran_det where id_pembayaran = ".QuoteValue(DPE_CHAR,$dataPenjualan[$x]['pembayaran_id'])." and id_jbayar in(select jbayar_id from global.global_jenis_bayar where jbayar_id <> '01' and jbayar_id <> 'x')";
            $PembayaranBank = $dtaccess->Fetch($sql);

            $sql = "select sum(pembayaran_det_total) as total from kasir.kasir_pembayaran_det where id_pembayaran = ".QuoteValue(DPE_CHAR,$dataPenjualan[$x]['pembayaran_id'])." and id_jbayar = 'Disc'";
            ...
            <tr>
              <td><?php echo $x+1 ?></td>
              <td><?php echo $dataPenjualan[$x]['pembayaran_create'] ?></td>
              <td><?php echo $dataPenjualan[$x]['penjualan_nomor'] ?></td>
              <td><?php echo $dataPenjualan[$x]['cust_usr_nama'] ?></td>
              <td><?php echo $dataPenjualan[$x]['pembayaran_who_create'] ?></td>
        ...
        <tfoot>
          <tr>
            <td colspan="5">TOTAL <?php echo $Kasir2[$i]['pembayaran_who_create'] ?></td>
...
$FixTotalDiskon = $totalDiskonGlobal + $SubDiscPenjualan;
...
<table cellspacing="0" width="100%" class="table table-bordered" border="1" style="margin-top:15px; font-size: 8px;">
  ...
  <tbody>
    <tr>
      ...
      $where = array();
          //if ($_GET["klinik"] && $_GET["klinik"] != "--") $where[] = "j.id_dep = " . QuoteValue(DPE_CHAR, $_GET["klinik"]);
      if ($_GET["tgl_awal"]) $where[] = "j.pembayaran_det_tgl >= " . QuoteValue(DPE_DATE, date_db($_GET["tgl_awal"]).' '.$_GET['waktu_awal']);
      if ($_GET["tgl_akhir"]) $where[] = "j.pembayaran_det_tgl <= " . QuoteValue(DPE_DATE, date_db($_GET["tgl_akhir"]).' '.$_GET['waktu_akhir']);
      if ($_GET["js_biaya"]) $where[] = "a.pembayaran_jenis = " . QuoteValue(DPE_CHAR, $_GET["js_biaya"]);
      if ($_GET['jbayar'] == '2') {
        $sql_where[] = "j.id_jbayar in(select jbayar_id from global.global_jenis_bayar)";
      }elseif ($_GET['jbayar'] == '5') {
      ...
      left join global.global_auth_user i on i.usr_id = d.id_dokter
      left join global.global_jenis_bayar k on k.jbayar_id=j.id_jbayar";
      $sql .= " where 1=1 and j.id_jbayar in(select jbayar_id from global.global_jenis_bayar where jbayar_id <> '01' and jbayar_id <> 'x') and  " . $where;
      ...
      if(!$_GET['kode']) {
        ...
        where b.id_jbayar != '01' and jbayar_id <> 'x' ";
            ...
            <?php for ($i = 0; $i < count($dataBank); $i++) { ?>
              <tr>
                ...
                <td align="right">
                  ...
                  ?>
                  <?php echo str_replace(',', '.', currency_format($totalPerBank)) ?>
                </td>
              </tr>
            <?php } ?>
          <?php } ?>
      ...
      <td align="right"><?= ($_GET['btnExcel']) ? str_replace(',', '.', currency_format($FixTotalPiutangUmumGlobal)) : currency_format($FixTotalPiutangUmumGlobal); ?></td>
