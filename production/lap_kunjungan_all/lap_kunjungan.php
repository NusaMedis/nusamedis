...
if($_POST["reg_tipe_rawat"]&& $_POST["reg_tipe_rawat"]!="--"){
    ...
    //Pilih Poli
     ...
     if($_POST["btnLanjut"] || $_POST["btnCetak"]){
      $sql = "select b.cust_usr_kode, b.cust_usr_nama, b.cust_usr_alamat, b.cust_usr_tanggal_lahir, b.cust_usr_jenis_kelamin,f.dep_nama, 
               ...
               a.reg_icd, k.rawat_diagnosa_utama, k.rawat_who_insert_icd,l.kondisi_akhir_pasien_nama,y.*,z.*
    ...
    //echo $sql;
     ...
     $tableHeader = "&nbsp;Report Registrasi Pasien Harian";
     ...
     for($i=0,$counter=0,$n=count($dataTable);$i<$n;$i++,$counter=0){
		...
		//{
         ...
         $tbContent[$i][$counter][TABLE_ISI] = $i + 1;
          ...
          $tbContent[$i][$counter][TABLE_ISI] = str_replace("*", "'",  $dataTable[$i]["cust_usr_nama"]);
          $tbContent[$i][$counter][TABLE_ALIGN] = "left";          
          $counter++;
...
//  $dataPoli = $dtaccess->FetchAll($rs_edit);
  ...
  $sql = "select a.poli_nama, a.poli_id from global.global_auth_poli a join global.global_auth_user_poli b on a.poli_id = b.id_poli and b.id_usr =".QuoteValue(DPE_CHAR,$userId); 
  $rs_edit = $dtaccess->Execute($sql,DB_SCHEMA_GLOBAL);
  $dataPoli = $dtaccess->FetchAll($rs_edit);
				    ...
				    <div class="col-md-4 col-sm-6 col-xs-12">
                      <label class="control-label col-md-12 col-sm-12 col-xs-12">Tipe Rawat </label>
                        <select name="reg_tipe_rawat" class="select2_single form-control" id="reg_tipe_rawat" required="" >
                          <option value="--" <?php if ($_POST["reg_tipe_rawat"] == "--") echo "selected" ?>>Pilih Tipe Rawat</option>
                          <option value="J" <?php if ($_POST["reg_tipe_rawat"] == "J") echo "selected" ?>>Rawat Jalan</option>;
                          <option value="I" <?php if ($_POST["reg_tipe_rawat"] == "I") echo "selected" ?>>Rawat Inap</option>
...
<?php if($_x_mode=="cetak"){ ?>	
  window.open('report_pasien_cetak.php?tipe=<?php echo $_POST["reg_tipe_rawat"];?>&klinik=<?php echo $_POST["klinik"];?>&tgl_awal=<?php echo $_POST["tgl_awal"];?>&tgl_akhir=<?php echo $_POST["tgl_akhir"];?>&shift=<?php echo $_POST["shift"];?>&dokter=<?php echo $_POST["id_dokter"];?>&id_poli=<?php echo $_POST["id_poli"];?>&cetak=y', '_blank');
<?php } ?>

<?php if($_x_mode=="excel"){ ?> 
  window.open('report_pasien_cetak.php?tipe=<?php echo $_POST["reg_tipe_rawat"];?>&klinik=<?php echo $_POST["klinik"];?>&tgl_awal=<?php echo $_POST["tgl_awal"];?>&tgl_akhir=<?php echo $_POST["tgl_akhir"];?>&shift=<?php echo $_POST["shift"];?>&dokter=<?php echo $_POST["id_dokter"];?>&id_poli=<?php echo $_POST["id_poli"];?>&excel=y', '_blank');
<?php } ?>
