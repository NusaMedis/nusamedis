<?php
   ...
   if($_GET["tipe"]) $sql_where[] = " reg_tipe_rawat = ".QuoteValue(DPE_CHAR,$_GET["tipe"]);     
     ...
     a.reg_jenis_pasien,a.reg_tanggal_pulang,reg_waktu_pulang, a.reg_shift,a.reg_asal,a.reg_status_pasien,a.reg_status,u.kelas_nama, a.reg_kartu,a.reg_keterangan, a.reg_waktu, a.reg_tanggal,
     ...
     a.reg_icd, k.rawat_diagnosa_utama, k.rawat_who_insert_icd,l.kondisi_akhir_pasien_nama,o.pembayaran_flag,p.rawatinap_id,p.id_reg,p.rawatinap_asal_instalasi,q.rujukan_nama
    ,r.id_rawatinap,y.*,z.* from klinik.klinik_registrasi a 
     ...
     left join klinik.klinik_kelas u on a.reg_kelas = u.kelas_id 
    ...
    $sql.= " and reg_tipe_rawat='I' and reg_status != 'I9' and reg_utama is null and (c.poli_nama='Ranap Nifas' or c.poli_nama='Ranap Neo' or c.poli_nama='Ranap Anak' or c.poli_nama='Ranap RGT')  and cust_usr_kode<>'100' and (a.reg_batal is null or reg_batal='n') ";
  ...
  //  var_dump($dataTable[0]); die();
    ...
    left join global.global_auth_poli c on c.poli_id = a.id_poli";
    $sql .=" where ".implode(" and ",$sql_where);
   $sql.= " and cust_usr_jenis_kelamin = 'L' AND a.reg_status != 'I9'  and reg_utama is null and (c.poli_nama='Ranap Nifas' or c.poli_nama='Ranap Neo' or c.poli_nama='Ranap Anak' or c.poli_nama='Ranap RGT')  and cust_usr_kode<>'100' and (a.reg_batal is null or reg_batal='n')";
    ...
    left join global.global_auth_poli c on c.poli_id = a.id_poli";
    $sql .=" where ".implode(" and ",$sql_where);
   $sql.= "and cust_usr_jenis_kelamin = 'P'AND a.reg_status != 'I9'  and reg_utama is null and (c.poli_nama='Ranap Nifas' or c.poli_nama='Ranap Neo' or c.poli_nama='Ranap Anak' or c.poli_nama='Ranap RGT')  and cust_usr_kode<>'100' and (a.reg_batal is null or reg_batal='n')";
    ...
    left join global.global_auth_poli c on c.poli_id = a.id_poli";
    $sql .=" where ".implode(" and ",$sql_where);
   $sql.= "and reg_status_pasien = 'L' AND a.reg_status != 'I9'  and reg_utama is null and (c.poli_nama='Ranap Nifas' or c.poli_nama='Ranap Neo' or c.poli_nama='Ranap Anak' or c.poli_nama='Ranap RGT')  and cust_usr_kode<>'100' and (a.reg_batal is null or reg_batal='n')";
 ...
 $sql.= " and reg_status_pasien = 'B'AND a.reg_status != 'I9'  and reg_utama is null and (c.poli_nama='Ranap Nifas' or c.poli_nama='Ranap Neo' or c.poli_nama='Ranap Anak' or c.poli_nama='Ranap RGT')  and cust_usr_kode<>'100' and (a.reg_batal is null or reg_batal='n') and (c.poli_nama='Ranap Nifas' or c.poli_nama='Ranap Neo' or c.poli_nama='Ranap Anak' or c.poli_nama='Ranap RGT')  and cust_usr_kode<>'100' and (a.reg_batal is null or reg_batal='n')";
   ...
   //  var_dump($dataTable[0]); die();
    ...
    //  $counterHeader++;
     ...
     $tbHeader[0][$counterHeader][TABLE_ISI] = "Instalasi Asal";
     $tbHeader[0][$counterHeader][TABLE_WIDTH] = "5%";     
     $counterHeader++;
     ...
     $tbHeader[0][$counterHeader][TABLE_ISI] = "Kelas Keperawatan";
     $tbHeader[0][$counterHeader][TABLE_WIDTH] = "5%";     
     $counterHeader++;

     $tbHeader[0][$counterHeader][TABLE_ISI] = "Pindah Intern";
     $tbHeader[0][$counterHeader][TABLE_WIDTH] = "5%";     
     $counterHeader++;

     $tbHeader[0][$counterHeader][TABLE_ISI] = "Tanggal & jam keluar RS";
     $tbHeader[0][$counterHeader][TABLE_WIDTH] = "5%";     
     $counterHeader++;
...
//echo $sql;
      ...
      $sqlTf  = "select b.kelas_nama from klinik.klinik_registrasi a ";
    
      $sqlTf .= "left join klinik.klinik_kelas b on a.reg_kelas = b.kelas_id ";
      $sqlTf .= " where id_cust_usr = ".QuoteValue(DPE_CHAR,$dataTable[$i]["id_cust_usr"])." and reg_status='I9' order by reg_tanggal asc limit 1 ";
      $dataTf = $dtaccess->Fetch($sqlTf);
    // echo $sqlTf;
    //if($_POST["id_poli"] == '--') 
    //{
  ...
  //         $counter++;
      ...
      if ( $dataTable[$i]["rawatinap_asal_instalasi"]=="J") {
       ...
       else if ( $dataTable[$i]["rawatinap_asal_instalasi"]=="G") {
        # code...
      ...
      $tbContent[$i][$counter][TABLE_ISI] = $dataTf["kelas_nama"];
      $tbContent[$i][$counter][TABLE_ALIGN] = "left";          
      $counter++;      

      $tbContent[$i][$counter][TABLE_ISI] = $dataTable[$i]["reg_tanggal_pulang"]." ".$dataTable[$i]["reg_waktu_pulang"];
      $tbContent[$i][$counter][TABLE_ALIGN] = "left";          
      $counter++;      
...
<table width="100%" border="1" cellpadding="1" cellspacing="0" style="border-collapse:collapse">
 ...
 <?php   # code...
          ...
          header('Content-Disposition: attachment; filename=laporan_kunjungan_pasien_irna.xls');?>
