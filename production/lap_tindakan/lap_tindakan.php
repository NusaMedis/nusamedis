...
if ($_POST["btnLanjut"] || $_POST["btnExcel"] || $_POST["btnCetak"]) {
  $sql = "select a.*,  c.cust_usr_nama, c.cust_usr_kode, c.cust_usr_umur, c.cust_usr_tanggal_lahir, c.cust_usr_alamat, c.cust_usr_penanggung_jawab, b.biaya_nama,              
             ...
             j.poli_nama, k.tipe_biaya_nama, l.shift_nama, m.usr_name as ptg_entri, n.is_cito ,d.reg_who_update, x.pembayaran_det_id      
  ...
  $sql .= " left join klinik.klinik_pembayaran_det x on x.pembayaran_det_id = a.id_pembayaran_det";
...
$tableHeader = "Rawat Jalan-Laporan Tindakan";
if ($_POST["btnExcel"]) {
                  ...
                  <div class="x_content">
                    <form name="frmView" action="<?php echo $_SERVER["PHP_SELF"] ?>" method="POST">
                      ...
                      <div class="col-md-4 col-sm-6 col-xs-12">
                        ...
                        <select class="select2_single form-control" name="id_dokter" id="id_dokter" onKeyDown="return tabOnEnter(this, event);">
                          ...
                          <?php for ($i = 0, $n = count($dataDokter); $i < $n; $i++) { ?>
                            <option <?php echo ($_POST['id_dokter'] == $dataDokter[$i]["usr_id"]) ? 'selected' : ''; ?> value="<?php echo $dataDokter[$i]["usr_id"]; ?>"><?php echo $dataDokter[$i]["usr_name"]; ?></option>
                          <?php } ?>
                        </select>
                      </div>
                      ...
                      <div class="col-md-4 col-sm-6 col-xs-12">
                        ...
                        <select class="select2_single form-control" name="status" onKeyDown="return tabOnEnter(this, event);">
                          <option value="">[ Pilih Status ]</option>
                          <option value="y" <?php if ($_POST['status'] == 'y') echo "selected"; ?>> Sudah Posting </option>
                          <option value="n" <?php if ($_POST['status'] == 'n') echo "selected"; ?>> Belum Posting </option>
                        </select>
                  ...
                  <table id="table" width="100%" class="table table-striped table-bordered" border="1" align="left" cellpadding="1" cellspacing="1">
                    <thead>
                      <tr>
                        ...
                        <td align="center" width="10%">Dokter</td>
                        <td align="center" width="10%">Pelaksana</td>
                        <td align="center" width="10%">Klinik</td>
                        <td align="center" width="10%">Status Pasien</td>
                      </tr>
                    </thead>
                    <tbody>
                      ...
                      ?>
                        <tr>
                          ...
                          <td><?= $pelaksana["usr_name"] ?></td>
                          <td><?= $dataTable[$i]["poli_nama"] ?></td>
                          <?php if ($dataTable[$i]["id_pembayaran"] != $dataTable[$i - 1]["id_pembayaran"]) { ?>
                            <td rowspan="<?= $dataSpan["jml_span"] ?>"><?= ($dataTable[$i]["pembayaran_det_id"]) ? "Sudah Diposting" : "Belum Diposting"?></td>
                          <?php } ?>
                        </tr>
                      <?php } ?>
