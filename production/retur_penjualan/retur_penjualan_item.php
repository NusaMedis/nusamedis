<?php
     ...
     $poli = "33"; //POLI APOTIK IRJ DIPATEN DULU SEMENTARA
...
//   echo $totalHarga;
 //  $kembali = "retur_penjualan_view.php?kembali=".$_POST["klinik"];
   ...
   if ($_x_mode == "New" && !$returId)  //Jika menyimpan penjualan
    ...
    //hapus yang kosong jumlah returnya
    $sql = "select retur_penjualan_detail_id, id_penjualan_retur, id_item from logistik.logistik_retur_penjualan_detail
    		where retur_penjualan_detail_jumlah='0'";
    $rs = $dtaccess->Execute($sql);
    $dataReturKosong = $dtaccess->FetchAll($rs);

    for($i=0,$n=count($dataReturKosong);$i<$n;$i++){
    	
    	$sql = "delete from logistik.logistik_retur_penjualan_detail where retur_penjualan_detail_id = ".QuoteValue(DPE_CHAR,$dataReturKosong[$i]["retur_penjualan_detail_id"]);
    	$rs = $dtaccess->Execute($sql);
 ...
 //      echo $sql;                 
        ...
        //        $dbValue[8] = QuoteValue(DPE_CHAR,'9999998'); //DIPATEN KLINIK BIAYA NYA kasus ini
        //        $dbValue[9] = QuoteValue(DPE_CHAR,$poli);
        //        $dbValue[10] = QuoteValue(DPE_NUMERIC,$dataTotal["reg_jenis_pasien"]);
        ...
        $sql = "update logistik.logistik_retur_penjualan set retur_penjualan_total = ".StripCurrency($dataTotal["grandtotal_retur"]).", id_fol = '".$folId."' where retur_penjualan_id =".QuoteValue(DPE_CHAR,$_POST["retur_penjualan_id"]);
        $rs = $dtaccess->Execute($sql);
...
function SendHrg(terima,urut) {    
     ...
     hmargin = SendItem * Harga;
     duit = Harga * 1.1; 
     tuslag = (5/100 * duit) * SendItem;
     ppn = hmargin * 0.1;
       document.getElementById('totalHargaTerima'+urut).value = formatCurrency(parseInt(hmargin + ppn + parseInt(tuslag)));
       TotalFunc=parseInt(hmargin + ppn + parseInt(tuslag));
...
<?php if($_x_mode=="cetak"){ ?>
         BukaWindow('retur_cetak.php?id=<?php echo $returId;?>','Nota');
  	       document.location.href='retur_penjualan_view.php?kembali=<? echo $_POST["klinik"];?>';
    <?php } ?>
</script>                      
  ...
  <body class="nav-md">
    <div class="container body">
          ...
          <div class="">
            ...
            <div class="row">
              ...
              <div class="col-md-12 col-sm-12 col-xs-12">
                <div class="x_panel">
                  <div class="x_title">
                    <h2>Retur Penjualan Obat</h2>
                    <div class="clearfix"></div>
                  </div>
                  <div class="x_content">
					<form name="frmEdit" id="demo-form2" method="POST" class="form-horizontal form-label-left" action="<?php echo $_SERVER["PHP_SELF"]?>">  
                      <div class="form-group">
                        <label class="control-label col-md-3 col-sm-3 col-xs-12" for="first-name">No Retur <span class="required">*</span>
                        </label>
                    ...
                    <td align="center" width="47%" class="tablecontent" rowspan="6"><font color='red' size='10'><span id=txtIsi ><?php echo currency_format($totalHarga);?></span></font></td>      
                      ...
                      <div class="form-group">
                        <label class="control-label col-md-3 col-sm-3 col-xs-12">No Nota</label>
                        <div class="col-md-4 col-sm-4 col-xs-12">
                          <a href="<?php echo $findNota;?>?TB_iframe=true&height=400&width=450&modal=true" class="thickbox" title="Pilih Nota">
            						  <?php echo $view->RenderTextBox("no_nota","no_nota","30","30",$_POST["no_nota"],"inputField", null,false);?><a href="<?php echo $findNota;?>?TB_iframe=true&height=400&width=450&modal=true" class="thickbox" title="Pilih Nota"><img src="<?php echo($ROOT);?>gambar/finder.png" border="0" align="top" class="tombol" style="cursor:pointer" title="Cari Nota" alt="Cari Nota" /></a>
                      ...
                      <div class="form-group">
                        <div class="col-md-6 col-sm-6 col-xs-12 col-md-offset-3">
                           <input type="submit" name="btnTampil" id="btnTampil" value="Tampilkan Obat" class="submit" onClick="javascript:return CekObat();">
							<input type="button" name="simpan" id="simpan" value="Kembali" class="submit" onClick="document.location.href='retur_penjualan_view.php'";/>
						</div>
                      </div>
     ...
     <tr>
                    ...
                    <td align="left" width="10%" class="subheader">&nbsp;Jml yg di Retur</td>
