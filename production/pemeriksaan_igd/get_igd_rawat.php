...
//Tanggal Kemarin
$tglKemarin = date('Y-m-d', strtotime("-2 day", strtotime(date("Y-m-d"))));
...
if (isset($_GET['reg_id'])) {
  $sql = "select a.reg_diagnosa_igd,
    ...
    h.rawat_anamnesa,h.rawat_keluhan,h.rawat_catatan,h.rawat_pemeriksaan_fisik,h.rawat_diagnosa_utama,h.rawat_ket, i.perusahaan_nama, a.id_jenis_kb, c.form_asmed,j.pembayaran_flag, a.id_dokter_2
    from
  ...
  $id_cust_usr = $dataTable[0]['id_cust_usr'];

  $sql="SELECT reg_id,b.id_pembayaran from klinik.klinik_inacbg a 
  left join klinik.klinik_registrasi b on a.id_reg = b.reg_id
  where b.id_cust_usr = '$id_cust_usr' and reg_tipe_rawat='I' 
  and reg_when_update > (select reg_when_update from klinik.klinik_registrasi where reg_id = '$_GET[reg_id]')
  order by reg_when_update desc limit 1";
  $regIrna = $dtaccess->Fetch($sql);

  $dataTable[0]['id_pembayaran'] = $regIrna['id_pembayaran'];
  $dataTable[0]['reg_id'] = $regIrna['reg_id'];
...
$Obat = '';
  ...
  $sql = "select a.reg_kode_trans,a.reg_id,a.reg_tanggal,a.reg_waktu,a.reg_status,
    ...
    ";
  $sql .= " where a.reg_status_kondisi = '2' and c.poli_tipe='G' and c.poli_id = '10'";
  $sql .= $kondisi;
