...
function tbui(ok) {
  $.getJSON('get_obgyn.php', {rawat_id: ok },function(rs) {
    ...
    if (rs.Amenore = 'y') {$('#amenore_ya').attr('checked', true);}
    ...
    if (rs.janin_tunggal == 'true') {$('#janin_tunggala').attr('checked', true)};
    if (rs.janin_kembar == 'true') {$('#janin_kembara').attr('checked', true)};
