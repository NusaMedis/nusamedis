...
if ($konf["conf_apotik_harga_otomatis_margin"] == 'y')  //jika harga dari margin maka dari perhitungan jika tidak ambil dari db
  {
    ...
    $ppn = intval(0.1 * $Item["item_harga_jual"]);
     ...
     $HargaJumlah = StripCurrency($hargajual) * StripCurrency($_POST["dosis"]);
     $PajakJumlah = StripCurrency(intval(($hargamargin * $_POST["dosis"]) * 0.1));
     $HargaBeliJumlah = StripCurrency($Item['item_hpp']) * StripCurrency($_POST["dosis"]);
     $TuslagJumlah = StripCurrency(intval($konfig * $HargaJumlah));
     $hargapokok = StripCurrency(intval($_POST["dosis"] * $hargamargin));
     $totalAll = intval($HargaJumlah + $TuslagJumlah);
...
$dbValue[8] = QuoteValue(DPE_CHAR, $hargajual);
