...
<script type="text/javascript">
	...
	function get_harga(index, record) {
		...
		var ed2 = $('#dg10').edatagrid('getEditor', {
			index: rowIndex,
			field: 'biaya'
		});
		var ed3 = $('#dg10').edatagrid('getEditor', {
			index: rowIndex,
			field: 'satuan_nama'
		});
	...
	function get_harga_bhp(index, record) {
		...
		var ed2 = $('#tb_bhp').edatagrid('getEditor', {
			index: rowIndex,
			field: 'satuan_nama'
		});
	...
	function getDataAlkes(index, record) {
		...
		var ed2 = $('#tb_alkes').edatagrid('getEditor', {
			index: rowIndex,
			field: 'alkes_kode'
		});
		var ed3 = $('#tb_alkes').edatagrid('getEditor', {
			index: rowIndex,
			field: 'alkes_merk'
		});
					...
					<form method="POST" id="form_pemeriksaan" action="proses_registrasi.php">
						...
						<div class="row">
							...
							">
										...
										<div class="col-md-6 col-sm-6 col-xs-12">
											<input id="dokter2" name="dokter_2" class="easyui-combobox" style="width:100%;" data-options="
											url: 'get_dokterPelaksana.php',
											valueField: 'usr_id',
											textField: 'usr_name',
											label: 'Dokter II:',
											...
											var reg_id = $('#regId').val(); 
											if(reg_id != ''){
												$.post('update_dokter2.php',{usr_id:usr_id, reg_id:reg_id },function(result){
											if (result.success){
											//alert(result.success);
										}
									},'json');
...
required:true
	...
	">Satuan</th>
						...
						<!--Alkes-->
						<div class="clearfix"><br></div>
						<table id="tb_alkes" title="Sewa Alkes" style="width:100%;" toolbar="#toolbar_alkes" idField="pemakaian_alkes_id" rownumbers="true" fitColumns="true" singleSelect="true">
							<thead>
								<tr>
...
">Status</th>
	...
	<script type="text/javascript">
			...
			if (row) {
				if (row.reg_status == 'G0') {
					...
					$.get('get_igd.php', {
						...
						$('#dokter2').combobox('setValue', result[0].id_dokter2);
		...
		function cetak_tindakan_rad() {
			...
			var dokter = $('input[name=dokter]').val();
			if (regIdRad) {
				var url = '../pemeriksaan_irj/cetak_lab_tindakan.php?id_reg=' + regIdRad + '&nmps=' + $('#nmps').val() + '&norm=' + $('#norm').val() + '&dokter=' + dokter;
				var printWindow = window.open(url);
				printWindow.addEventListener('load', function() {}, true);
			}
		}
