...
<table id="opname" class="table table-striped table-bordered" border="0" cellpadding="0" cellspacing="0">
  ...
  <tr>
    ...
    <td><?=number_format($awal*$dataTable[$i]["item_hpp"], 0, ',', '.')?></td>

    <?php
    $hpp = $awal*$dataTable[$i]["item_hpp"];
    $opname += $dataTable[$i]["stok_item_saldo"];
    $hppOpname += $hpp;
    ?>

    <td><?=number_format($dataTable[$i]["stok_item_saldo"], 2, ',', '.')?></td>
    <td><?=number_format($dataTable[$i]["stok_item_saldo"]*$dataTable[$i]["item_hpp"], 0, ',', '.')?></td>

    <td><?=number_format($selisih, 2, ',', '.')?></td>
    <td><?=number_format($dataTable[$i]["item_hpp"]*$selisih, 0, ',', '.')?></td>
  </tr>
    <?php
    $hpp_akhir = $dataTable[$i]["stok_item_saldo"]*$dataTable[$i]["item_hpp"];
    $hpp_selisih = $dataTable[$i]["item_hpp"]*$selisih;
