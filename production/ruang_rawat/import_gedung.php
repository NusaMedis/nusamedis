<?php
    ...
    if(isset($_POST["btnImport"])){
      ...
      if ((!$myFile = @fopen(stripslashes($csvFile), "r")) || $err==true) {
        ...
        while ($data = fgetcsv($myFile, 500000, $delimiter))  {
          if($data[0]) {
            $dbTable = "temp.global_gedung_rawat";

            $dbField[] = "gedung_rawat_id";   // PK
            $dbField[] = "gedung_rawat_kode";
            $dbField[] = "gedung_rawat_nama";
            $dbField[] = "gedung_lantai_ke";
        ...
        $sql = "SELECT * FROM temp.global_gedung_rawat ORDER BY gedung_rawat_nama ASC";
  ...
  /* KLIK APPROVE */
    if(isset($_POST["btnApprove"])){
      $sql = "INSERT INTO global.global_gedung_rawat SELECT * FROM temp.global_gedung_rawat WHERE UPPER(gedung_rawat_nama) NOT IN (SELECT UPPER(gedung_rawat_nama) FROM global.global_gedung_rawat)";
      $dtaccess->Execute($sql);

      $sql = "DELETE FROM temp.global_gedung_rawat";
      $dtaccess->Execute($sql);

      header("Location: ruang_rawat_view.php");
      exit();
    }
  /* KLIK APPROVE */
?>
                      ...
                      <div class="x_content">
                            ...
                            <table class="table table-bordered">
                              ...
                              <tr>
                                <td colspan="2" align="right">
                                  ...
                                  <a href="ruang_rawat_view.php" class="btn btn-danger">Kembali</a>
                                </td>
                              </tr>
                            </table>
                          </form>
                        ...
                        <!-- Syarat Import -->
                          <font size="2" color="red">
                            ...
                            &nbsp;&nbsp;Kolom 1 &nbsp;&nbsp;: Kode Gedung Rawat <br />
                            &nbsp;&nbsp;Kolom 2 &nbsp;&nbsp;: Nama Gedung Rawat <br />
                            &nbsp;&nbsp;Kolom 3 &nbsp;&nbsp;: Gedung Lantai Ke <br />
                            &nbsp;&nbsp;Untuk Contoh data silahkan klik 
                            <a href="import_gedung.csv">disini</a> <br>
                          </font>
                        <!-- Syarat Import -->
                      </div>
                    <!-- Sebelum Import -->
                    <?php if ($_POST['btnImport']) { ?>
                      <div class="x_content">
                        <table class="table table-bordered">
                          <tr>
                            <td>No</td>
                            <td>Kode Gedung Rawat</td>
                            <td>Nama Gedung Rawat</td>
                            <td>Gedung Lantai Ke</td>
                          </tr>
                          <?php if($dataTable) : ?>
                            <?php foreach($dataTable as $key => $value) : ?>
                              <tr>
                                <td><?= $key+1 ?></td>
                                <td><?= $value['gedung_rawat_kode'] ?></td>
                                <td><?= $value['gedung_rawat_nama'] ?></td>
                                <td><?= $value['gedung_lantai_ke'] ?></td>
                              </tr>
                            <?php endforeach; ?>
                          <?php endif; ?>
                        </table>
