<?php
  ...
  /* Pengecekan Tindakan Paket */
  $sql = "select a.*,b.*,c.* from global.global_detail_paket a left join klinik.klinik_biaya_tarif b on a.id_biaya_tarif = b.biaya_tarif_id left join klinik.klinik_biaya c on b.id_biaya = c.biaya_id where b.biaya_tarif_tgl_awal <= '$skr' and b.biaya_tarif_tgl_akhir >= '$skr' and a.id_paket = ".QuoteValue(DPE_CHAR,$_POST["paket"]);
  $dataTarifPaket = $dtaccess->FetchAll($sql);
  /* Pengecekan Tindakan Paket */

  for($a=0,$b=count($dataTarifPaket);$a<$b;$a++) {      
    /* INSERT TINDAKAN PAKET */       
    ...
    if($_POST["reg_jenis_pasien"]=="5" || $_POST["reg_jenis_pasien"]=="7" || $_POST["reg_jenis_pasien"]=="18" || $_POST["reg_jenis_pasien"]=='26') $dbValue[17] = QuoteValue(DPE_NUMERIC,StripCurrency(0));
    else $dbValue[17] = QuoteValue(DPE_NUMERIC,StripCurrency($dataTarifPaket[$a]["biaya_total"]));
    
    if($_POST["reg_jenis_pasien"]=="5" || $_POST["reg_jenis_pasien"]=="7" || $_POST["reg_jenis_pasien"]=="18" || $_POST["reg_jenis_pasien"]=='26') $dbValue[18] = QuoteValue(DPE_NUMERIC,StripCurrency($dataTarifPaket[$a]["biaya_total"]));
    ...
    /* Pengecekan Split Tindakan */
		$sql = "select * from  klinik.klinik_biaya_split where id_biaya = ".QuoteValue(DPE_CHAR,$dataTarifPaket[$a]["biaya_tarif_id"])." and bea_split_nominal > 0";
    ...
    /* Pengecekan Split Tindakan */
      ...
      /* INSERT SPLIT */
			...
			$dbField[1] = "id_fol";
			$dbField[2] = "id_split";
			if($_POST["reg_jenis_pasien"]=='6') $dbField[3] = "folsplit_nominal";
  		else $dbField[3] = "folsplit_nominal";
      	  
			$dbValue[0] = QuoteValue(DPE_CHAR,$dtaccess->GetTransID());
			$dbValue[1] = QuoteValue(DPE_CHAR,$folId2);
			$dbValue[2] = QuoteValue(DPE_CHAR,$dataSplitKarcis2[$i]["id_split"]);
			if($_POST["reg_jenis_pasien"]=='6') $dbValue[3] = QuoteValue(DPE_NUMERIC,'0.00');
