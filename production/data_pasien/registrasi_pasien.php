...
/* Pengecekan Pasien Masih Ada Penunjang Yang Belum Selesai */
$sql = "select count(reg_id) as jkn_irj, string_agg(poli_nama, ', ') from klinik.klinik_registrasi a left join global.global_auth_poli b on a.id_poli = b.poli_id  WHERE id_cust_usr = " . QuoteValue(DPE_CHAR, $_GET['usr_id']) . " and reg_tanggal='" . date('Y-m-d') . "' and  (reg_status = 'E0' or reg_status = 'E1' or reg_status = 'R0' or reg_status = 'R1') and (b.poli_tipe ='L' or b.poli_tipe ='R')";
$kunjungan = $dtaccess->Fetch($sql);


/* Pengecekan Pasien Masih Ada Pemeriksaan Yang Belum Selesai */
$sql = "select count(reg_id) as jkn_irj, string_agg(poli_nama, ', ') from klinik.klinik_registrasi a left join global.global_auth_poli b on a.id_poli = b.poli_id  WHERE id_cust_usr = " . QuoteValue(DPE_CHAR, $_GET['usr_id']) . " and reg_tanggal='" . date('Y-m-d') . "' and (reg_tipe_rawat = 'J' or reg_tipe_rawat = 'G') and (reg_status = 'E1' or reg_status = 'G1' or reg_status = 'G0') and (b.poli_tipe !='L' and b.poli_tipe !='R' and b.poli_tipe !='A')";
...
/* Pengecekan Pasien Masih Menginap */
$sql = "select count(reg_id) as jkn_irna from klinik.klinik_registrasi a WHERE id_cust_usr = " . QuoteValue(DPE_CHAR, $_GET['usr_id']) . " and reg_status = 'I3' and reg_tanggal <= '" . date('Y-m-d') . "' and reg_tipe_rawat = 'I'  ";
$pas_rencana_pulang = $dtaccess->Fetch($sql);
/* Pengecekan Pasien Masih Menginap */

/* Pengecekan Pasien Belum Verif */

  # code...
$sql = "select count(id_reg) as jumlah from klinik.klinik_registrasi_buffer a WHERE id_cust_usr = " . QuoteValue(DPE_CHAR, $_GET['usr_id']) . " and reg_buffer_batal = 'n' and is_daftar = 'n' and reg_buffer_tanggal = '" . date('Y-m-d') . "'  ";
  // echo $sql;
$pas_belum_verif = $dtaccess->Fetch($sql);

if ($_GET['lanjut']) {
  # code...
  $sql="update klinik.klinik_registrasi_buffer set reg_buffer_batal='y',is_daftar='k' where id_cust_usr = " . QuoteValue(DPE_CHAR, $_GET['usr_id'])." and reg_buffer_tanggal = '" . date('Y-m-d') . "'  ";
...
if ($_POST["btnUpdate"]) {
  ...
  $_POST['cust_usr_nama'] = str_replace("'", "*", $_POST['cust_usr_nama']);
  $_POST['cust_usr_penanggung_jawab'] = str_replace("'", "*", $_POST['cust_usr_penanggung_jawab']);
  ...
  $sql="select count(cust_usr_kode) as rmsama from global.global_customer_user where cust_usr_kode = ". QuoteValue(DPE_CHAR,  $_POST["cust_usr_kode"]);
  $rmSama = $dtaccess->Fetch($sql);
  if($rmSama['rmsama']>1){?>
   <script type="text/javascript">
    alert('Pasien memiliki  No RM sama');
    location.href = '<?= $backPage ?>';
  </script>
<?php }
/* Pengecekan Pasien Nama dan Tanggal Lahir Sama */
...
$banyak = ($konf['dep_konf_reg_ulang'] == 'y') ? '0' : '100';
  if (!$sameday_irj['sameday'] > $banyak || !$lunas['lunas'] > 0) { // Tidak Ada Masalah
    ...
    if ($_POST['reg_status_pasien'] == "B" || $_POST['reg_status_pasien'] == "L") {
      ...
      $sql = "select * from global.global_lokasi where lokasi_kode = ".QuoteValue(DPE_CHAR, $_POST['id_prop'].'.'.$kota[0].'.'.$kec[0].'.'.$kel);
      ...
      $dbField[31] = "cust_usr_alergi";
      $dbField[32] = "cust_usr_kode_tampilan";
      $dbField[33]="cust_usr_tanggal_lahir_penanggung_jawab";
      $dbField[34] = "cust_usr_umur_penanggung_jawab";
      $dbField[35]="cust_usr_penanggung_jawab_pekerjaan";
      $dbField[36]="cust_usr_penanggung_jawab_pendidikan";
      $dbField[37]="cust_usr_penanggung_jawab_no_hp";
      ...
      $dbValue[31] = QuoteValue(DPE_CHAR, $_POST["alergi"]);

      $dbValue[32] = QuoteValue(DPE_CHAR, $_POST["cust_usr_kode_tampilan"]);
      $dbValue[33] = QuoteValue(DPE_DATE, date_db($_POST["cust_usr_tanggal_lahir_penanggung_jawab"]));
      $dbValue[34] = QuoteValue(DPE_CHAR, $_POST["tahun_penanggung_jawab"] . "~" . $_POST["bulan_penanggung_jawab"] . "~" . $_POST["hari_penanggung_jawab"]);
      $dbValue[35] = QuoteValue(DPE_CHAR, $_POST["cust_usr_penanggung_jawab_pekerjaan"]);
      $dbValue[36] = QuoteValue(DPE_CHAR, $_POST["cust_usr_penanggung_jawab_pendidikan"]);
      $dbValue[37] = QuoteValue(DPE_CHAR, $_POST["cust_usr_penanggung_jawab_no_hp"]);
    ...
    if ($_POST['reg_jenis_pasien'] == '5') {
      ?>
      <script type='text/javascript'>
        window.location.replace('../sep/create.php?reg_id=<?= $regId ?>&instalasi=<?= $_POST["instalasi"]?>');
      </script>
      <?php
    } else {
...
// $sql = "select * from global.global_auth_poli_tipe where (poli_tipe_id='M' or poli_tipe_id='G' or poli_tipe_id='J' or poli_tipe_id='I') order by poli_tipe_nama asc";

$sql = "select * from global.global_auth_poli_tipe where (poli_tipe_id='M' or poli_tipe_id='G' or poli_tipe_id='J') order by poli_tipe_nama asc";
...
if ($_GET['usr_id']) {
  ...
  $usr_kode =substr( $row['cust_usr_kode'],0,2);
  // $arr = str_split($usr_kode, "2");
  $usr_kode_tampilan = $usr_kode.".".substr( $row['cust_usr_kode'],2);
  ...
  $birthdayPJ = $row['cust_usr_tanggal_lahir_penanggung_jawab'];
  $todayPJ = new DateTime();
  $bidayPJ = new DateTime($birthdayPJ);
  $diffPJ = $todayPJ->diff($bidayPJ);

  $tahun_penanggung_jawab = $diffPJ->y;
  $bulan_penanggung_jawab = $diffPJ->m;
  $hari_penanggung_jawab = $diffPJ->d;
  // echo $birthdayPJ."-". $tahun_penanggung_jawab;
  /* Menentukan Tanggal Lahir */
  ...
  $now=date("Y-m-d");
  $sql = "select * from klinik.klinik_registrasi a left join global.global_auth_poli b on b.poli_id = a.id_poli where reg_utama is null and id_cust_usr = ".QuoteValue(DPE_CHAR, $_GET['usr_id'])." and a.id_pembayaran is not null order by reg_tanggal desc, reg_waktu desc";
      ...
      $.post("get_pasien.php", {
        ...
        function(data) {
          ...
          if (data.id_prop != null) {
            pilih_kota(isiprop,isikota,isiprop);
            if(isikota != null){
              pilih_kecamatan(isikota,isikota,isiprop,isikec);
              if(isikec != null){
                pilih_kelurahan(isikec,isiprop,isikota,isikec);
              }
            }
          } else {
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    ...
    function getProsedurMasuk(val) {
      ...
      if (prosedur_id) {
        ...
        if (prosedur_id=='3' && (instalasiId=='I' || instalasiId=='G')) {
          $("div#isi_lainnya_").css("display", "inline-block");

          $('input:text').attr('isi_lainnya','Isi Nama Puskesmas');

        }
        else if (prosedur_id=='1' && instalasiId=='G') {
          $("div#isi_lainnya_").css("display", "inline-block");

          $('input:text').attr('isi_lainnya','Isi Nama Dokter');

        }

        else if (prosedur_id=='4' && (instalasiId=='I' || instalasiId=='G')) {
          $("div#isi_lainnya_").css("display", "inline-block");
          // document.getElementById("isi_lainnya").placeholder = "Isi Nama Rumah Sakit";
          $('input:text').attr('isi_lainnya','Isi Nama Rumah Sakit');

        }
        else if (prosedur_id=='5' && (instalasiId=='I' || instalasiId=='G')) {
          $("div#isi_lainnya_").css("display", "inline-block");
          // document.getElementById("isi_lainnya").placeholder = "Isi Nama Bidan";
          $('input:text').attr('isi_lainnya','Isi Nama Bidan');


        }
        else if (prosedur_id=='6' && (instalasiId=='I' || instalasiId=='G')) {
          $("div#isi_lainnya_").css("display", "inline-block");
          // document.getElementById("isi_lainnya").placeholder = "Isi Nama Faskes 1";
          $('input:text').attr('isi_lainnya','Isi Nama Faskes 1');


        }
        else{
          $("div#isi_lainnya_").css("display", "none");
    ...
    function getRujukanDet(val) {
      ...
      // alert(rujukan_id);
        // if (rujukan_id=='2') {
        //  alert(rujukan_id);
        //  $("div#dokter_2").css("display", "inline-block");

        // }
        if (rujukan_id=='3' &&  (instalasiId=='I' || instalasiId=='G')) {
          $("div#isi_lainnya_").css("display", "inline-block");
          document.getElementById("isi_lainnya").placeholder = "Isi Nama Dokter";

        }
        else if (rujukan_id=='7df89720d264548b1b4a21c480e9d92f' &&  (instalasiId=='I' || instalasiId=='G')) {
          $("div#isi_lainnya_").css("display", "inline-block");
          document.getElementById("isi_lainnya").placeholder = "Isi disini";
        }

        else if (rujukan_id=='9' &&  (instalasiId=='I' || instalasiId=='G')) {
          $("div#isi_lainnya_").css("display", "inline-block");
          document.getElementById("isi_lainnya").placeholder = "Isi Nama Poli Klinik";
        }
        else if (rujukan_id=='4' &&  (instalasiId=='I' || instalasiId=='G')) {
          $("div#isi_lainnya_").css("display", "inline-block");
          document.getElementById("isi_lainnya").placeholder = "Isi Nama Puskesmas";
        }
        else if (rujukan_id=='5' &&  (instalasiId=='I' || instalasiId=='G')) {
          $("div#isi_lainnya_").css("display", "inline-block");
          document.getElementById("isi_lainnya").placeholder = "Isi Nama RS";
        }
    ...
    function funLocation(isi) {
      var prop = isi.substr(0, 2);
      var kota = isi.substr(2, 2);
      var kecamatan = isi.substr(4, 2);
      var isikota = kota;
      var isiprop = prop;

      $('#id_prop').val(prop);
      if (prop != null) {
        pilih_kota(prop,kota,prop);
        if(kota != null){
          pilih_kecamatan(kota,kota,prop,kecamatan);
          if(kecamatan != null){
            pilih_kelurahan(kecamatan,prop,kota,kecamatan);
          }
        }
      } else {
        pilih_kota(<?= $konf['dep_kode_prop'] ?>);
  ...
  <script>
    ...
    function Umur_penanggung_jawab(umur) {
      tgllahir = umur;
      tanggal = tgllahir.split("-");
      ...
      document.getElementById("tahun_penanggung_jawab").value = Math.floor(umurtahun);
      document.getElementById("bulan_penanggung_jawab").value = Math.floor(umurbulan);
      document.getElementById("hari_penanggung_jawab").value = Math.floor(umurhari);
    }

    function TanggalLahir_penanggung_jawab(tanggal) {
      umur = document.getElementById("tahun_penanggung_jawab").value;
      ...
      thn = skr - umur;
      var tahunlahir = thn;
      document.getElementById("cust_usr_tanggal_lahir_penanggung_jawab").value = "01-01-" + Math.floor(tahunlahir);
    }
  ...
  <div class="container body">
    <div class="main_container">
      ...
      <form id="form_utama" method="POST" class="form-horizontal form-label-left" action="<?php echo $_SERVER["PHP_SELF"] ?>">
        ...
        <input type="hidden" name="cust_usr_kode_tampilan"  value="<?php echo $usr_kode_tampilan; ?>">
            ...
            <div class="row form">
              ...
              <div class="col-md-6 col-sm-6 col-xs-6">
                <div class="x_panel">
                  ...
                  <div class="x_content"><br />
                    <div class="item form-group">
                      <input type="hidden" name="waktu_awal" value="<?php if ($_GET['status_pasien'] == 'B') echo date('Y-m-d H:i:s');
                     else echo $_GET['waktu'] ?>">
                     <label class="control-label col-md-3 col-sm-3 col-xs-12" for="name">No RM <span class="required">*</span></label>
                     <div class="col-md-3 col-sm-3 col-xs-3">
                      <input id="cust_usr_kode" name="cust_usr_kode" readonly="readonly" value="<?php echo $row['cust_usr_kode']; ?>" class="form-control col-md-5 col-xs-5" type="text">
                    </div>
                    <div class="col-md-6 col-sm-6 col-xs-6">
                  ...
                  <div class="item form-group">
                    ...
                    <label class="control-label col-md-2 col-sm-2 col-xs-12">Jam Lahir</label>
                    <div class="col-md-3 col-sm-3  col-xs-12">
                  ...
                  <div class="item form-group">
                    ...
                    <div class="col-md-8 col-sm-8 col-xs-12">
                      <select id="cust_usr_jenis_kelamin" class="form-control" name="cust_usr_jenis_kelamin" onKeyDown="return tabOnEnter(this, event);" required>
                        ...
                        <option value="A" <?php if ($row["cust_usr_jenis_kelamin"] == "A") echo "selected"; ?>>Sex Ambigu</option>
                      </select>
                    </div>
                  </div>
                  <div class="item form-group">
                ...
                <div class="item form-group">
                  ...
                  <div class="col-md-8 col-sm-8 col-xs-12">
                    <input type="text" name="alergi" id="alergi" class="form-control" value="<?php echo $row["cust_usr_alergi"]; ?>">
                  </div>
                </div>
                <div class="item form-group">
              ...
              <div class="item form-group">
                ...
                <div class="col-md-8 col-sm-8 col-xs-12">
                  <input type="text" class="form-control" name="cust_usr_no_identitas" id="cust_usr_no_identitas" size="30" maxlength="65" value="<?php echo $row["cust_usr_no_identitas"]; ?>" onKeyDown="return tabOnEnter_select_with_button(this, event);" onKeyUp="funLocation(this.value)">
                </div>
              </div>
                <div class="item form-group">
              ...
              <div class="item form-group">
                ...
                <div class="col-md-8 col-sm-8 col-xs-12">
                  ...
                  <div class="x_panel">
                    <div class="x_content"><br />
                      ...
                      <div class="item form-group">
                        <label class="control-label col-md-3 col-sm-3 col-xs-12" for="nik">No. HP Penanggung Jawab <span class="required">*</span>
                        </label>
                        <div class="col-md-8 col-sm-8 col-xs-12">
                          <input type="text" id="cust_usr_penanggung_jawab_no_hp" name="cust_usr_penanggung_jawab_no_hp" value="<?php echo $_POST["cust_usr_penanggung_jawab_no_hp"]; ?>" maxlength="13" required="required" data-validate-length-range="10,13" class="form-control col-md-7 col-xs-12">

                        </div>
                      </div>

                      <div class="item form-group">
                        <label class="control-label col-md-3 col-sm-3 col-xs-12">Tanggal Lahir Penanggung Jawab<span class="required">*</span></label>
                        <div class="col-md-3 col-sm-3  col-xs-12">
                          <input type="text" class="form-control" id="cust_usr_tanggal_lahir_penanggung_jawab" name="cust_usr_tanggal_lahir_penanggung_jawab" value="<?php echo format_date($row["cust_usr_tanggal_lahir_penanggung_jawab"]); ?>" data-inputmask="'alias': 'dd-mm-yyyy'" onChange="Umur_penanggung_jawab(this.value);" required="required" />
                        </div>
                      </div>


                      <div class="item form-group">
                        <label class="control-label col-md-3 col-sm-3 col-xs-12" for="umur">Umur Penanggung Jawab<span class="required">*</span></label>
                        <div class="col-md-8 col-sm-8 col-xs-12">
                          <input type="text" name="tahun_penanggung_jawab" id="tahun_penanggung_jawab" size="3" maxlength="3" value="<?php echo $tahun_penanggung_jawab; ?>" onKeyDown="return tabOnEnter_select_with_button(this, event);" onChange="TanggalLahir_penanggung_jawab(this.value);" /> tahun
                          <input type="text" name="bulan_penanggung_jawab" id="bulan_penanggung_jawab" size="3" maxlength="3" value="<?php echo $bulan_penanggung_jawab; ?>" onKeyDown="return tabOnEnter_select_with_button(this, event);" onChange="TanggalLahir_penanggung_jawab(this.value);" /> bulan
                          <input type="text" name="hari_penanggung_jawab" id="hari_penanggung_jawab" size="3" maxlength="3" value="<?php echo $hari_penanggung_jawab; ?>" onKeyDown="return tabOnEnter_select_with_button(this, event);" onChange="TanggalLahir_penanggung_jawab(this.value);" /> hari
                        </div>
                      </div>


                      <div class="item form-group">
                        <label class="control-label col-md-3 col-sm-3 col-xs-12">Pekerjaan Penanggung Jawab</label>
                        <div class="col-md-8 col-sm-8 col-xs-12">
                          <select class="form-control" name="cust_usr_penanggung_jawab_pekerjaan" id="cust_usr_penanggung_jawab_pekerjaan" onKeyDown="return tabOnEnter(this, event);">
                            <option value="">Pilih Pekerjaan</option>
                            <?php for ($i = 0, $n = count($dataPekerjaan); $i < $n; $i++) { ?>
                              <option value="<?php echo $dataPekerjaan[$i]["pekerjaan_id"]; ?>" <?php if ($dataPekerjaan[$i]["pekerjaan_id"] == $row["cust_usr_penanggung_jawab_pekerjaan"]) echo "selected"; ?>><?php echo ($i + 1) . ". " . $dataPekerjaan[$i]["pekerjaan_nama"]; ?></option>
                            <?php } ?>
                          </select>
                        </div>
                      </div>
                      ...
                      <div class="item form-group">
                        <label class="control-label col-md-3 col-sm-3 col-xs-12" for="name">Pendidikan Penanggung Jawab <span class="required">*</span>
                        </label>
                        <div class="col-md-8 col-sm-8 col-xs-12">
                          <select class="form-control" name="cust_usr_penanggung_jawab_pendidikan" id="cust_usr_penanggung_jawab_pendidikan" onKeyDown="return tabOnEnter(this, event);">
                            <option value="--">[ Pilih sekolah ]</option>
                            <?php for ($i = 0, $n = count($dataPendidikan); $i < $n; $i++) { ?>
                              <option value="<?php echo $dataPendidikan[$i]["pendidikan_id"]; ?>" <?php if ($dataPendidikan[$i]["pendidikan_id"] == $row["cust_usr_penanggung_jawab_pendidikan"]) echo "selected"; ?>><?php echo ($i + 1) . ". " . $dataPendidikan[$i]["pendidikan_nama"]; ?></option>
                            <?php } ?>
                          </select>
                        </div>
                      </div>
                ...
                <div class="col-md-6 col-sm-6 col-xs-6">
                  ...
                  <div class="x_panel">
                    ...
                    <div class="x_content">
                      ...
                      <div id="div_reg_waktu" class="col-md-6 col-sm-12 col-xs-12" style="display:block;">
                        <label class="control-label pull-left col-md-12 col-sm-12 col-xs-12" style="text-align:left;">Jam Registrasi</label>
                        <input type="time" name="reg_waktu" class="form-control" value="<?php echo date('H:i'); ?>" >
                      </div>
                ...
                <div id="div_iks" class="col-md-6 col-sm-6 col-xs-12" style="display: none;">
                  ...
                  <select id="tipe_iks" class="select2_single form-control" name="perusahaan" disabled="" required>
                ...
                <div id="div_asal" class="col-md-6 col-sm-12 col-xs-12" style="display:none;">
                  ...
                  <select id="asal_pasien" class="select2_single form-control" name="asal_pasien">
                    <?php if ($dataInstalasiTerakhir['poli_tipe'] == '') { ?>
                      <option value="-">- Pilih instalasi -</option>
                    <?php } ?>
                    <?php if ($dataInstalasiTerakhir['poli_tipe'] == 'J') { ?>
                     <!--    <option value="<?php echo $dataInstalasiTerakhir['poli_tipe'] ?>">Rawat Jalan</option> -->
                     <option value="J">Rawat Jalan</option>
                   <?php } ?>
                   <?php if ($dataInstalasiTerakhir['poli_tipe'] == 'G') { ?>
                     <option value="G">IGD</option> 
                     <!--  <option value="<?php echo $dataInstalasiTerakhir['poli_tipe'] ?>">Rawat Jalan</option> -->
                   <?php }
                   ?>
              ...
              <div class="col-md-6 col-sm-12 col-xs-12">
                ...
                <select id="reg_rujukan_id" class="select2_single form-control" name="reg_rujukan_id" required>
              ...
              <div id="isi_lainnya_" class="col-md-6 col-sm-6 col-xs-12" style="display: none;">
               <label class="control-label col-md-12 col-sm-12 col-xs-12" style="text-align:left;"></label>
               <input name="isi_lainnya_text" type="text" class="form-control" id="isi_lainnya" placeholder="" required="" value=" ">
...
<script type="text/javascript">
  ...
  function pilih_kecamatan(isi,kota,prop,kec) {
    if(kota == null) kota = isi;
    $.getJSON('select_kota.php?prop='+prop+'&kota=' + kota, function(nilai) {
      $("#kec").html(`<option>- Pilih Kecamatan -</option>`);
      $.each(nilai, function(index, val) {
        if (val.lokasi_kecamatan == kec && val.lokasi_kabupatenkota == kota && val.lokasi_propinsi == prop) {
          var pilihankec = 'selected'
        }
        $("#kec").append(
          "<option value = '" + val.lokasi_kecamatan + "&kota2=" + val.lokasi_kabupatenkota + "&prop2=" + val.lokasi_propinsi + "' " + pilihankec + ">" + val.lokasi_nama + "</option>"
          );
      });
    });
  }

  function pilih_kelurahan(isi,prop,kota,kecamatan) {
    if(kecamatan == null) kecamatan = isi;
    $.getJSON('select_kota.php?prop2='+prop+'&kota2=' + kota+'&kecamatan2='+kecamatan, function(nilai) {
  ...
  // <?php 
  // if ($sep_sameday['sep_sameday'] > 0 && $sep_sameday['string_agg'] <> '') { ?>
  //   swal({
  //     title: '',
  ...
  /* POP UP PASIEN MASIH ADA Lab */
  <?php if ($kunjungan['jkn_irj'] > 0) { ?>
    swal({
      title: '',
      html: '<h3>Pasien masih di <?= $kunjungan["string_agg"] ?>.</h3>',
      type: 'warning',
      timer: 10000,
      showConfirmButton: true,
      backdrop: 'rgba(0,0,0,0.8)'
  ...
  /* POP UP PASIEN RENCANA PULANG */
  <?php if ($pas_rencana_pulang['jkn_irna'] > 0) { ?>
    swal({
      title: '',
      html: '<h3>Pasien Rencana Pulang !!!</h3>',
      type: 'warning',
      timer: 10000,
      showConfirmButton: true,
      backdrop: 'rgba(0,0,0,0.8)'
  ...
  /* POP UP PASIEN Belum Verif */
  <?php
  $lanjut="registrasi_pasien.php?usr_id=".$_GET['usr_id']."&status_pasien=L&lanjut=1";
  if ($pas_belum_verif['jumlah'] > 0 && $_GET['lanjut']!='1') { ?>
    swal({

      title: '<span > Pasien Registrasi Online Silahkan Lakukan Verifikasi .</span>',
      html: '<span> Mohon Tanyakan Kepada Pasien Dulu  Sebelum Melanjutkan</span>',
      type: 'warning',
