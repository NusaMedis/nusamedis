<?php     
    ...
    $userName = $auth->GetUserName();     
     ...
     $findPage = "cari_tarif.php?";
      ...
      $editPage = "biaya_askep_edit.php";
      $backPage = "biaya_askep_view.php";
      
      if ($_GET["id"])
         $tableHeader = "EDIT SETUP BIAYA ASKEP "; 
      else
         $tableHeader = "TAMBAH SETUP BIAYA ASKEP";
	
	function CheckDataIcd($icdNomor,$biayaVisiteId=null)
	{
          global $dtaccess;                  
	        $sql = "SELECT * FROM global.global_biaya_visite
              WHERE biaya_visite_id =".$_GET["id"];
          $rs = $dtaccess->Execute($sql);
          $dataAdaIcd = $dtaccess->Fetch($rs);
            
		return $dataAdaIcd["biaya_visite_id"];
     }  
     ...
     if ($_GET["id"]) {         
          if ($_POST["btnDelete"]) { 
               ...
               $_x_mode = "Edit";
               $biayaVisiteId = $enc->Decode($_GET["id"]);
          }
           
          $sql = "SELECT a.*,b.biaya_nama FROM global.global_biaya_visite a left join klinik.klinik_biaya b on a.id_biaya=b.biaya_id
              WHERE a.biaya_visite_id= ".QuoteValue(DPE_CHAR,$biayaVisiteId); 
	...
	elseif($_x_mode=="Edit") $privMode = PRIV_UPDATE;
     ...
     if ($_POST["btnSave"] || $_POST["btnUpdate"]) {          
          if($_POST["btnUpdate"]){
               ...
               $biayaVisiteId = & $_POST["biaya_visite_id"];
               $_x_mode = "Edit";
          }
         
          if ($err_code == 0) {
               $dbTable = "global.global_biaya_visite";
               
               $dbField[0] = "biaya_visite_id";   // PK
               $dbField[1] = "id_biaya";
 			
               if(!$biayaVisiteId) $biayaVisiteId = $dtaccess->GetTransID();   
               $dbValue[0] = QuoteValue(DPE_CHAR,$biayaVisiteId); 
     ...
     //delete
      if ($_GET["del"]) 
      {
          $biayaVisiteId = $enc->Decode($_GET["id"]);
           ...
           $sql = "delete from global.global_biaya_visite where biaya_visite_id = ".QuoteValue(DPE_CHAR,$biayaVisiteId);
