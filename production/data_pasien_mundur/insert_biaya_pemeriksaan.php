...
/*              $sql_rawat = "select * from klinik.klinik_perawatan 
            ...
            if(!$dataPerawat){
              ...
              $sql = "select a.*,b.*,c.*  from  klinik.klinik_biaya_pemeriksaan a                       
                      ...
                      a.id_tipe_biaya=".QuoteValue(DPE_CHAR,$_POST["layanan"])." and 
              ...
              $dbValue[0] = QuoteValue(DPE_CHARKEY,$rawatTindId);
                ...
                $idSplit = "1"; //DIPATEN 1 untuk JASA MEDIK  HARUSNYA KONFIGURASI
          		$sql = "select * from  klinik.klinik_biaya_split where id_split = ".QuoteValue(DPE_CHAR,$idSplit)." and id_biaya = ".QuoteValue(DPE_CHAR,$periksa["biaya_id"])." 
                        and bea_split_nominal > 0";
                $dataBiayaSplit = $dtaccess->FetchAll($sql);
          			
          		for($i=0,$n=count($dataBiayaSplit);$i<$n;$i++) 
                  {
                        ...
                        $dbValue[3] = QuoteValue(DPE_NUMERIC,$dataBiayaSplit[$i]["bea_split_nominal"]);
                        ...
                        $folPosisi = '10'; //Dokter Operator
            			$sql = "select biaya_remunerasi_prosentase from klinik.klinik_biaya_remunerasi 
                        where id_biaya =".QuoteValue(DPE_CHAR, $periksa["biaya_id"]);
            			$sql .= " and id_split = ".QuoteValue(DPE_CHAR, $dataBiayaSplit[$i]["id_split"]);
            			$sql .= " and id_folio_posisi = ".QuoteValue(DPE_CHAR, $folPosisi);
            			...
            			//echo $sql; die();
          				...
          				$biaya_split = (int) $dataBiayaSplit[$i]["bea_split_nominal"];
                		...
                		$dbValue[3] = QuoteValue(DPE_CHAR,$folPosisi);
                		$dbValue[4] = QuoteValue(DPE_NUMERIC,$hasilRemunerasi);
                    	$dbValue[5] = QuoteValue(DPE_CHAR,$folioSplit["folsplit_id"]);
                    	$dbValue[6] = QuoteValue(DPE_CHAR,$folPosisi);
