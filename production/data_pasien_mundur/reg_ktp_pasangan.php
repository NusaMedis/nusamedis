    ...
    $sql = "select cust_usr_ktp_pasangan from global.global_customer_user where cust_usr_kode like ".QuoteValue(DPE_CHAR,$_GET["nama"]."%")."
            order by cust_usr_when_update desc";
    $rs = $dtaccess->Execute($sql);
    $custFoto = $dtaccess->Fetch($rs);
    
    $foto = explode("_",$custFoto["cust_usr_ktp_pasangan"]); 
        
    // --- buat foto ---
    if(!$_GET["orifoto"]) $oriFoto = "ktp_psg_pasien_".$foto[2];
    ...
    if($_POST["btnSave"]){
        ...
        if($_FILES["fotopas"]["tmp_name"]){
            switch($_FILES["fotopas"]["type"]){
                case "image/gif":
                    $destName = "ktp_psg_pasien_".$namaFoto."_1.gif";
                    break;
                case "image/jpeg":
                case "image/pjpeg":
                    $destName = "ktp_psg_pasien_".$namaFoto."_1.jpg";
                    break;
                case "image/png":
                    $destName = "ktp_psg_pasien_".$namaFoto."_1.png";
                    break;
            }
...
<?php if($err_code == 0){ ?>
    <script>
        window.opener.document.frmEdit.cust_usr_ktp_pasangan.value='<?php echo $destName;?>';
        window.opener.document.original2.src='<?php echo $lokasi."/".$destName;?>';
        window.close();
    </script>
<?php } ?>
