<?php
   ...
   //die();
    ...
    if ($_POST["instalasi"]=="I") {
      $sql .= " and reg_tipe_rawat = 'I' ";
    }elseif ($_POST["instalasi"]=="G"){
      $sql .= " and reg_tipe_rawat = 'G' ";
	...
	$diff = $today->diff($biday);	
     ...
     else //Jika Belum pernah mendaftar hari ini maka ciptakan klinik_pembayaran
	 {
			...
			if ($_POST["instalasi"]=="I") 
      {
        ...
        $status = "G0"; 
        $tipe_rawat = 'G'; 
        if(!empty($_POST['klinik_asal'])) { 
          $poli_asal = $_POST['klinik_asal'] ; 
        } else {
          $poli_asal = $_POST['klinik'] ; 
        }
      }
      ...
      $dbValue[38] = QuoteValue(DPE_DATE,date_db($_POST["reg_tanggal"]));
      ...
      $dbValue[40] = QuoteValue(DPE_CHAR,$_POST["perusahaan"]);
		...
		//print_r($dbValue); die();
         ...
         unset($dbKey);
            ...
            if(!$dataPerawat){
		      ...
		      if ($_POST["reg_status_pasien"] == "B") include("insert_biaya_kartu.php");
          //if (!$AdaDataReg){
          ...
          $dbValue[3] = QuoteValue(DPE_DATE,date_db($_POST['reg_tanggal'])." ".date("H:i:s"));
       ...
       /* if ($AdaDataReg) { //Jika Sudah pernah mendaftar hari ini maka ambil id_pembayaran dari reg yang lama
          ...
          $sql = "select pembayaran_id from klinik.klinik_pembayaran where pembayaran_id = '$byrId' and id_reg in (select reg_id from klinik.klinik_registrasi where reg_id = '$reg_utama')";
          $xxx = $dtaccess->Fetch($sql);

          if (count($xxx) < 1){ # jika id_reg di pembayaran tidak ada di tabel registrasi
             $sql = "update klinik.klinik_pembayaran set id_reg = '$regId' where pembayaran_id = '$byrId'";
