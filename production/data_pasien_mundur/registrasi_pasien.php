<?php    
     ...
     $CustUsrNama = str_replace("'", "*", $_POST["cust_usr_nama"]);    
    ...
    #konfigurasi 
    $sql = "select dep_konf_reg_no_rm_depan, dep_konf_reg_banyak, dep_konf_reg_ulang from global.global_departemen";
    ...
    if(!$_POST["reg_no_rujukan"]) $_POST["reg_no_rujukan"]="0";
     ...
     $sql .= " WHERE id_cust_usr = ".QuoteValue(DPE_CHAR,$_GET['usr_id'])." and reg_tanggal='".date('Y-m-d')."' and (reg_tipe_rawat = 'J' or reg_tipe_rawat = 'G') and (reg_status = 'E1' or reg_status = 'G1') and (b.poli_tipe !='L' and b.poli_tipe !='R' and b.poli_tipe !='A')";  //and reg_jenis_pasien = '5'
     $pas_irj = $dtaccess->Fetch($sql);  
     //echo $sql;

     # poli sebelumnya menggunakan bpjs
     $sql = "select count(reg_id) as sep_sameday, string_agg(reg_no_sep, ', ')
            from klinik.klinik_registrasi a
            left join global.global_auth_poli b on a.id_poli = b.poli_id ";
     $sql .= " WHERE id_cust_usr = ".QuoteValue(DPE_CHAR,$_GET['usr_id'])." and reg_tanggal='".date('Y-m-d')."' and (reg_tipe_rawat = 'J' or reg_tipe_rawat = 'G') and (b.poli_tipe !='L' and b.poli_tipe !='R' and b.poli_tipe !='A') and reg_jenis_pasien = '5' and (reg_no_sep != '' or reg_no_sep is null)"; 
     //echo $sql; 
    ...
    # cek lagi
     ...
     $sql = "select count(reg_id) as jkn_irj, string_agg(poli_nama, ', ')
            from klinik.klinik_registrasi a
            left join global.global_auth_poli b on a.id_poli = b.poli_id";
     $sql .= " WHERE id_cust_usr = ".QuoteValue(DPE_CHAR,$_POST['cust_usr_id'])." and reg_tanggal='".date_db($_POST['reg_tanggal'])."' and (reg_tipe_rawat = 'J' or reg_tipe_rawat = 'G') and (reg_status = 'E1' or reg_status = 'G1') and (b.poli_tipe !='L' and b.poli_tipe !='R' and b.poli_tipe !='A')";  //and reg_jenis_pasien = '5'
    ...
    # pasien irj di poli yang sama
     $sql = "select count(reg_id) as sameday
            from klinik.klinik_registrasi a ";
     $sql .= " WHERE id_cust_usr = ".QuoteValue(DPE_CHAR,$_POST['cust_usr_id'])." and id_poli = ".QuoteValue(DPE_CHAR,$_POST['klinik'])." and reg_tanggal='".date_db($_POST['reg_tanggal'])."' and reg_tipe_rawat = 'J'";  
     $sameday_irj = $dtaccess->Fetch($sql);      
     # pasien nama & tgl lahir sama
     $sql = "select count(cust_usr_id) as exist, string_agg(cust_usr_kode, ', ')
            from global.global_customer_user ";
     $sql .= " WHERE UPPER(cust_usr_nama) like ".QuoteValue(DPE_CHAR,'%%'.strtoupper($CustUsrNama).'%%')." and cust_usr_tanggal_lahir = ".QuoteValue(DPE_DATE,date_db($_POST["cust_usr_tanggal_lahir"]));  
    ...
    // Pasien Lunas
    $sql = "select count(fol_id) as lunas from klinik.klinik_folio ";
    $sql .= " WHERE id_cust_usr = ".QuoteValue(DPE_CHAR,$_POST['cust_usr_id'])." and tindakan_tanggal='".date_db($_POST['reg_tanggal'])."' and fol_lunas = 'n'";  
    $lunas = $dtaccess->Fetch($sql); 
    // End Pasien Lunas

    if ( $jkn_irj['jkn_irj'] > 0   ){  echo "<script>alert('Pelayanan di poli ".$jkn_irj['string_agg']." belum di selesaikan.'); window.history.back();</script>"; exit;} 
    elseif ($konf['dep_konf_reg_ulang'] != 'y' && $sameday_irj['sameday'] > 0  ){  echo "<script>alert('Pasien sudah terdaftar di klinik'); window.history.back();</script>"; exit;}
    ...
    elseif ( $pasien_exist['exist'] > 0 && $_POST['reg_status_pasien'] == "B" ){  echo "<script>alert('Pasien memiliki nama dan tanggal lahir yang sama dengan pasien No RM ".$pasien_exist['string_agg']." '); window.history.back();</script>"; exit;}  
    // Pasien Lunas
    elseif ($konf['dep_konf_reg_banyak'] == 'y' && $lunas['lunas'] > 0 && $_POST['instalasi'] != 'I'){  echo "<script>alert('Pasien Masih Belum Melunasi Tindakan Sebelumnya'); window.history.back();</script>"; exit;} 
    // End Pasien Lunas

    //echo  $sameday_irj['sameday'].$jkn_irj['jkn_irj'].$jkn_irna['jkn_irna'].$pasien_exist['exist'];
    //die();
    if ($konf['dep_konf_reg_ulang'] != 'y') {
        $ulang = 1;
    } else {
        $ulang = 100;
    }
    if ($sameday_irj['sameday'] < $ulang && $jkn_irj['jkn_irj'] < 1 && $jkn_irna['jkn_irna'] < 1 ) {   
   // die();
    ...
    if($_POST['reg_status_pasien'] == "B" || $_POST['reg_status_pasien'] == "L" ) { 
        ...
        if (empty($_POST['kota'])) {
            ...
            $kota = $lokasidaerah["lokasi_kabupatenkota"];
        }
        if (empty($_POST['kec'])) {
            $kec = $_POST['kec'];
        } else {
            $kec = $lokasidaerah["lokasi_kecamatan"];
        }
        if (empty($_POST['kel'])) {
            $kel = $_POST['kel'];
        } else {
            $kel = FormatNoTrans($lokasidaerah["lokasi_kelurahan"]);
        }
         ...
         if ($norm_depan == 'n') {
          $dbField[30] = "cust_usr_kode";
          $dbField[31] = "cust_usr_kode_tampilan";
          if($_POST["id_card"] == "KTP") $dbField[32] = "cust_usr_nik";
         }else{ if($_POST["id_card"] == "KTP") { $dbField[30] = "cust_usr_nik";} }
         ...
         if ($norm_depan == 'n') {
          $dbValue[30] = QuoteValue(DPE_CHAR,$_POST["cust_usr_kode"]);
          $dbValue[31] = QuoteValue(DPE_CHAR,$_POST["cust_usr_kode_tampilan"]);
          $dbValue[32] = QuoteValue(DPE_CHAR,$_POST["cust_usr_no_identitas"]);
        }else{ if($_POST["id_card"] == "KTP") { $dbValue[30] = QuoteValue(DPE_CHAR,$_POST["cust_usr_no_identitas"]); } }
...
//cari data pekerjaan
 ...
 //combo negara kebangsaan
     ...
     $sql = "select * FROM global.global_lokasi where lokasi_propinsi='35' and lokasi_kecamatan='00' and lokasi_kelurahan='0000' and lokasi_kabupatenkota!='00' order by lokasi_nama";
    ...
    if ($_GET['usr_id']) { 
     ...
     $Cust_Usr_Nama = str_replace("*", "'", $row["cust_usr_nama"]);

     $sql = "select length(cust_usr_kode) as rm from global.global_customer_user where cust_usr_id = '$_GET[usr_id]'";
     $dataRM = $dtaccess->Fetch($sql);
     //print_r($row);
     #NO RM
                  ...
                  <div class="x_content">
                    <br />
                     ...
                     <div class="item form-group">
                        ...
                        <?php if ($dataRM["rm"]!='8' && $_GET["status_pasien"]=='L') { ?>
                            <div class="col-md-3 col-sm-3 col-xs-3">
                              <input id="cust_usr_kode" name="cust_usr_kode" readonly="readonly" value="<?php echo "0".$usr_kode;?>" class="form-control col-md-5 col-xs-5" type="text">
                            </div>
                        <?php } else{?>
                            <div class="col-md-3 col-sm-3 col-xs-3">
                      ...
                      <div class="field form-group">
                        ...
                        <div class="col-md-8 col-sm-8 col-xs-12">
                          <input id="cust_usr_nama" name="cust_usr_nama" value="<?php echo $Cust_Usr_Nama;?>" class="form-control col-md-7 col-xs-12" data-validate-length-range="2" data-validate-words="2"  placeholder="dua kata contoh:Moch Mansyur" required="required" type="text"> 
                     ...
                     <div class="item form-group">
                      ...
                      <div class="item form-group">
                        <label class="control-label col-md-3 col-sm-3 col-xs-12" for="alamat">Alergi<span class="required">*</span>
                        </label>
                        <div class="col-md-8 col-sm-8 col-xs-12">
                          <textarea class="form-control" id="cust_usr_alergi" name="cust_usr_alergi"> <?php echo htmlspecialchars($row["cust_usr_alergi"]);?></textarea>
                        ...
                        <div class="col-md-8 col-sm-8 col-xs-12">
                          <select class="form-control" name="id_prop" id="id_prop" onchange="ajaxkota(this.value)">
                            ...
                            for($i=0,$n=count($dataProvinsi);$i<$n;$i++) { ?>  
                              ...
                              <?php if($dataProvinsi[$i]["lokasi_propinsi"]==$row["id_prop"]) { echo "selected"; } elseif($dataProvinsi[$i]["lokasi_propinsi"]=='35') echo "selected";?>
                        ...
                        <div class="col-md-8 col-sm-8 col-xs-12">
                          <select class="form-control" name="kota" id="kota" onchange="ajaxkec(this.value)">
                            ...
                            for($i=0,$n=count($dataKota);$i<$n;$i++) { ?>  
                              <option value="<?php echo $dataKota[$i]['lokasi_kabupatenkota'].'&prop=35';?>" 
                                <?php if($dataKota[$i]["lokasi_kabupatenkota"]==$row["id_prop"]) {echo "selected"; } elseif ($dataKota[$i]["lokasi_kabupatenkota"]=='17') echo "selected";?>><?php echo $dataKota[$i]['lokasi_nama'];?></option>';
                            <? } ?>                                                                   
                            <option value="0">Tidak Tahu</option>
                          </select> 
                        </div>
...
<!--                 <div class="clearfix"><br></div>
            ...
            /*if ($sep_sameday['sep_sameday'] > 0 ){
                echo "<script>alert('Pasien sudah menggunakan layanan BPJS hari ini dengan No SEP ".$sep_sameday['string_agg'].". Registrasi selanjutnya harus sebagai pasien umum!')</script>";
            }*/

            if ($sep_sameday['sep_sameday'] > 0 ){
                echo "<script>swal({
                    title: '',
                    html:  '<h4>Pasien sudah menggunakan layanan BPJS hari ini dengan No SEP <b>".$sep_sameday['string_agg']."</b></h4>',
                    type: 'warning',
                    timer: 10000,
            ...
            if ( $pas_irj['jkn_irj'] > 0 ){
                echo "<script>swal({
                    title: '',
                    html:  '<h4>Pasien masih di poli ".$pas_irj['string_agg'].". Sebelum registrasi minta petugas terkait menyelesaikan pelayanan.</h4>',
                    type: 'warning',
                    timer: 10000,
                    showConfirmButton: true,
                    backdrop: 'rgba(0,0,0,0.8)'
                }); </script>";
            } else if ( $pas_irna['jkn_irna'] > 0 ){
                echo "<script>swal({
                    title: '',
