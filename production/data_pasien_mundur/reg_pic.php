    ...
    $sql = "select cust_usr_foto from global.global_customer_user where cust_usr_foto like 'pasien_foto_%'
            order by cust_usr_when_update desc";
    $rs = $dtaccess->Execute($sql);
    $custFoto = $dtaccess->Fetch($rs);
    
    $foto = explode("_",$custFoto["cust_usr_foto"]); 
        
    // --- buat foto ---
    if(!$_GET["orifoto"]) $oriFoto = "pasien_foto_".$foto[2];
    ...
    if($_POST["btnSave"]){
        ...
        if($_FILES["fotopas"]["tmp_name"]){
            switch($_FILES["fotopas"]["type"]){
                case "image/gif":
                    $destName = "pasien_".$namaFoto."_".$counter."_.gif";
                    break;
                case "image/jpeg":
                case "image/pjpeg":
                    $destName = "pasien_".$namaFoto."_".$counter."_.jpg";
                    break;
                case "image/png":
                    $destName = "pasien_".$namaFoto."_".$counter."_.png";
                    break;
            }
...
<?php if($err_code == 0){ ?>
    <script>
        window.opener.document.frmFind.cust_usr_foto.value='<?php echo $destName;?>';
        window.opener.document.original.src='<?php echo $lokasi."/".$destName;?>';
        window.close();
    </script>
<?php } ?>
