...
/*	   $sql = "select * from apotik.apotik_conf where id_dep = ".QuoteValue(DPE_CHAR,$_POST["klinik"]);
  ...
  //   $delimiter = ";";
     ...
     $tglcreate = date_db($_POST["opname_tanggal"])." ".$_POST['opname_waktu'];
 ...
 //       echo $sql;
   ...
   //   echo "<br>logistik_stok_batch_dep_periode"; print_r($dbValue);     
     ...
     $sql = "select item_id from logistik.logistik_item where
             item_kode = ".QuoteValue(DPE_CHAR,$data[1]);
     $rs = $dtaccess->Execute($sql);
     $iditem1 =$dtaccess->Fetch($rs);
     $iditem= $iditem1['item_id'];
     $tglop= QuoteValue(DPE_DATE,date_db($_POST["opname_tanggal"]));


     $sql = "select stok_item_saldo from logistik.logistik_stok_item where
             id_item = ".QuoteValue(DPE_CHAR,$iditem)." and 
             id_gudang = ".QuoteValue(DPE_CHAR,$_POST["id_gudang"])." and
             stok_item_create < ".$tglop."
...
// echo  "<br>stok real input".$_POST["stokRealtot".$i];
     ...
     //echo "jumlah : ".$datastoksebelumOpname[$i]["stok_item_saldo"];
     //die();
...
// echo $sql; //die();   
     ...
     // $iditem =$dtaccess->Fetch($rs);
      ...
      $dbValue[2] = QuoteValue(DPE_CHAR,$iditem);
      $dbValue[3] = QuoteValue(DPE_CHAR,$_POST["id_gudang"]);
      $dbValue[4] = QuoteValue(DPE_CHAR,'O');
      $dbValue[5] = QuoteValue(DPE_DATE,$tglop);
 ...
 //      }
     ...
     //if($data[1]=="c3ae3a0364c0e97e0b1bf54252bbeac3") echo $sql; die();
          ...
          $dbValue[1] = QuoteValue(DPE_CHAR,$iditem);//QuoteValue(DPE_NUMERIC,number_format($_POST['txtJumlah']));
          $dbValue[2] = QuoteValue(DPE_NUMERIC,StripCurrency($data[3]));    
          $dbValue[3] = QuoteValue(DPE_DATE, $tglop);//$tglcreate
          $dbValue[4] = QuoteValue(DPE_DATE,date_db($_POST["opname_tanggal"]));
          $dbValue[5] = QuoteValue(DPE_CHAR,$_POST["id_gudang"]);
          $dbValue[6] = QuoteValue(DPE_CHAR,$depId);
  ...
  //     echo $sql;
          ...
          $dbValue[1] = QuoteValue(DPE_CHAR,$iditem);//QuoteValue(DPE_NUMERIC,number_format($_POST['txtJumlah']));
          $dbValue[2] = QuoteValue(DPE_NUMERIC,StripCurrency($data[3]));    
          $dbValue[3] = QuoteValue(DPE_DATE,$tglop);
          $dbValue[4] = QuoteValue(DPE_DATE,date_db($_POST["opname_tanggal"]));
          $dbValue[5] = QuoteValue(DPE_CHAR,$_POST["id_gudang"]);
          $dbValue[6] = QuoteValue(DPE_CHAR,$depId);
     ...
     <tr>
      ...
      <div class="col-md-4 col-sm-6 col-xs-12" align="right">
       ...
       <tr><td>     
            ...
            <tr>
              <td>
            <div class="col-md-4 col-sm-6 col-xs-12" align="right">
              <label class="control-label col-md-12 col-sm-12 col-xs-12">Waktu Opname (Jam:Menit:Detik)</label>
              </div>
              <div class="col-md-3 col-sm-3 col-xs-6">  
              <div class='input-group date' id='datepicker'>
              <input name="opname_waktu" type='text' class="form-control"  
              value="<?php if ($_POST['optane_waktu']) { echo $_POST['optane_waktu']; } else { echo date('H:i:s'); } ?>"  />
            </div>                   
            </div>
              </td>
            </tr>
      ...
      <div class="col-md-4 col-sm-6 col-xs-12" align="right">
              ...
              <select class="select2_single form-control" name="delimiter" id="delimiter" onKeyDown="return tabOnEnter(this, event);" required="required"> <!-- onChange="this.form.submit();" --> 
                        <option value="" >[ Pilih Jenis Pemisah ]</option>
