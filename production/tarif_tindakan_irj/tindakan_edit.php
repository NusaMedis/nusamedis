<?php
  ...
  $backPage = "tindakan_view.php?id_kategori_tindakan_header_instalasi=".$_POST["id_kategori_tindakan_header_instalasi"]."&id_kategori_tindakan_header=".$_POST["id_kategori_tindakan_header"]."&biaya_kategori=".$_POST["biaya_kategori"];
     ...
     if($_GET["id"] || $_GET["id_dep"]) 
     {
          ...
          $sql = "select h.biaya_nama, h.biaya_urut,a.*, b.kategori_tindakan_id, b.id_kategori_tindakan_header,b.kategori_tindakan_nama, 
              ...
              left join klinik.klinik_biaya h on a.id_biaya = h.biaya_id
              join klinik.klinik_kategori_tindakan b on b.kategori_tindakan_id = h.biaya_kategori
              left join gl.gl_perkiraan e on e.id_prk = h.id_prk 
              left join gl.gl_perkiraan f on f.id_prk = h.id_prk_beban
          ...
          $dtaccess->Clear($rs_edit);   
           ...
           //isi biaya split jaspel       
           $sql = "select * from klinik.klinik_biaya_split 
              where id_split = '1' and id_biaya_tarif = ".QuoteValue(DPE_CHAR,$_POST["biaya_tarif_id"]);
           $rs = $dtaccess->Execute($sql);
           $dataBiayaJaspel= $dtaccess->Fetch($rs); 
           
           //isi biaya split sarana
           $sql = "select * from klinik.klinik_biaya_split 
              where id_split = '2' and id_biaya_tarif = ".QuoteValue(DPE_CHAR,$_POST["biaya_tarif_id"]);
           $rs = $dtaccess->Execute($sql);
	...
	elseif($_x_mode=="Edit") $privMode = PRIV_UPDATE;
     ...
     { 
          ...
          if($_POST["btnUpdate"])
          {
               ...
               $dbValue[9] = QuoteValue(DPE_CHAR,$_POST["is_cito"]);
                //print_r($dbValue);
                //die();
           ...
           $dataBiayaJaspel= $dtaccess->Fetch($rs); 
            ...
            if($_POST["biaya_split_jaspel"])
            {
               ...
               if ($dataBiayaJaspel) $biayaJaspelId = $dataBiayaJaspel["biaya_split_id"];
               else $biayaJaspelId = $dtaccess->GetTransId();  
           ...
           $dataBiayaSarana= $dtaccess->Fetch($rs); 
            ...
            if($_POST["biaya_split_sarana"])
            {
               ...
               if ($dataBiayaSarana) $biayaSaranaId = $dataBiayaSarana["biaya_split_id"];
               else $biayaSaranaId = $dtaccess->GetTransId();  
...
//      if(($_POST["biaya_kategori"] <> $_POST["biaya_kategori_lama"]) || $_POST["btnSave"]){
  ...
  //      $sql = "update klinik.klinik_biaya set biaya_urut = null where biaya_id = ".QuoteValue(DPE_CHAR,$biayaTarifId);
...
//       var_dump($dataDbLog);
       ...
       for($i=0,$n=count($dataBiaya);$i<$n;$i++) {       
         ...
         $dtaccess->Execute($sql);
			...
			//$sql = "update klinik.klinik_biaya set biaya_total = ".QuoteValue(DPE_NUMERIC,$beaNominal)." where biaya_id = ".QuoteValue(DPE_CHAR,$biayaTarifId);
			//$dtaccess->Execute($sql);
          ...
          <div class="">
            ...
            <div class="form-group">
             ...
             <label class="control-label col-md-3 col-sm-3 col-xs-12" for="first-name">Tgl Awal Tarif Tindakan  <span class="required">*</span>
              ...
              <div class="col-md-6 col-sm-6 col-xs-12">
                  <?php echo $view->RenderTextBox("biaya_tarif_tgl_awal","biaya_tarif_tgl_awal","85","100",format_date($_POST["biaya_tarif_tgl_awal"]),"inputField", null,false);?>
						</div>
					  </div>
            <div class="form-group">
             <label class="control-label col-md-3 col-sm-3 col-xs-12" for="first-name">Tgl Akhir Tarif Tindakan  <span class="required">*</span>
              ...
              <div class="col-md-6 col-sm-6 col-xs-12">
                  <?php echo $view->RenderTextBox("biaya_tarif_tgl_akhir","biaya_tarif_tgl_akhir","85","100",format_date($_POST["biaya_tarif_tgl_akhir"]),"inputField", null,false);?>
						</div>
					  </div> 
            <div class="form-group">
            ...
            <!--
					  ...
					  <input type="hidden" name="split_1" id="split_1" value="<?php echo $_POST["split_1"];?>" />
						...
						<?php echo $view->RenderHidden("biaya_tarif_id","biaya_tarif_id",$biayaTarifId);?>
