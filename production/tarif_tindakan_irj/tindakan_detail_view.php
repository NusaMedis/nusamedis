...
//              left join gl.gl_perkiraan f on f.id_prk = a.id_prk_beban
   	  ...
   	  $sql = "select h.*,a.*, b.kategori_tindakan_id, b.id_kategori_tindakan_header,b.kategori_tindakan_nama, c.dep_nama, 
              d.kegiatan_kategori_nama, j.jenis_nama, 
     ...
     $tbHeader[0][$counterHeader][TABLE_ISI] = "CITO";
     $tbHeader[0][$counterHeader][TABLE_WIDTH] = "5%";
     $counterHeader++;
...
//     $counterHeader++; 
     ...
     for($i=0,$counter=0,$n=count($dataTable);$i<$n;$i++,$counter=0){
            ...
            $tbContent[$i][$counter][TABLE_ISI] = $ketCito[$dataTable[$i]["is_cito"]];
	          $tbContent[$i][$counter][TABLE_ALIGN] = "left";
	          $counter++;
...
//	          $counter++;   
	  ...
	  //        $counter++;
       ...
       // } else {
        ...
        if($isAllowedUpdate)
        {
               $tbContent[$i][$counter][TABLE_ISI] = '<a href="'.$editPage.'&check='.$enc->Decode($_GET["check"]).'&id='.$enc->Encode($dataTable[$i]["biaya_tarif_id"])."&biaya_tarif_tgl_awal=".format_date($dataTable[$i]["biaya_tarif_tgl_awal"])."&biaya_tarif_tgl_akhir=".format_date($dataTable[$i]["biaya_tarif_tgl_akhir"]).'"><img hspace="2" width="25" height="25" src="'.$ROOT.'gambar/icon/edit.png" alt="Edit" title="Edit" border="0"></a>';
        } else {
               $tbContent[$i][$counter][TABLE_ISI] = '&nbsp;';                
        }
               $tbContent[$i][$counter][TABLE_ALIGN] = "center";
    ...
    $dataKategoriTindakan = $enc->Decode($_GET["check"]); 
