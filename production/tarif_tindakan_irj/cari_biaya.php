...
function GetData($in_nama=null){
  ...
  //perkiraan yang di select hanya kas saja
	$sql = "select *  from  klinik.klinik_biaya where id_dep = ".QuoteValue(DPE_CHAR,$depId);  
    ...
    $tbContent[$i][$counter][TABLE_ISI] = "&nbsp;".$dataTable[$i]["biaya_kode"];
		...
		$tbContent[$i][$counter][TABLE_ISI] = '<img src="'.$ROOT.'gambar/icon/cari.png" style="cursor:pointer" border="0" alt="Pilih" title="Pilih" width="30" height="30" class="img-button" OnClick="javascript: sendValue(\''.addslashes(htmlentities($dataTable[$i]["biaya_nama"])).'\',\''.$dataTable[$i]["biaya_kode"].'\',\''.$dataTable[$i]["biaya_id"].'\')"/>';    
		$tbContent[$i][$counter][TABLE_ALIGN] = "center";
		$tbContent[$i][$counter][TABLE_CLASS] = $class;                    
		$counter++;
...
function sendValue(nama,no,id,urut) {
  ...
  //self.parent.document.getElementById('biaya_kode').value = no;
	...
	<td>
		<table cellpadding="1" cellspacing="1" border="0" align="center" width="100%">
			<tr class="tablesmallheader" >
				<td colspan="2"><center>Pencarian&nbsp;Master Tindakan</center></td>
			</tr> 
			<tr>
				<td align="right" class="tablecontent">Nama Tindakan</td>
