...
if(isset($_POST["instalasi_id"]) && !empty($_POST["instalasi_id"])){
     ...
     if($_POST["instalasi_id"] == 'J') {
        $kondisi = "  poli_tipe <> 'I' and poli_tipe <> 'G' and poli_tipe <> 'A'  ";
     } else {
  ...
  // die($sql);
    ...
    if($rowCount > 0){
        ...
        for($i=0,$n=count($dataDokter);$i<$n;$i++){?> 
            ...
            if($_POST["dokter"]==$dataDokter[$i]["usr_id"]){echo"selected";}.'>'.$dataDokter[$i]["usr_name"].'</option>'; -->
            <option class="form_control" value="<?php echo $dataDokter[$i]['usr_id'];?>"<?php if ($dataDokter[$i]['usr_id']==$_POST['id_dokter'] ) { echo "selected";} ?>><?php echo $dataDokter[$i]['usr_name']; ?></option>
...
if(isset($_POST["kamar_id"]) && !empty($_POST["kamar_id"])){
    ...
    $sql = "select bed_id, bed_kode from klinik.klinik_kamar_bed where 1=1 ";// bed_reserved='n' and bed_keterangan='n'";
