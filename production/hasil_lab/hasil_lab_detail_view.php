...
$kembaliPage = "hasil_lab_view.php?id_kategori_tindakan_header_instalasi=" . $_POST["id_kategori_tindakan_header_instalasi"] . "&id_kategori_tindakan_header=" . $_POST["id_kategori_tindakan_header"] . "&biaya_kategori=" . $_POST["biaya_kategori"];
$editPage = "tindakan_detail_add.php?id_biaya=" . $biayaId;
$thisPage = "hasil_lab_view.php";
$isAllowedUpdate = 1;
...
if ($biayaId && $biayaId != "--") $sql_where[] = "h.id_biaya = " . QuoteValue(DPE_CHAR, $biayaTarifId);
...
$sql = "select h.*,a.*, b.kategori_tindakan_id, b.id_kategori_tindakan_header,b.kategori_tindakan_nama, c.dep_nama, 
              ...
              where " . $sql_where;
$sql .= " order by g.kategori_tindakan_header_urut,b.kategori_urut,a.biaya_urut ASC";
...
if ($isAllowedUpdate) {
  $tombolAdd = '<input type="button" name="btnAdd" value="Tambah" class="pull-right col-md-12 col-sm-12 col-xs-12 btn btn-primary" onClick="document.location.href=\'' . $addPage . '?id_biaya=' . $biayaId . '\'"></button>';
...
$sql = "select * FROM klinik.klinik_hasil_lab where id_biaya = " . QuoteValue(DPE_CHAR, $id_biaya);
...
for ($i = 0, $counter = 0, $n = count($dataTable); $i < $n; $i++, $counter = 0) {
  ...
  $tbContent[$i][$counter][TABLE_ISI] = $dataTable[$i]["hasil_lab_keterangan"];
  $tbContent[$i][$counter][TABLE_ALIGN] = "left";
  $counter++;
  ...
  // {
  //        $tbContent[$i][$counter][TABLE_ISI] = '<a href="'.$addPage.'&id='.($dataTable[$i]["biaya_id"]).'"><img hspace="2" width="25" height="25" src="'.$ROOT.'gambar/icon/add.png" alt="Tambah" title="Tambah" border="0"></a>';
  // } else {
  ...
  $tbContent[$i][$counter][TABLE_ISI] = '<a href="' . $editPage . '&id=' . ($dataTable[$i]["hasil_lab_id"]) . '"><img hspace="2" width="25" height="25" src="' . $ROOT . 'gambar/icon/edit.png" alt="Edit" title="Edit" border="0"></a>';
  $tbContent[$i][$counter][TABLE_ALIGN] = "center";
  $counter++;

  $tbContent[$i][$counter][TABLE_ISI] = '<a href="' . $editPage . '&parent=' . ($dataTable[$i]["hasil_lab_kode"]) . '"><img hspace="2" width="25" height="25" src="' . $ROOT . 'gambar/anak.png" alt="Tambah Anak" title="Tambah Anak" border="0"></a>';
  $tbContent[$i][$counter][TABLE_ALIGN] = "center";
  $counter++;

  $tbContent[$i][$counter][TABLE_ISI] = '<a href="' . $editPage . '&id=' . ($dataTable[$i]["hasil_lab_id"]) . '&del=1"><img hspace="2" width="25" height="25" src="' . $ROOT . 'gambar/icon/hapus.png" alt="Hapus" title="Hapus" border="0" onclick="javascript: return Hapus();"></a>';
  $tbContent[$i][$counter][TABLE_ALIGN] = "center";
  $counter++;
}
        ...
        <div class="">
          ...
          <div class="row">
            <div class="col-md-12 col-sm-12 col-xs-12">
              <div class="x_panel">
                <div class="x_title">
                  <h2>Nama Tindakan : <?php echo $dataTindakan["biaya_nama"]; ?></h2>
                  <div class="clearfix"></div>
                </div>
                <div class="x_content">
