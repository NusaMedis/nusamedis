...
class Registry {
    ...
    */
    function Registry($in_sess_id = "expressa") {
    ...
    function &Instance($in_sess_id = "expressa") {
...
class Session {
     ...
     function Read($sessID) {
          // fetch session-data
          $sql = "SELECT sess_data FROM   global_session WHERE
     ...
     function Write($sessID,$sessData)
     {
          ...
          $sql = "select sess_id from   global_session where sess_id = ".QuoteValue(DPE_CHAR,$sessID);
          $rs = $this->_dataAccess->Execute($sql,DB_SCHEMA_GLOBAL);
          $dataSession = $this->_dataAccess->Fetch($rs);
          
          $isSave = ($dataSession["sess_id"]) ? false : true;
          if($isSave){
                $sql = "insert into   global_session(sess_id, sess_expires, sess_data) values (".
                        QuoteValue(DPE_CHAR,$sessID).",".
                        QuoteValue(DPE_NUMERIC,$newExp).",".
                        QuoteValue(DPE_CHAR,$sessData).")";
          } else {
                $sql = "update   global_session set ".
     ...
     function Destroy($sessID) {
          ...
          $sql = "delete from   global_session where sess_id = ".QuoteValue(DPE_CHAR,$sessID);
          $rs = $this->_dataAccess->Execute($sql,DB_SCHEMA_GLOBAL);
     ...
     function Gc($sessMax_lifeTime) {
          $sql = "delete from   global_session where sess_expires < ".time();
          $rs = $this->_dataAccess->Execute($sql,DB_SCHEMA_GLOBAL);
