<?php
     ...
     if($login && !$dataTable)
     {
      ...
      //echo 'Login True';
      echo header("Location:../bag_umum/menu_umum.php");
         exit();
     }
     ...
     if($login && $dataTable) {
        ...
        if($_POST["cmbSystem"]==1){
         //echo 'login true';//"<script>top.location.href='menukeu.php'</script>";    
         echo header("Location:../bag_umum/menu_umum.php");   
			exit();         
     ...
     elseif(!$login)
       {
         header("Location:../bag_umum.php?ref=64&msg=kode_eror01&user=".$_POST["txtUser"]);
       } 
     elseif(!$dataTable)
       {
