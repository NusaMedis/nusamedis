<?php
     ...
     if($login && !$dataTable)
     {
      ...
      //echo 'Login True';
      echo header("Location:../dem/menu_dem.php");
         exit();
     }
     ...
     if($login && $dataTable) {
        ...
        if($_POST["cmbSystem"]==1){
         //echo 'login true';//"<script>top.location.href='menu_dem.php'</script>";    
         echo header("Location:../dem/menu_dem.php");   
			exit();         
     ...
     elseif(!$login)
       {
         header("Location:../index.php?ref=10&msg=kode_eror01&user=".$_POST["txtUser"]);
       } 
     elseif(!$dataTable)
       {
