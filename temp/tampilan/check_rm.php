<?php
     ...
     if($login && !$dataTable)
     {
      ...
      //echo 'Login True';
      echo header("Location:../rekam_medik/menu_rm.php");
         exit();
     }
     ...
     if($login && $dataTable) {
        ...
        if($_POST["cmbSystem"]==1){
         //echo 'login true';//"<script>top.location.href='menu_rekam_medik.php'</script>";    
         echo header("Location:../rekam_medik/menu_rm.php");   
			exit();         
     ...
     elseif(!$login)
       {
         header("Location:../index.php?ref=11&msg=kode_eror01&user=".$_POST["txtUser"]);
       } 
     elseif(!$dataTable)
       {
