<?php
      ...
      $da = $dtaccess->Execute($sql,DB_SCHEMA_GLOBAL);
      $poli_id = $dtaccess->Fetch($da); 
                      
      $login = $auth->IsLoginOk($_POST["txtUser"],$_POST["txtPass"],$poli_id['poli_id']);
     ...
     if($login && !$dataTable)
     {
      ...
      //echo 'Login True';
      echo header("Location:../ugd/menu_ugd.php");
         exit();
     }
     ...
     if($login && $dataTable) {
        ...
        if($_POST["cmbSystem"]==1){
         //echo 'login true';//"<script>top.location.href='menu_ugd.php'</script>";    
         echo header("Location:../ugd/menu_ugd.php");   
			exit();         
     ...
     elseif(!$login)
       {
         header("Location:../index.php?ref=4&msg=kode_eror01&user=".$_POST["txtUser"]);
       } 
     elseif(!$dataTable)
       {
