<?php
     ...
     if($login && !$dataTable)
     {
      ...
      //echo 'Login True';
      echo header("Location:../loket_rehab/menu_loket_rehab.php");
         exit();
     }
     
     if($login && $dataTable) {

        if($_POST["cmbSystem"]==46){
         //echo 'login true';//"<script>top.location.href='menu_loket_rehab.php'</script>";    
         echo header("Location:../loket_rehab/menu_loket_rehab.php");   
			exit();         
     ...
     elseif(!$login)
       {
         header("Location:../index.php?msg=kode_eror01&user=".$_POST["txtUser"]);
       } 
     elseif(!$dataTable)
       {
