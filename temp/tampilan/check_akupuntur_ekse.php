<?php
     ...
     if($login && !$dataTable)
     {
      ...
      //echo 'Login True';
      echo header("Location:../poli_akupuntur_reg/menu_akupuntur_ekse.php");
         exit();
     }
     
     if($login && $dataTable) {

        if($_POST["cmbSystem"]==45){
         //echo 'login true';//"<script>top.location.href='menu_orto_ekse.php'</script>";    
         echo header("Location:../poli_akupuntur_ekse/menu_akupuntur_ekse.php");   
			exit();         
     ...
     elseif(!$login)
       {
         header("Location:../akupuntur_ekse.php?ref=47&msg=kode_eror01&user=".$_POST["txtUser"]);
       } 
     elseif(!$dataTable)
       {
