<?php
     ...
     if($login && !$dataTable)
     {
      ...
      //echo 'Login True';
      echo header("Location:../antrian_executive/menu_antrian.php");
         exit();
     }
     ...
     if($login && $dataTable) {
        ...
        if($_POST["cmbSystem"]==1){
         //echo 'login true';//"<script>top.location.href='menu_antrian.php'</script>";    
         echo header("Location:../antrian_executive/menu_antrian.php");   
			exit();         
     ...
     elseif(!$login)
       {
         header("Location:../index.php?ref=33&msg=kode_eror01&user=".$_POST["txtUser"]);
       } 
     elseif(!$dataTable)
       {
